...
describe("site logo preparation", () => {
  it("rasterizes without cropping and closes the decoded image", async () => {
    ...
    vi.stubGlobal(
      "createImageBitmap",
      vi.fn<() => Promise<typeof bitmap>>().mockResolvedValue(bitmap),
    );
  ...
  it("rejects dangerous raster dimensions before decoding", async () => {
    ...
    for (const [name, type, data] of [
      ...
      await expect(prepareSiteLogo(new File([data], `logo.${name}`, { type }))).rejects.toThrow(
        "dimensions",
      );
    }
    expect(decode).not.toHaveBeenCalled();
  ...
  it("rejects encoded PNG output over the backend limit", async () => {
    ...
    await expect(
      prepareSiteLogo(
        new File([pngHeader(100, 100)], "logo.png", {
          type: "image/png",
        }),
      ),
  ...
  it("rejects SVGs with embedded raster resources before native decoding", async () => {
    ...
    const svg =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:x="http://www.w3.org/2000/svg"><x:image href="d&#97;ta:image/png;base64,AA=="/></svg>';

    await expect(
      prepareSiteLogo(new File([svg], "logo.svg", { type: "image/svg+xml" })),
