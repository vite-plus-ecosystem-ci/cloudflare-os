...
export default function ResourcePicker({
  ...
  const buildRefineUrl = useCallback(
    (suffix: string, replaceSearch?: boolean) => {
  ...
  const searchHasPlaceholders = lowerSearch
    ? getPlaceholderRanges(searchText.trim()).length > 0
    : false;
  ...
  const hasSpecificMatches = matchedResources.some(
    ({ resource }) => resource.urlPattern !== "https://*",
  );

  if (lowerSearch && httpItem && hasSpecificMatches) {
    matchedResources = matchedResources.filter(
      ({ resource }) => resource.urlPattern !== "https://*",
    );

    const httpVendorName = httpItem.vendor.description.displayName;
    const httpAccounts = [...allAccounts.entries()].filter(
      ([_, { vendor: v }]) => v.displayName === httpVendorName,
    );
  ...
  useEffect(() => {
    if (ready)
      sessionSnapshots.set(authenticatedApi, { vendors: allVendors, accounts: allAccounts });
  ...
  const handleGrantResourceAccess = useCallback(
    async (accountId: number, resourceUrlPatterns: string[]) => {
    ...
    <div style={style}>
      <div className="overflow-y-auto" style={{ maxHeight }}>
        {!ready ? (
          ...
          (() => {
            let itemIdx = 0;
            return matchedResources.map(
              ({ resource, vendor, classification, suffix, replaceSearch, accountsOnly }, i) => {
                ...
                return (
                  <div
                    key={`${vendor.id}-${resource.urlPattern}`}
                    className={i > 0 ? "border-t border-kumo-line" : ""}
                  >
                    ...
                    {vendorAccounts.map((account) => {
                      ...
                      const accountRow = (
                        <div
                          onClick={() => {
                            ...
                            if (needsAccess) {
                              if (!isGranting)
                                void handleGrantResourceAccess(account.id, missingGrants);
                          ...
                          className={`${PICKER_ROW} ${isActive ? PICKER_ROW_ACTIVE : ""} ${
                            (isExpired && !isReconnecting) || needsAccess || searchHasPlaceholders
                              ? "opacity-70"
                              : ""
                          }`}
                          style={{
                            cursor: searchHasPlaceholders
                              ? "default"
                          ...
                          {isReconnecting || isGranting ? (
                            ...
                            <span className="flex flex-shrink-0 items-center gap-1">
                              ...
                              <span className="text-[11.5px] leading-4 text-kumo-warning">
                                Expired — click to re-authenticate
                              </span>
                            </span>
                          ) : needsAccess ? (
                            <span className="flex flex-shrink-0 items-center gap-1">
                      ...
                      if (searchHasPlaceholders) {
                        return (
                          <Tooltip
                            key={account.id}
                            content="Replace all placeholders in the URL before selecting an account"
                            asChild
                          >
                            {accountRow}
                          </Tooltip>
