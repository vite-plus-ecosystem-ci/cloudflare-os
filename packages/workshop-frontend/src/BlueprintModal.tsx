...
async function compressBlueprintScreenshot(file: File): Promise<Blob> {
  ...
  try {
    ...
    ctx.drawImage(
      ...
      0,
      0,
      BLUEPRINT_SCREENSHOT_WIDTH,
      BLUEPRINT_SCREENSHOT_HEIGHT,
    );
...
export default function BlueprintModal({ open, onClose, overseer, gadget, metadata }: Props) {
  ...
  const savedScreenshotUrl =
    formMode === "edit" && editingBlueprint?.screenshotUrl && !clearScreenshot
  ...
  return (
    ...
    >
      <Dialog
        className="responsive-dialog !z-[1000] !top-[clamp(24px,10vh,80px)] !flex !max-h-[calc(100vh-clamp(24px,10vh,80px)-24px)] !w-[min(640px,calc(100vw-32px))] !-translate-y-0 flex-col overflow-hidden bg-kumo-base p-0"
        size="lg"
      >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="m-0 mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                        ...
                        editingBlueprint?.screenshotUrl &&
                        !clearScreenshot
                          ? " The current screenshot will stay unless you upload a new one."
                          : ""}
                      </p>
                    </div>
                  ...
                  <div className="space-y-2">
                    {bindings.map((b) => (
                      <BlueprintBindingCard
                        ...
                        data={b}
                        onChange={(annotation) =>
                          updateBindingAnnotation(b.bindingName, annotation)
                        }
                      />
                    ))}
                  </div>
                    ...
                    <BlueprintRow
                      ...
                      onUpdateCode={async () => {
                        try {
                          ...
                          toasts.add({
                            title: "Blueprint updated to current code.",
                            variant: "success",
                          });
                          loadBlueprints();
                        } catch (err: any) {
                          toasts.add({
                            title: err.message || "Failed to update blueprint.",
                            variant: "error",
                          });
                        }
                      }}
                      onRetryPublish={async () => {
                        try {
                          ...
                          toasts.add({
                            title: "Blueprint published successfully.",
                            variant: "success",
                          });
                          loadBlueprints();
                        } catch (err: any) {
      ...
      >
        <div className="flex w-full flex-wrap items-center gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="m-0 mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              People who started a gadget from this blueprint won't be affected, but the link will
              stop working.
            </p>
          </div>
          <button
