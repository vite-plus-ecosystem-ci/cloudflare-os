...
function accountLabel(account: AccountInfo | undefined, accountId: number): string {
  return (
    account?.description.uniqueName || account?.description.displayName || `Account ${accountId}`
  );
}
...
function missingResourceUrlPatterns(account: AccountInfo, required: string[]): string[] {
  ...
  return granted === undefined
    ? required
    : required.filter((pattern) => !granted.includes(pattern));
}
...
export default function ObserverConfigModal({
  ...
  useEffect(() => {
    setChoices((prev) => {
      ...
      for (const need of needs) {
        ...
        const failed =
          need.failure && accounts.has(need.failure.accountId) ? need.failure.accountId : undefined;
  ...
  const handleConnect = async (need: ObserverBindingNeed) => {
    ...
    try {
      ...
      if (vendor?.description.autoProvisionsAccount) {
        ...
        openConnectWindow(
          await authenticatedApi.connectAccount(
            vendorId,
  ...
  const handleGrantResourceAccess = async (need: ObserverBindingNeed, account: AccountInfo) => {
    const required = requiredResourceUrlPatterns(need, vendorsById.get(need.vendorId), account);
          ...
          next.set(account.id, {
            ...
            description: {
              ...
              grantedResourceUrlPatterns: [
                ...new Set([...(current.description.grantedResourceUrlPatterns ?? []), ...missing]),
              ],
            },
          });
          return next;
        ...
        {!ready || !vendorsReady ? (
          ...
          <div className="flex flex-col gap-4 mt-5">
            {needs.map((need) => {
              ...
              return (
                <div
                  key={need.gatekeeperId}
                  className="rounded-xl border border-kumo-line bg-kumo-base p-4"
                >
                  <div
                    className={`flex items-center gap-3${matching.length === 0 && !need.failure ? "" : " mb-3"}`}
                  >
                    <Avatar
                      src={vendor?.logo?.url}
                  ...
                  {need.failure && (
                    <div
                      className={`flex items-start gap-2 px-3 py-2 rounded-md text-xs text-kumo-warning bg-kumo-warning-tint border border-kumo-warning/20${matching.length === 0 ? "" : " mb-3"}`}
                    >
