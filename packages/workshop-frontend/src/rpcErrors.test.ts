...
vi.mock("./errorReporting", () => ({
  reportIssue: vi.fn<(site: string, err: unknown, options?: object) => void>(),
}));
...
describe("classifyRpcError", () => {
  ...
  it("trusts the durableObjectReset flag over an unrecognized message", () => {
    expect(classifyRpcError(rpcError("internal error", { durableObjectReset: true }))).toBe(
      "do-reset",
    );
  });
  ...
  it("prefers do-reset when both reset and retryable flags are set", () => {
    expect(classifyRpcError(rpcError("x", { durableObjectReset: true, retryable: true }))).toBe(
      "do-reset",
    );
  });
  ...
  it("classifies capnweb transport messages as connection", () => {
    ...
    expect(
      classifyRpcError(new Error("RPC session was shut down by disposing the main stub")),
    ).toBe("connection");
    expect(
      classifyRpcError(new Error("Attempted to use RPC stub after it has been disposed.")),
    ).toBe("connection");
  });
...
describe("capnweb error serialization", () => {
  it("round-trips the custom properties the classifier reads", () => {
    const sent = Object.assign(storageTimeoutReset(), {
