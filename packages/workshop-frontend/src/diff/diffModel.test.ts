...
describe("buildDiffModel", () => {
  ...
  it("expands a sub-word edit to whole-word inline highlights", () => {
    ...
    const modText = change.inlineModifiedRanges
      .get(1)!
      .map((s) => "the bat sat".slice(s.startCol - 1, s.endCol - 1))
      .join("");
    expect(modText).toBe("bat");
    const origText = change.inlineOriginalRanges
      .get(1)!
      .map((s) => "the cat sat".slice(s.startCol - 1, s.endCol - 1))
      .join("");
  ...
  it("highlights only the replaced word of a mostly-unchanged sentence", () => {
    ...
    const modText = change.inlineModifiedRanges
      .get(1)!
      .map((s) => modified.slice(s.startCol - 1, s.endCol - 1))
      .join("");
    expect(modText).toBe("nearly");
    const origText = change.inlineOriginalRanges
      .get(1)!
      .map((s) => original.slice(s.startCol - 1, s.endCol - 1))
      .join("");
