...
import { Pencil, Trash, Blueprint, Warning, X } from "@phosphor-icons/react";
...
export default function Connections({
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ name: string; resourceTitle: string } | null>(
    null,
  );
  const [deleteHookTarget, setDeleteHookTarget] = useState<{ id: number; title: string } | null>(
    null,
  );
                  ...
                  >
                    {isDeleting ? (
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="truncate text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            The binding <span className="font-mono">{gk.name}</span> will be removed
                            from this gadget.
                          </p>
                        </div>
                        <WorkshopButton
                        ...
                        <WorkshopButton onClick={() => setDeleteTarget(null)}>
                          Cancel
                        </WorkshopButton>
                      </div>
                    ) : isEditing ? (
                      <div className="flex items-center gap-2">
                        ...
                        <WorkshopButton onClick={handleEditCancel}>Cancel</WorkshopButton>
                      </div>
                    ) : (
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="flex items-center gap-2 truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-default">
                            ...
                            {isPending && (
                              <Tooltip
                                content="Added in this chat; kept when you accept the chat's changes"
                                asChild
                              >
                        ...
                        <div className="ml-auto flex shrink-0 items-center gap-1">
                          ...
                          <Tooltip content="Delete connection" asChild>
                            <WorkshopIconButton
                              danger
                              onClick={() =>
                                setDeleteTarget({ name: gk.name, resourceTitle: gk.resourceTitle })
                              }
                    ...
                    {isDeleting ? (
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="truncate text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            This permanently removes the hook. Future events will stop being
                            delivered.
                          </p>
                        </div>
                        <WorkshopButton
                        ...
                        <WorkshopButton onClick={() => setDeleteHookTarget(null)}>
                          Cancel
                        </WorkshopButton>
                      </div>
                    ) : (
                        ...
                        <div className="ml-auto flex shrink-0 items-center gap-2">
                          ...
                          <Tooltip content="Delete hook" asChild>
                            <WorkshopIconButton
                              danger
                              onClick={() =>
                                setDeleteHookTarget({ id: hook.id, title: hook.description.title })
                              }
...
function BlueprintAnnotationModal({
  ...
  return (
    <>
      ...
      >
        <Dialog
          className="responsive-dialog !z-[1000] !w-[min(480px,calc(100vw-32px))] overflow-hidden bg-kumo-base p-0"
          size="lg"
        >
          ...
          <div className="border-t border-kumo-line px-4 py-3 sm:px-5">
            ...
            <div className="flex items-center justify-end gap-2">
              <WorkshopButton onClick={onClose} disabled={saving}>
                Cancel
              </WorkshopButton>
              <WorkshopButton tone="primary" onClick={handleSave} disabled={saving || !data}>
                {saving ? "Saving..." : "Save"}
              </WorkshopButton>
            </div>
          </div>
