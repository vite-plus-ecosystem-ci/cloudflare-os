...
export default function SandboxedGatekeeperApp({
  ...
  const accentColor =
    configuredAccentColor && isHexColor(configuredAccentColor) ? configuredAccentColor : null;
  ...
  const present = useCallback<PresentController>(
    (active) => {
  ...
  const openTarget = useCallback<OpenTarget>(
  ...
  const resolveWorkspaceTitles = useCallback<ResolveWorkspaceTitles>(
    async (ids) => {
      let entry = titlesRef.current;
      if (!entry || Date.now() - entry.at >= WORKSPACE_TITLES_TTL_MS) {
        entry = {
          at: Date.now(),
          titles: authenticatedApi
            .listGadgets()
  ...
  const openPrompt = useCallback<OpenPrompt>(
    (prompt) => {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (
        forwardTrustedFrameError(event, frameWindow, {
