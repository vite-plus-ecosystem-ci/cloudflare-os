...
class ResourceConfiguratorHostImpl extends RpcTarget implements ResourceConfiguratorHost {
  ...
  constructor(
    ...
    private readonly getInitialResourceImpl: () => {
      resourceUrl: string;
...
export default function SandboxedResourceConfigurator({
  ...
  const [frameRect, setFrameRect] = useState<{ top: number; left: number; width: number } | null>(
    null,
  );
  const [clipInsets, setClipInsets] = useState<{
    top: number;
    right: number;
    bottom: number;
    left: number;
  ...
  const initialResourceRef = useRef<{ resourceUrl: string; resourceUrlPattern: string } | null>(
    null,
  );
  initialResourceRef.current =
    initialResourceUrl && resourceUrlPattern
  ...
  const findScrollAncestor = (): HTMLElement | null => {
    if (scrollAncestorRef.current && scrollAncestorRef.current.isConnected)
      return scrollAncestorRef.current;
  ...
  const lastClipRef = useRef<{ top: number; right: number; bottom: number; left: number } | null>(
    null,
  );
  ...
  const connectIframe = (port: MessagePort) => {
    ...
    rpcSessionRef.current?.[Symbol.dispose]?.();
    const iframe = newMessagePortRpcSession<ResourceConfiguratorIframe>(
      port,
      new ResourceConfiguratorHostImpl(
        configuratorRef.current,
        (nextHeight, nextLayoutHeight) => {
          if (!Number.isFinite(nextHeight)) return;
          const maxHeight = Math.max(
            MIN_CONFIGURATOR_HEIGHT,
            Math.min(MAX_CONFIGURATOR_HEIGHT, window.innerHeight - 120),
          );
  ...
  const collectResourceUrl = () => {
    if (iframeInvalidatedRef.current)
      return Promise.reject(new Error("Configurator is no longer available."));
    const iframe = iframeRpcRef.current;
    if (!iframe || !iframeConnectedRef.current)
      return Promise.reject(new Error("Configurator is not ready."));
  ...
  const applyForwardedScroll = (deltaX: number, deltaY: number) => {
    ...
    scrollFrameRef.current = requestAnimationFrame(() => {
      ...
      const deltaX = clamp(
        pendingScrollRef.current.x,
        -SCROLL_FORWARD_MAX_DELTA,
        SCROLL_FORWARD_MAX_DELTA,
      );
      const deltaY = clamp(
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const key = rect
        ? `${Math.round(rect.width)}x${Math.round(rect.left)}x${Math.round(rect.top)}`
        : "";
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (frameWindow && forwardTrustedFrameError(event, frameWindow, { surface: "configurator" }))
        return;
