  ...
  const handleAddCollaborator = async () => {
    ...
    try {
      ...
      if (result === null) {
        ...
        toasts.add({
          title: `Added ${result.profile.name} as a collaborator.`,
          variant: "success",
        });
      }
    } catch (err: any) {
          ...
          {sharingProhibited ? (
            <div className="flex h-full flex-col items-center justify-center px-6 py-12 text-center">
              ...
              <p className="mt-2 max-w-[320px] text-balance text-[12px] leading-[18px] tracking-[-0.1px] text-kumo-subtle">
                To share something similar, create a blueprint from a gadget in this workspace, then
                use it to create a new workspace.
              </p>
            </div>
          ) : (
            <>
              ...
              >
                ...
                >
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (!isImeComposing(e) && e.key === "Enter") handleAddCollaborator();
                    }}
                ...
                <div className="overflow-hidden rounded-2xl border border-kumo-line/80 bg-kumo-base">
                  {collaboratorRows.map((row, index) => {
                    ...
                    const isRemoving =
                      ...
                      removeTarget?.profileId === row.info.profile.id;
                    ...
                    return (
                      <div
                        key={key}
                        className={`group ${index > 0 ? "border-t border-kumo-line/70" : ""} ${landedPersonId === profile.id ? "share-row-land" : "transition-colors duration-150 hover:bg-kumo-elevated/50"} px-3 py-2.5`}
                      >
                            ...
                            <div className="mt-2.5 share-expand-in">
                              ...
                              <DependentKeepList
                                ...
                                onKeepSetChange={(keepSet) =>
                                  setConfirmationTarget((current) =>
                                    current?.kind === "revoke" &&
                                    current.linkId === revokeTarget.linkId
                                      ? { ...current, keepSet }
                                      : current,
                                  )
                                }
