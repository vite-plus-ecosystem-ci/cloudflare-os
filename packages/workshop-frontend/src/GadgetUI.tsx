...
function GadgetUISession({
      ...
      try {
        const replacementStub = await Promise.race([
          ...
          new Promise<never>((_, reject) => {
            timeout = setTimeout(
              () => reject(new Error("Timed out reconnecting gadget UI.")),
  ...
  useEffect(() => {
    ...
    if (
      ...
      reloadTrigger !== prevReloadTriggerRef.current &&
      reloadTrigger > 0
    ) {
  ...
  if (!isVisible && !hasLoaded) {
    ...
    return (
      <div className="flex items-center justify-center text-kumo-subtle" style={{ height }}>
        <Text variant="secondary">Switch to this tab to load the Gadget UI</Text>
      </div>
    );
  }
