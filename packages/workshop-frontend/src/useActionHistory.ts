...
export function useActionHistory(
  ...
  const loadMore = useCallback(() => {
    ...
    if (!overseer || session.inFlight || (session.hasLoadedPage && session.frontier === undefined))
      return;
    const first = !session.hasLoadedPage;
    session.inFlight = true;
    setState((prev) => ({ ...prev, error: null }));

    overseer.listActions({ beforeId: session.frontier, filter }).then(
      (page) => {
  ...
  return {
    ...
    status: (state.error === "initial"
      ? "error"
      : session.hasLoadedPage
        ? "ready"
        : "loading") satisfies ActionHistoryStatus,
