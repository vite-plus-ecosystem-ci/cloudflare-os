...
describe("CodeDiffEditor", () => {
  ...
  async function mount(session: FakeSession, original: string | null) {
    await act(async () => {
      root.render(
        <StrictMode>
          <ThemeProvider>
            <CodeDiffEditor filename="notes.txt" original={original} session={session} />
          </ThemeProvider>
        </StrictMode>,
      );
    });
  ...
  it("shows a whole-file deletion zone after a remote remove", async () => {
    ...
    expect(container.querySelector(".gadgets-deleted-code-zone")?.textContent).toBe("helloworld");
  });
