...
function fetchCommitFiles(
  ...
  if (!cached) {
    cached = overseer
      .getCodeAtCommit(commitId)
      .then(({ files }) => new Map(files) as ReadonlyMap<string, string>);
...
export default function GadgetCodeInterface({
  ...
  const [headFilesState, setHeadFilesState] = useState<{
    commitId: string;
  ...
  const headFiles: ReadonlyMap<string, string> | null =
  ...
  // while loading.
  const baseDisplayFiles: ReadonlyMap<string, string> | null = branchMode
    ? (chatFiles ?? headFiles)
    : headFiles;
  ...
  const ensureEditable = useCallback(() => {
    ...
    if (!activeClient.hasGadget(workpieceIdRef.current)) {
      activeClient.ensureGadgetEditable(
        workpieceIdRef.current,
        headCommitIdRef.current,
        headFilesRef.current ?? EMPTY_FILES,
      );
    }
    return true;
  }, [client]);
  ...
  if (headLoadFailed && headFiles === null) {
    return (
      <div
        ...
        style={{ height }}
      >
        <p className="m-0 text-sm text-kumo-danger">Failed to load this gadget&apos;s code.</p>
        <WorkshopButton
          tone="secondary"
          className="!h-8"
  ...
  const activeFileText = activeFile !== null ? (displayFiles?.get(activeFile) ?? null) : null;
  // The committed side of the diff; null = the file doesn't exist at head (an added file).
  const activeFileOriginal = activeFile !== null ? (headFiles?.get(activeFile) ?? null) : null;
  const activeFileDownloadable = activeFile !== null && displayFiles?.get(activeFile) !== undefined;
