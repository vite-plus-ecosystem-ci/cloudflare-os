...
describe("GadgetExportMenu", () => {
  it("loads formats on open and exports the selected one with its metadata", async () => {
    const exportFormat = vi.fn<
      (id: string, chatId?: number) => Promise<ReadableStream<Uint8Array>>
    ...
    expect(mocks.saveStreamToFile).toHaveBeenCalledWith(expect.any(Function), "Report.csv", {
      description: "CSV",
      contentType: "text/csv",
      extension: ".csv",
    });
  ...
  it("disables the export button and updates its tooltip while exporting", async () => {
    ...
    const client = gadget({
      getExportFormats: vi.fn<(chatId?: number) => Promise<GadgetExportFormat[]>>(async () => [
        format,
      ]),
    });
    ...
    expect(trigger?.closest("[data-tooltip]")?.getAttribute("data-tooltip")).toBe(
      "Exporting to CSV",
    );
  ...
  it("loads fresh formats on every open and ignores a response after close", async () => {
    ...
    const first = new Promise<GadgetExportFormat[]>((resolve) => {
    ...
    const getExportFormats = vi
      .fn<(chatId?: number) => Promise<GadgetExportFormat[]>>()
    ...
    expect(
      container.querySelector('[role="status"][aria-label="Loading export formats"]'),
    ).not.toBeNull();
    await act(async () => {
      button("close export menu")?.click();
    });
    expect(
      container.querySelector('[role="status"][aria-label="Loading export formats"]'),
    ).toBeNull();
