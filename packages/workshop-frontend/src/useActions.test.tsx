...
describe("useActions", () => {
  ...
  it("never re-marks a record pending after a live resolution beat its stale page copy", async () => {
    const server = makeOverseer();
    await view.render(<Probe overseer={server.overseer} />);
    await server.resolveSubscription();
  ...
  it("downgrades ready to error when the subscribe call fails after the pages drained", async () => {
    vi.spyOn(console, "error").mockImplementation(() => {});
    const server = makeOverseer();
    await view.render(<Probe overseer={server.overseer} />);
  ...
  it("updates a late listener without replaying and disposes after the last consumer", async () => {
    ...
    await view.render(<Harness onEntry={firstCallback} showActions={true} showEntries={false} />);
    await server.resolveSubscription();
    await server.emit(entry(1));
    await server.emit(entry(2, { state: "approved" }));

    await view.render(<Harness onEntry={firstCallback} showActions={true} showEntries={true} />);
    expect(firstReceived).toEqual([1, 2]);
    expect(server.subscribeCalls).toHaveLength(1);

    await view.render(<Harness onEntry={secondCallback} showActions={true} showEntries={true} />);
    expect(firstReceived).toEqual([1, 2]);
    ...
    await view.render(<Harness onEntry={secondCallback} showActions={false} showEntries={true} />);
    expect(server.subscriptionDispose).not.toHaveBeenCalled();

    await view.render(<Harness onEntry={secondCallback} showActions={false} showEntries={false} />);
    expect(server.subscriptionDispose).toHaveBeenCalledOnce();
  });
});
