...
import { Plus, Trash, Lightning, MagnifyingGlass, DotsThreeVertical } from "@phosphor-icons/react";
...
function ProvidersPage() {
  ...
  const isBuiltIn = (modelId: string): boolean => {
    ...
    const enabled = new Set(
      (aiConfig as Extract<AiGatewayInfo, { enabled: true }>).enabledProviders,
    );
    ...
    <div className="mx-auto flex h-full w-full max-w-4xl flex-col px-3 sm:px-10">
      <header className="flex flex-col items-stretch gap-4 px-3 pb-3 pt-6 sm:flex-row sm:items-end sm:justify-between sm:pt-10">
        ...
        <button
          ...
          className={`${PRIMARY_BTN} h-11 justify-center text-[14px] sm:h-9 sm:text-[13px]`}
        >
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            ...
            <button
              type="button"
              onClick={fetchAll}
              className="mt-1 cursor-pointer text-kumo-brand underline"
            >
              Try again
            </button>
          </div>
