          ...
          <div className="flex items-start gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[11px] leading-4 font-normal tracking-[-0.1px] text-kumo-subtle">
                Keeps each workspace limited to the resources you connect and ensures every user has
                the required permissions before accessing them.
              </p>
            </div>
          </div>
...
function ConnectorsPage() {
  ...
  const sectionGridClass = view === "list" ? "flex flex-col gap-0.5" : "grid gap-3 sm:grid-cols-2";
    ...
    <div className="h-full overflow-y-auto bg-kumo-base">
      <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-8 sm:py-14">
        <header className="mb-8 grid gap-8 lg:grid-cols-[minmax(0,540px)_444px] lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-[14px] leading-[20px] font-normal tracking-[-0.25px] text-kumo-subtle">
              Add the apps and accounts your workspaces can use. Connect once, then wire them into
              anything you build.
            </p>
          </div>
              ...
              {filteredAccounts.map((account) => {
                ...
                return (
                  <ConnectorCard
                    ...
                    metaLine={
                      <span
                        className={`truncate ${account.credentialsValid ? "" : "text-kumo-danger"}`}
                      >
                        {account.credentialsValid ? displayName : "Credentials expired"}
                      </span>
                    }
                    tagline={tagline}
        ...
        {!initialLoading &&
          ...
          filteredAvailable.length === 0 && (
            <EmptyState
              title={search ? "No gatekeepers match" : "No gatekeepers yet"}
              description={
                search
