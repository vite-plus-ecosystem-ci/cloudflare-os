...
export default function CodeDiffEditor({
  ...
  const [diffLayoutPreference, setDiffLayoutPreference] = useState<DiffLayoutPreference>(
    getInitialDiffLayoutPreference,
  );
  ...
  const dispatchRender = useCallback(() => {
    ...
    modifiedViewRef.current?.dispatch({
      effects: setDiffRender.of({
        ...base,
        layout: splitDiffRef.current ? "split" : "stacked",
      }),
    });
  ...
  const recompute = useCallback(() => {
    ...
    setSummary((prev) =>
      prev.status === model.status &&
      prev.additions === model.additions &&
  ...
  return (
    <div
      ref={containerRef}
      className="flex min-h-0 overflow-hidden bg-kumo-base"
      style={{ height }}
    >
      <div
          ...
          <div className="flex h-7 items-center gap-0.5 rounded-lg border border-kumo-line bg-kumo-base px-0.5 shadow-sm">
            ...
            <button
              ...
              className={layoutButtonClass(
                diffLayoutPreference === "split" && canSplitDiff,
                !canSplitDiff,
              )}
