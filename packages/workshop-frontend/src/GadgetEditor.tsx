...
function persistWorkspaceView(
  gadgetId: string,
  view: { mode: "chat" } | { mode: "app"; appId: WorkpieceId },
) {
  try {
...
export default function GadgetEditor() {
  ...
  const [gadget, setGadget] = useState<{ id: WorkpieceId; stub: RpcStub<GadgetClient> } | null>(
    null,
  );
  ...
  const [liveEditPreviews, setLiveEditPreviews] = useState<ChatLiveEditPreviews | undefined>(
    undefined,
  );
  ...
  const [anyChatProposedWorkpieces, setAnyChatProposedWorkpieces] = useState<
    readonly WorkpieceId[]
  >([]);
  ...
  const [selectedChatProposedWorkpieces, setSelectedChatProposedWorkpieces] = useState<
    readonly WorkpieceId[]
  >([]);
  ...
  const isWorktreeId = (workpieceId: WorkpieceId) =>
    workpieces.get(workpieceId)?.type === "worktree";
  ...
  const pinInitialChatSelection = singleInitialChat && !hasCommittedCode && !userNavigatedToList;
  ...
  const visibleGadgets = useMemo(() => {
    return allGadgets.filter((w) => w.chatId === undefined || w.chatId === effectiveSelectedChatId);
  ...
  const visibleWorkpieces: WorkpieceSummary[] = useMemo(
    () => [...visibleGadgets, ...allWorktrees.filter((w) => w.chatId === effectiveSelectedChatId)],
  ...
  const pendingGadgetIds = useMemo(
    () =>
      new Set(
        allGadgets
          .filter((w) => w.chatId !== undefined && w.chatId === effectiveSelectedChatId)
          .map((w) => w.id),
      ),
    [allGadgets, effectiveSelectedChatId],
  );
  ...
  const selectedWorkpieceSummary =
  ...
  useEffect(() => {
    ...
    const fallback =
      acceptedApps.find((g) => g.id === metadata?.defaultGadgetId) ?? acceptedApps[0];
    if (fallback) {
  ...
  const selectedGadgetStub = gadget !== null && gadget.id === selectedGadgetId ? gadget.stub : null;
  ...
  const streamingActiveFile =
    streamingActiveFileState?.chatId === effectiveSelectedChatId
  ...
  useEffect(() => {
    ...
    overseer.stub
      ...
      .then((hooks) => {
        if (!cancelled)
          setHookedGadgetIds(new Set(hooks.filter((h) => h.enabled).map((h) => h.gadgetId)));
      })
  ...
  const effectiveHasCode =
    selectedGadgetSummary !== undefined ? hasCode : workpiecesReady ? false : null;

  const codeStateReady = effectiveHasCode !== null;
  const hasCodeRelatedState =
  ...
  const showFullEditor =
    layoutModeReady &&
    (showingActivity ||
      (hasVisibleWorkpieces &&
        (workspaceView === null ? !simpleMode : workspaceView.mode === "app")));
  ...
  const workspaceTransitionClass =
    workspaceTransitionEnabled && !isResizing
  ...
  const setWorkspaceVisibility = useCallback(
    (visibility: "open" | "closed", appId?: WorkpieceId) => {
      setWorkspaceTransitionEnabled(true);
  ...
  const closeWorkspacePane = useCallback(() => {
    ...
    const returnShowsPane =
      hasVisibleWorkpieces && (returnView?.mode === "app" || (returnView === null && !simpleMode));
  ...
  useEffect(() => {
    ...
    const target = newlyCreated.findLast(
      (app) => app.chatId !== undefined && app.chatId === effectiveSelectedChatId,
    );
    if (!target) return;
    ...
    (chatId: number | null, options?: { replace?: boolean }) => {
      ...
      navigate({
        ...
        search: (prev: Record<string, unknown>) => ({
          ...
          w: leavingPendingApp ? undefined : typeof prev.w === "number" ? prev.w : undefined,
        }),
  ...
  const handleResizePointerUp = useCallback(
    (e: ReactPointerEvent<HTMLDivElement>) => {
      ...
      const width = e.type === "pointercancel" ? chatWidthRef.current : clampChatWidth(e.clientX);
      setChatWidth(width);
  ...
  const handleSelectWorkpiece = useCallback(
  ...
  const handleRenameWorkpiece = useCallback(
    async (workpieceId: WorkpieceId, title: string) => {
      if (!overseer) return;
  ...
  const handleSaveTitle = async () => {
    ...
    try {
      ...
      toasts.add({ title: "Failed to update title", variant: "error" });
    } finally {
  ...
  const mobilePrimaryActive =
    showFullEditor && !paneShowsActivity && activeTab === mobilePrimaryTab;
    ...
    <div className="relative flex h-full flex-col overflow-hidden bg-kumo-base">
      ...
      <div className="flex h-12 shrink-0 items-center gap-1 border-b border-kumo-line bg-kumo-base px-2 md:hidden">
        ...
        <button
          ...
          disabled={
            mobilePrimaryTab === "app"
              ? !selectedGadgetStub
              : selectedWorkpieceSummary === undefined
          }
        ...
        <DropdownMenu>
          ...
          <DropdownMenu.Content className={MENU_CONTENT} style={MENU_POSITIONER_STYLE}>
            ...
            {visibleWorkpieces.length > 1 &&
              visibleWorkpieces.map((workpiece) => (
                <DropdownMenu.Item
                  key={workpiece.id}
                ...
                >
                  <span className="flex min-w-0 items-center gap-2">
                    <WorkpieceGlyph
                      summary={workpiece}
                      className="flex-shrink-0 text-kumo-subtle"
                    />
      ...
      <div className="flex flex-1 min-h-0 relative overflow-hidden">
        ...
        <div
          ...
          style={{
            width: showFullEditor ? chatWidth : `calc(100% - ${outputRailWidth}px)`,
          }}
        >
          {overseer ? (
                ...
                {paneShowsActivity ? (
                  ...
                  selectedWorkpieceSummary && (
                    <PaneLabel
                      ...
                      badge={
                        selectedWorkpieceSummary.type === "gadget" &&
                        selectedWorkpieceSummary.chatId !== undefined
                          ? "Draft"
                          : undefined
                      }
                    />
              ...
              <div className={paneShowsActivity ? "hidden" : "contents"}>
                ...
                >
                  {selectedGadgetStub && !previewMode ? (
                    ...
                    !previewMode && <NoGadgetPlaceholder height="100%" />
                  )}
                  {isGadgetFullscreen && showFullscreenHint && (
                    ...
                    >
                      <div className="rounded-full border border-kumo-line bg-kumo-base/90 px-4 py-1.5 text-[13px] leading-[18px] text-kumo-default shadow-md backdrop-blur-sm">
                        Press{" "}
                        <kbd className="rounded border border-kumo-line bg-kumo-elevated px-1.5 py-0.5 text-[11px] font-medium">
                          Esc
                        </kbd>{" "}
                        to exit full screen
                      </div>
      ...
      <DeleteConfirmationDialog
        ...
        description={
          <>
            This removes <span className="font-medium text-kumo-default">{metadata.title}</span>.
            You can&apos;t undo this.
          </>
        }
