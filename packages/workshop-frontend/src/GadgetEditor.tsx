...
export default function GadgetEditor() {
  ...
  const pinInitialChatSelection = singleInitialChat && hasCode !== true && !userNavigatedToList;
  ...
  const selectedGadgetSummary =
    selectedGadgetId !== null ? visibleGadgets.find((g) => g.id === selectedGadgetId) : undefined;
  ...
  const hasCodeRelatedState =
    effectiveHasCode === true || hasAnyProposedChanges || streamingActiveFile != null;
  ...
  const closeWorkspacePane = useCallback(() => {
    ...
    const returnShowsPane =
      returnView?.mode === "app" || (returnView === null && hasAnyApps && !simpleMode);
