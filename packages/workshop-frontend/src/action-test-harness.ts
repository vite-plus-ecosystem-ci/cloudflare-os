...
function parkedQueue() {
  ...
  return {
    ...
    async resolveNext(page: ActionHistoryPage) {
      await act(async () => {
        parked.shift()!.resolve(page);
      });
    },
    async rejectNext(err: unknown) {
      await act(async () => {
        parked.shift()!.reject(err);
      });
    },
  };
}
  ...
  return {
    ...
    async resolveSubscription() {
      await act(async () => {
        pendingSubscribes
          .shift()!
          .resolve({ [Symbol.dispose]: subscriptionDispose } as unknown as RpcStub<{}>);
      });
    },
    async rejectSubscription(err: unknown) {
      await act(async () => {
        pendingSubscribes.shift()!.reject(err);
      });
    },
