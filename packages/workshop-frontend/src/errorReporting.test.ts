...
import { FRONTEND_ERROR_MESSAGE_TYPE, type FrontendErrorReportV1 } from "@gadgets/error-reporting";
import {
...
describe("Workshop reporter initialization", () => {
  ...
  it("captures the current origin and pathname with the authenticated user", async () => {
    ...
    expect(
      reports.map(({ pageLocation, reportedUserId }) => ({ pageLocation, reportedUserId })),
    ).toEqual([
...
describe("createBrowserErrorReporter", () => {
  ...
  it("isolates failed transports", async () => {
    const transport = vi
      .fn<(report: FrontendErrorReportV1) => Promise<void>>()
