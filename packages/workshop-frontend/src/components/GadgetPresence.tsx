    ...
    <Popover>
            ...
            <span ref={stackRef} className="relative inline-flex">
              <span className="flex -space-x-2">
                {visible.map((p) => (
                  <Tooltip
                    key={p.user.id}
                    content={`${p.user.name} · ${ROLE_LABELS[p.role]}`}
                    asChild
                  >
                ...
                {overflow > 0 && (
                  <span data-flip-id="__overflow" className="inline-flex cursor-pointer">
                    <span className="presence-pop-in grid h-[26px] w-[26px] place-items-center rounded-full bg-kumo-tint text-[10px] font-semibold text-kumo-strong ring-2 ring-kumo-base">
                      +{overflow}
                    </span>
                  </span>
                )}
      ...
      >
        ...
        <div className="presence-scroll min-h-0 flex-1 overflow-y-auto">
          {display.map((p) => (
            <div key={p.user.id} className="flex items-center gap-2.5 rounded-xl px-2.5 py-1.5">
              <PersonAvatar
                api={authenticatedApi}
                userId={p.user.id}
                name={p.user.name}
