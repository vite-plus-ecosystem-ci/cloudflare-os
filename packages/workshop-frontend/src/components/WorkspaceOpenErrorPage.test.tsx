...
describe("WorkspaceOpenErrorPage", () => {
  ...
  it("explains how to recover when access is denied without exposing workspace metadata", async () => {
    const {
      ...
      onGoToWorkspaces,
      onRetry,
    } = await render("access-denied");

    expect(renderedContainer.querySelector("h1")?.textContent).toBe(
      "You don't have access to this workspace",
    );
    expect(renderedContainer.textContent).toContain(
      "Ask the workspace owner to grant you access, then try again.",
    );
  ...
  it("gives a missing workspace a distinct, non-retryable state", async () => {
    ...
    expect(renderedContainer.textContent).toContain(
      "The link may be incorrect, or the workspace may have been deleted.",
    );
    expect(
      [...renderedContainer.querySelectorAll("button")].map((button) => button.textContent),
  ...
  it("lets a refused redeemer retry once the owner adds them directly", async () => {
    ...
    expect(renderedContainer.querySelector("h1")?.textContent).toBe(
      "Share links are turned off for this workspace",
    );
    expect(renderedContainer.textContent).toContain(
      "Ask the workspace owner to add you directly, then try again.",
    );
  ...
  it("keeps unexpected failures retryable", async () => {
    ...
    expect(renderedContainer.textContent).toContain(
      "Try again. If the problem continues, return to your workspaces.",
    );
    expect(
      [...renderedContainer.querySelectorAll("button")].map((button) => button.textContent),
    ).toEqual(["Go to workspaces", "Try again"]);
  ...
  it("classifies stable open error codes without treating unexpected errors as expected", () => {
    ...
    expect(
      classifyWorkspaceOpenFailure(new Error(OPEN_GADGET_ERROR_CODES.workspaceAccessDenied)),
    ).toBe("unexpected");
