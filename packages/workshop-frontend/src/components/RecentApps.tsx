...
function AppRow({ gadget }: { gadget: GadgetMetadataWithTimestamps }) {
  ...
  return (
    ...
    >
      {/* Gradient swatch */}
      <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${gradient} flex-shrink-0`} />
      ...
      <section className="w-full max-w-2xl mx-auto">
        ...
        <div className="flex flex-col gap-2">
          {[1, 2].map((i) => (
            <div
              key={i}
              className="h-[64px] rounded-xl border border-kumo-line bg-kumo-elevated animate-pulse"
            />
          ))}
        </div>
      </section>
