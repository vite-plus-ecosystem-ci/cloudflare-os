          ...
          <div className="space-y-4">
            {!connected ? (
              <p className="text-sm text-kumo-subtle">
                You've used all {usage.dailyLimit} of your free{" "}
                {usage.dailyLimit === 1 ? "request" : "requests"} for today. Connect your Cloudflare
                account to keep building now — usage beyond the free tier is billed to your own
                Cloudflare AI Gateway credits
                {usage.resetAt ? (
                  <>
                    {" "}
                    — or wait: your free{" "}
                    {usage.dailyLimit === 1 ? "request resets" : "requests reset"} at 00:00 UTC, in{" "}
