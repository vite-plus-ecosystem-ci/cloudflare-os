...
describe("ComposerAddMenu", () => {
  ...
  it.each([
    ...
    "renders search $position when the menu placement requires it",
    async ({ anchorTop, orderClass }) => {
      const getOverseer = () =>
        ({
          listSlashCommands: async () => [connectedSkill],
        }) as unknown as RpcStub<Overseer>;
      const host = await mount(
        <Harness anchorTop={anchorTop} getOverseer={getOverseer} onSelectSkill={() => {}} />,
      );
      await act(async () =>
        host.querySelector<HTMLButtonElement>('[aria-haspopup="dialog"]')!.click(),
      );
  ...
  it("keeps the focused search mounted when placement flips", async () => {
    ...
    await act(async () =>
      root!.render(<Harness anchorTop={500} getOverseer={getOverseer} onSelectSkill={() => {}} />),
    );
  ...
  it("does not offer catalog entries loaded by a previous Overseer source", async () => {
    ...
    const host = await mount(<Harness getOverseer={firstSource} onSelectSkill={() => {}} />);
    ...
    await act(async () =>
      root!.render(<Harness getOverseer={secondSource} onSelectSkill={() => {}} />),
    );
    await act(async () => trigger.click());
  ...
  it("uses Enter and Tab for activation, and restores focus on Escape", async () => {
    ...
    await act(async () =>
      reopened.dispatchEvent(new KeyboardEvent("keydown", { key: "ArrowUp", bubbles: true })),
    );
    await act(async () =>
  ...
  it("refetches the catalog after a connection invalidates it", async () => {
    ...
    const getOverseer = () => ({ listSlashCommands }) as unknown as RpcStub<Overseer>;
    const host = await mount(<Harness getOverseer={getOverseer} onSelectSkill={() => {}} />);
    const trigger = host.querySelector<HTMLButtonElement>('[aria-haspopup="dialog"]')!;
    await act(async () => trigger.click());
    await waitFor(() => document.body.textContent?.includes("Review   writing") === true);
    await act(async () => trigger.click());
