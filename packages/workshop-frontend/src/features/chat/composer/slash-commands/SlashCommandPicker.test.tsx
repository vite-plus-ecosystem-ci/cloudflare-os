  ...
  return (
    <>
      ...
      <button
        ...
        aria-label="Choose second skill"
        onClick={() => picker.setIndex(1)}
      />
      <button
        type="button"
        data-testid="invalidate-catalog"
        aria-label="Invalidate skills"
...
describe("SlashCommandPicker", () => {
  ...
  it("shows the rounded skill-row presentation and filters a single catalog load", async () => {
    ...
    await act(async () =>
      vi.waitFor(() => expect(document.querySelectorAll('[role="option"]')).toHaveLength(2)),
    );
    ...
    expect(document.querySelector('[role="option"]')?.textContent).toContain(
      "deployDeploy the current project.",
    );
    expect(document.querySelector('[role="option"]')?.textContent).toContain(
      "Context · Production",
    );
    ...
    await act(async () =>
      root!.render(<Harness inputValue="/dep" getOverseer={getOverseer} onSelect={() => {}} />),
    );
    await act(async () =>
      vi.waitFor(() => expect(document.querySelectorAll('[role="option"]')).toHaveLength(1)),
    );
  ...
  it("does not select a unique exact command without explicit confirmation", async () => {
    const onSelect =
      vi.fn<(choice: SlashCommandChoice, tokenStart: number, tokenEnd: number) => void>();
    await render(
      <Harness
        inputValue="/deploy production"
        cursorPosition={8}
        getOverseer={() => pickerOverseer(choices)}
        onSelect={onSelect}
      />,
    );
    await act(async () =>
  ...
  it("does not default an ambiguous exact command", async () => {
    ...
    await render(
      <Harness
        inputValue="/deploy"
        getOverseer={() => pickerOverseer(duplicates)}
        onSelect={() => {}}
      />,
    );
    await act(async () =>
  ...
  it("offers built-in commands only after the chat exists", async () => {
    ...
    await act(async () =>
      vi.waitFor(() => expect(document.querySelectorAll('[role="option"]')).toHaveLength(3)),
    );
  });
  ...
  it("rejects stale catalog results after invalidation", async () => {
    ...
    await act(async () =>
      vi.waitFor(() => expect(document.body.textContent).toContain("Debug an issue.")),
    );
  ...
  it("shows a replacement-load error instead of stale commands", async () => {
    ...
    await act(async () =>
      vi.waitFor(() =>
        expect(document.body.textContent).toContain("Couldn’t load commands. disconnected"),
      ),
    );
