    ...
    if (side === "modified") {
      ...
      if (layout === "stacked") {
        ...
        pushZoneAfter(
          deco,
          doc,
          run.modifiedStart + run.modifiedCount - 1,
      ...
      if (run.modifiedCount > run.originalCount) {
        pushZoneAfter(
          deco,
          doc,
          run.originalStart + run.originalCount - 1,
...
function pushLineDecorations({
  ...
  for (let lineNumber = start; lineNumber < start + count; lineNumber++) {
    ...
    const marker =
      side === "modified"
        ? paired.has(lineNumber)
          ? MODIFIED_MARKER
          : ADD_MARKER
        : paired.has(lineNumber)
          ? DELETE_MODIFIED_MARKER
          : DELETE_MARKER;
  ...
  override toDOM(): HTMLElement {
    ...
    for (const row of zoneRows(this.run, this.expanded)) {
      if (row.kind === "omitted") {
        ...
        button.textContent = `Show ${row.hidden} hidden deleted line${row.hidden === 1 ? "" : "s"}`;
...
class DeletionZoneGutterMarker extends GutterMarker {
  constructor(
    readonly widget: DeletionZoneWidget,
    readonly kind: "bar" | "num",
  ) {
    super();
    this.elementClass = kind === "bar" ? "gadgets-deleted-bar-zone" : "gadgets-deleted-margin";
  }
  eq(other: GutterMarker): boolean {
    return (
      other instanceof DeletionZoneGutterMarker &&
