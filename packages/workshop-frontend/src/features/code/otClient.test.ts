...
class TestHarness {
  ...
  ) => Promise<{ generation: number; revision: number }> = async () => ({
    generation: 0,
    revision: 0,
  });
  discards = 0;
...
describe("ChatOtClient", () => {
  ...
  it("submits local edits with a first-keystroke pin declaration and applies the echo", async () => {
    ...
    h.client.pushRow(
      row(0, 1, submission.change, { clientId: submission.clientId, seq: submission.seq }),
    );
    await flush();
    ...
    h.client.pushRow(row(0, 2, second.change, { clientId: second.clientId, seq: second.seq }));
    await flush();
    expect(h.client.hasLocalEdits()).toBe(false);
  ...
  it("rides a content-preserving epoch reset, bridging the pending buffer", async () => {
    ...
    h.client.pushRow(
      row(1, 1, h.submissions[0].change, {
        clientId: h.submissions[0].clientId,
        seq: h.submissions[0].seq,
      }),
    );
    await flush();
    expect(h.client.hasLocalEdits()).toBe(false);
  ...
  it("fails fatally when an edit's base is absent or unreadable", async () => {
    ...
    unreadable.delegate.fetchFilesAtCommit = async () => {
