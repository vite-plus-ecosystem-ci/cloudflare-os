...
const noUi = new Proxy(
  ...
  {
    get() {
      throw new Error("resourceUrl must not call the ui capability");
    },
  },
) as never;
...
describe("configurator resource URLs", () => {
  ...
  it("builds the same Worker URL the server-side builder does", () => {
    ...
    expect(parseObservabilityResourceUrl(url as string)).toEqual({
  ...
  it("reports readiness only once the URL it would build is parseable", () => {
    ...
    expect(
      workerConfigurator.isReady!({ values: { accountId: ACCOUNT_ID, workerName: null } }),
    ).toBe(false);
    expect(
      workerConfigurator.isReady!({
