...
describe("the formula parser", () => {
  ...
  it("parses a range as two references", () => {
    expect(parseFormula("SUM(A1:B2)")).toEqual({
      k: "call",
      name: "SUM",
      args: [{ k: "range", a: "A1", b: "B2" }],
    });
    expect(serializeAst(parseFormula("Sheet2!A1:'Other Sheet'!B9"))).toBe(
      "Sheet2!A1:'Other Sheet'!B9",
    );
  });

  it.each(["A1:5", "A1:(", "A1:", "SUM(A1:)"])(
    "rejects %s, whose range has no end reference",
    (src) => {
      let thrown: unknown;
      try {
        parseFormula(src);
  ...
  it("keeps parentheses when writing a formula back", () => {
    ...
    expect(parseFormula("(1+2)*3")).toEqual({
      k: "bin",
      op: "*",
      a: { k: "paren", a: { k: "bin", op: "+", a: { k: "num", v: 1 }, b: { k: "num", v: 2 } } },
      b: { k: "num", v: 3 },
    });
  });
