...
const ICONS = {
  ...
  percent:
    '<line x1="19" y1="5" x2="5" y2="19"/><circle cx="7" cy="7" r="2.2"/><circle cx="17" cy="17" r="2.2"/>',
  decDec:
    '<path d="M4 8l4 4-4 4"/><text x="11" y="16" font-size="11" fill="currentColor" stroke="none">.0</text>',
  incDec:
    '<path d="M12 8l-4 4 4 4"/><text x="1" y="16" font-size="11" fill="currentColor" stroke="none">.00</text>',
  ...
  sortAsc:
    '<path d="M6 4v16"/><path d="M3 8l3-4 3 4"/><line x1="11" y1="6" x2="20" y2="6"/><line x1="11" y1="12" x2="17" y2="12"/><line x1="11" y1="18" x2="14" y2="18"/>',
  sortDesc:
    '<path d="M6 4v16"/><path d="M3 16l3 4 3-4"/><line x1="11" y1="6" x2="14" y2="6"/><line x1="11" y1="12" x2="17" y2="12"/><line x1="11" y1="18" x2="20" y2="18"/>',
  insRow:
    '<rect x="3" y="4" width="18" height="6" rx="1"/><line x1="12" y1="14" x2="12" y2="20"/><line x1="9" y1="17" x2="15" y2="17"/>',
  insCol:
    '<rect x="4" y="3" width="6" height="18" rx="1"/><line x1="17" y1="9" x2="17" y2="15"/><line x1="14" y1="12" x2="20" y2="12"/>',
...
function rcToRef(r: number, c: number): string {
...
function dateToSerial(d: Date): number {
  return (d.getTime() - DATE_EPOCH) / 86400000;
}
...
function displayValue(computed: Value, fmt: CellFmt | null | undefined): Display {
  ...
  if (typeof computed === "number") {
    ...
    switch (nf) {
      case "number":
        text = fmtNumber(computed, d ?? 2, true);
        break;
      case "integer":
        text = fmtNumber(Math.round(computed), 0, true);
        break;
      case "currency":
        text = (computed < 0 ? "-$" : "$") + fmtNumber(Math.abs(computed), d ?? 2, true);
        break;
      case "percent":
        text = fmtNumber(computed * 100, d ?? 2, true) + "%";
        break;
      case "scientific":
        text = computed.toExponential(d ?? 2);
        break;
      case "date":
        text = fmtDate(computed);
        break;
      case "time":
        text = fmtTime(computed);
        break;
      case "datetime":
        text = fmtDate(computed) + " " + fmtTime(computed);
        break;
      case "text":
        text = fmtGeneral(computed);
        break;
      default:
        text = d != null ? fmtNumber(computed, d, false) : fmtGeneral(computed);
    }
...
function makeEngine(model: Model): Engine {
  ...
  function sheetByName(name: string): string | null {
    ...
    for (const id of model.sheetOrder)
      if (model.sheets[id].name.toLowerCase() === name.toLowerCase()) return id;
    return null;
  }
  function splitRef(
    ref: string,
  ...
  function makeMatrix(sheetId: string, r1: number, c1: number, r2: number, c2: number): Matrix {
    return {
      ...
      c2,
      get(i, j) {
        return evalCellRef(sheetId, r1 + i, c1 + j);
      },
      rows: r2 - r1 + 1,
      cols: c2 - c1 + 1,
    };
  ...
  function evalNode(node: Ast, ctx: EvalContext): Value {
    switch (node.k) {
      ...
      case "pct":
        return divSafe(toNum(evalNode(node.a, ctx)), 100);
      case "ref": {
      ...
      case "un": {
        ...
        try {
          ...
          return e as CellError;
        }
      }
      case "bin":
        return evalBin(node, ctx);
      case "call":
        return callFn(node, ctx);
    }
    return ERR.VALUE();
  }
  ...
  function evalBin(node: Extract<Ast, { k: "bin" }>, ctx: EvalContext): Value {
    const op = node.op;
    let a = evalNode(node.a, ctx),
      b = evalNode(node.b, ctx);
  ...
  const H: Helpers = {
    ...
    scalar(node, ctx) {
      let v = evalNode(node, ctx);
      if (isMatrix(v)) v = v.get(0, 0);
      return v;
    },
    matrixOf(node, ctx) {
      const v = evalNode(node, ctx);
      return isMatrix(v) ? v : { matrix: true, rows: 1, cols: 1, get: () => v };
    },
  };
  function collectVals(v: Value, out: Value[]): void {
    if (isMatrix(v)) {
      for (let i = 0; i < v.rows; i++) for (let j = 0; j < v.cols; j++) out.push(v.get(i, j));
    } else out.push(v);
  }
  ...
  return {
    computeRef(sheetId: string, ref: string) {
      const rc = parseRef(ref);
      return rc ? evalCellRef(sheetId, rc.r, rc.c) : ERR.REF();
    },
  };
}
function divSafe(a: number, b: number): number | CellError {
...
const FUNCTIONS: Record<string, FormulaFn> = (() => {
  ...
  F.PRODUCT = (a, c, H) => {
    const n = numArgs(a, c, H);
    return n.length ? n.reduce((x, y) => x * y, 1) : 0;
  };
  F.AVERAGE = (a, c, H) => {
    const n = numArgs(a, c, H);
  ...
  F.MEDIAN = (a, c, H) => {
    const n = numArgs(a, c, H).sort((x, y) => x - y);
  ...
  F.LN = (a, c, H) => {
    const x = H.toNum(s(a, c, H, 0));
    return x <= 0 ? ERR.NUM() : Math.log(x);
  };
  F.LOG10 = (a, c, H) => {
    const x = H.toNum(s(a, c, H, 0));
    return x <= 0 ? ERR.NUM() : Math.log10(x);
  };
  F.LOG = (a, c, H) => {
    const x = H.toNum(s(a, c, H, 0));
    const b = a.length > 1 ? H.toNum(s(a, c, H, 1)) : 10;
    return x <= 0 ? ERR.NUM() : Math.log(x) / Math.log(b);
  };
  F.MOD = (a, c, H) => {
    const y = H.toNum(s(a, c, H, 1));
  ...
  F.TRUNC = (a, c, H) => {
    const x = H.toNum(s(a, c, H, 0));
    const d = a.length > 1 ? H.toNum(s(a, c, H, 1)) : 0;
  ...
  F.MROUND = (a, c, H) => {
    const x = H.toNum(s(a, c, H, 0));
    const m = H.toNum(s(a, c, H, 1));
    return m === 0 ? 0 : Math.round(x / m) * m;
  };
  F.CEILING = (a, c, H) => {
    const x = H.toNum(s(a, c, H, 0));
    const m = a.length > 1 ? H.toNum(s(a, c, H, 1)) : 1;
    return m === 0 ? 0 : Math.ceil(x / m) * m;
  };
  F.FLOOR = (a, c, H) => {
    const x = H.toNum(s(a, c, H, 0));
    const m = a.length > 1 ? H.toNum(s(a, c, H, 1)) : 1;
    return m === 0 ? 0 : Math.floor(x / m) * m;
  };
  F.PI = () => Math.PI;
  F.SQRTPI = (a, c, H) => Math.sqrt(H.toNum(s(a, c, H, 0)) * Math.PI);
  F.RAND = () => Math.random();
  F.RANDBETWEEN = (a, c, H) => {
    const lo = Math.ceil(H.toNum(s(a, c, H, 0)));
    const hi = Math.floor(H.toNum(s(a, c, H, 1)));
    return lo + Math.floor(Math.random() * (hi - lo + 1));
  };
  F.GCD = (a, c, H) => {
    const n = numArgs(a, c, H).map((x) => Math.abs(Math.trunc(x)));
    const g = (x: number, y: number): number => (y ? g(y, x % y) : x);
    return n.reduce((x, y) => g(x, y), 0);
  };
  F.LCM = (a, c, H) => {
    const n = numArgs(a, c, H).map((x) => Math.abs(Math.trunc(x)));
    const g = (x: number, y: number): number => (y ? g(y, x % y) : x);
    return n.reduce((x, y) => (x && y ? (x * y) / g(x, y) : 0), 1);
  };
  F.FACT = (a, c, H) => {
    let x = Math.floor(H.toNum(s(a, c, H, 0)));
  ...
  for (const fn of ["SIN", "COS", "TAN", "ASIN", "ACOS", "ATAN", "SINH", "COSH", "TANH"] as const)
    F[fn] = (a, c, H) => Math[fn.toLowerCase() as Lowercase<typeof fn>](H.toNum(s(a, c, H, 0)));
  ...
  function matchCriteria(val: Value, crit: Value): boolean {
    ...
    if (op === "=" || op === "<>") {
      ...
      if (rhsNum && typeof val === "number") eq = val === rn;
        ...
        const re = wildToRe(rhs);
        eq = re.test(String(val ?? ""));
  ...
  function wildToRe(p: string): RegExp {
    return new RegExp(
      "^" +
        p
          .replace(/[.+^${}()|[\]\\]/g, "\\$&")
  ...
  function matVals(m: Matrix): Value[] {
    const o: Value[] = [];
    for (let i = 0; i < m.rows; i++) for (let j = 0; j < m.cols; j++) o.push(m.get(i, j));
    return o;
  }
  ...
  function ifsMatch(a: Ast[], c: EvalContext, H: Helpers, startIdx: number): boolean[] {
    ...
    for (let i = startIdx; i + 1 < a.length; i += 2)
      pairs.push([matVals(H.matrixOf(a[i], c)), H.scalar(a[i + 1], c)]);
  ...
  F.SUMIFS = (a, c, H) => {
    const sum = matVals(H.matrixOf(a[0], c));
    const mask = ifsMatch(a, c, H, 1);
    let t = 0;
    for (let i = 0; i < mask.length; i++)
      if (mask[i] && typeof sum[i] === "number") t += sum[i] as number;
    return t;
  };
  F.COUNTIFS = (a, c, H) => ifsMatch(a, c, H, 0).filter(Boolean).length;
  F.AVERAGEIFS = (a, c, H) => {
    const avg = matVals(H.matrixOf(a[0], c));
    const mask = ifsMatch(a, c, H, 1);
    let t = 0,
      n = 0;
    for (let i = 0; i < mask.length; i++)
      if (mask[i] && typeof avg[i] === "number") {
        t += avg[i] as number;
        n++;
      }
    return n ? t / n : ERR.DIV0();
  ...
  F.LARGE = (a, c, H) => {
    const n = matVals(H.matrixOf(a[0], c))
      .filter((v) => typeof v === "number")
      .sort((x, y) => y - x);
    const k = H.toNum(s(a, c, H, 1));
    return n[k - 1] ?? ERR.NUM();
  };
  F.SMALL = (a, c, H) => {
    const n = matVals(H.matrixOf(a[0], c))
      .filter((v) => typeof v === "number")
  ...
  F.RANK = (a, c, H) => {
    const x = H.toNum(s(a, c, H, 0));
    const arr = matVals(H.matrixOf(a[1], c)).filter((v) => typeof v === "number");
    const asc = a.length > 2 && H.toBool(s(a, c, H, 2));
    const sorted = arr.slice().sort((p, q) => (asc ? p - q : q - p));
    const i = sorted.indexOf(x);
    return i < 0 ? ERR.NA() : i + 1;
  };
  F.PERCENTILE = (a, c, H) => {
    const arr = matVals(H.matrixOf(a[0], c))
      .filter((v) => typeof v === "number")
      .sort((x, y) => x - y);
    const p = H.toNum(s(a, c, H, 1));
    if (!arr.length) return ERR.NUM();
    const idx = p * (arr.length - 1);
    const lo = Math.floor(idx);
    return arr[lo] + (arr[Math.min(lo + 1, arr.length - 1)] - arr[lo]) * (idx - lo);
  };

  // ---- Logical ----
  F.IF = (a, c, H) => {
    const t = H.toBool(H.scalar(a[0], c));
    if (t) return a.length > 1 ? H.scalar(a[1], c) : true;
    return a.length > 2 ? H.scalar(a[2], c) : false;
  };
  F.IFS = (a, c, H) => {
    for (let i = 0; i + 1 < a.length; i += 2)
      if (H.toBool(H.scalar(a[i], c))) return H.scalar(a[i + 1], c);
    return ERR.NA();
  };
  F.IFERROR = (a, c, H) => {
    const v = H.scalar(a[0], c);
    return isErr(v) ? (a.length > 1 ? H.scalar(a[1], c) : "") : v;
  };
  F.IFNA = (a, c, H) => {
    const v = H.scalar(a[0], c);
    return isErr(v) && v.value === "#N/A" ? H.scalar(a[1], c) : v;
  };
  F.AND = (a, c, H) => {
    for (const v of H.flatVals(a, c)) {
      if (isErr(v)) return v;
      if (v != null && v !== "" && !H.toBool(v)) return false;
    }
    return true;
  };
  F.OR = (a, c, H) => {
    ...
    for (const v of H.flatVals(a, c)) {
      if (isErr(v)) return v;
      if (v != null && v !== "" && H.toBool(v)) any = true;
    }
    return any;
  };
  F.XOR = (a, c, H) => {
    let cnt = 0;
    for (const v of H.flatVals(a, c)) if (H.toBool(v)) cnt++;
    return cnt % 2 === 1;
  };
  ...
  F.SWITCH = (a, c, H) => {
    ...
    for (; i + 1 < a.length; i += 2) {
      if (H.compare(target, H.scalar(a[i], c), "=")) return H.scalar(a[i + 1], c);
    }
    return i < a.length ? H.scalar(a[i], c) : ERR.NA();
  };

  // ---- Text ----
  F.CONCAT = (a, c, H) =>
    H.flatVals(a, c)
      .map((v) => (v == null ? "" : H.toStr(v)))
      .join("");
  F.CONCATENATE = F.CONCAT;
  F.TEXTJOIN = (a, c, H) => {
    const delim = H.toStr(H.scalar(a[0], c));
    const skip = H.toBool(H.scalar(a[1], c));
    const vals = H.flatVals(a.slice(2), c).map((v) => (v == null ? "" : H.toStr(v)));
    return (skip ? vals.filter((v) => v !== "") : vals).join(delim);
  };
  F.LEFT = (a, c, H) => {
    const t = H.toStr(H.scalar(a[0], c));
    const n = a.length > 1 ? H.toNum(H.scalar(a[1], c)) : 1;
  ...
  F.MID = (a, c, H) => {
    const t = H.toStr(H.scalar(a[0], c));
    const start = H.toNum(H.scalar(a[1], c));
    const len = H.toNum(H.scalar(a[2], c));
    return t.slice(Math.max(0, start - 1), Math.max(0, start - 1) + Math.max(0, len));
  };
  ...
  F.PROPER = (a, c, H) =>
    H.toStr(H.scalar(a[0], c))
      .replace(/\b\w/g, (m) => m.toUpperCase())
      .replace(/\B\w/g, (m) => m.toLowerCase());
  F.TRIM = (a, c, H) => H.toStr(H.scalar(a[0], c)).replace(/\s+/g, " ").trim();
  F.CLEAN = (a, c, H) => H.toStr(H.scalar(a[0], c)).replace(/[\x00-\x1F]/g, "");
  F.SUBSTITUTE = (a, c, H) => {
    const t = H.toStr(H.scalar(a[0], c));
    const oldT = H.toStr(H.scalar(a[1], c));
    const newT = H.toStr(H.scalar(a[2], c));
    if (a.length > 3) {
      const inst = H.toNum(H.scalar(a[3], c));
      let k = 0;
      let idx = -1;
      while ((idx = t.indexOf(oldT, idx + 1)) >= 0) {
        if (++k === inst) return t.slice(0, idx) + newT + t.slice(idx + oldT.length);
      }
      return t;
    }
    return oldT === "" ? t : t.split(oldT).join(newT);
  };
  F.REPLACE = (a, c, H) => {
    const t = H.toStr(H.scalar(a[0], c));
    const start = H.toNum(H.scalar(a[1], c));
    const len = H.toNum(H.scalar(a[2], c));
    const newT = H.toStr(H.scalar(a[3], c));
    return t.slice(0, start - 1) + newT + t.slice(start - 1 + len);
  };
  F.FIND = (a, c, H) => {
    const find = H.toStr(H.scalar(a[0], c));
    const within = H.toStr(H.scalar(a[1], c));
    const start = a.length > 2 ? H.toNum(H.scalar(a[2], c)) : 1;
    const i = within.indexOf(find, start - 1);
    return i < 0 ? ERR.VALUE() : i + 1;
  };
  F.SEARCH = (a, c, H) => {
    const find = H.toStr(H.scalar(a[0], c)).toLowerCase();
    const within = H.toStr(H.scalar(a[1], c)).toLowerCase();
    const start = a.length > 2 ? H.toNum(H.scalar(a[2], c)) : 1;
    const i = within.indexOf(find, start - 1);
    return i < 0 ? ERR.VALUE() : i + 1;
  };
  ...
  F.CODE = (a, c, H) => {
    const t = H.toStr(H.scalar(a[0], c));
    return t ? t.charCodeAt(0) : ERR.VALUE();
  };
  F.UNICODE = (a, c, H) => {
    const t = H.toStr(H.scalar(a[0], c));
    return t ? t.codePointAt(0) : ERR.VALUE();
  };
  F.T = (a, c, H) => {
    const v = H.scalar(a[0], c);
  ...
  F.TEXT = (a, c, H) => {
    const v = H.toNum(H.scalar(a[0], c));
    const f = H.toStr(H.scalar(a[1], c));
    return applyTextFormat(v, f);
  };
  ...
  function applyTextFormat(v: number, f: string): string {
    ...
    if (/0|#/.test(f)) {
      const dec = (f.split(".")[1] || "").replace(/[^0#]/g, "").length;
      const grp = /[#0],[#0]/.test(f) || /,/.test(f.split(".")[0]);
  ...
  // ---- Lookup / reference ----
  F.CHOOSE = (a, c, H) => {
    const i = H.toNum(H.scalar(a[0], c));
    return i >= 1 && i < a.length ? H.scalar(a[i], c) : ERR.VALUE();
  };
  F.ROW = (a, c, H, R) => {
    if (!a.length) return c.r + 1;
    const nd = unwrapParens(a[0]);
    if (nd.k === "ref") {
      const p = R.splitRef(nd.ref, c.sheetId);
      return p ? p.r + 1 : ERR.REF();
    }
    if (nd.k === "range") {
      const p = R.splitRef(nd.a, c.sheetId);
      return p ? p.r + 1 : ERR.REF();
    }
    return ERR.REF();
  };
  F.COLUMN = (a, c, H, R) => {
    if (!a.length) return c.c + 1;
    const nd = unwrapParens(a[0]);
    if (nd.k === "ref") {
      const p = R.splitRef(nd.ref, c.sheetId);
      return p ? p.c + 1 : ERR.REF();
    }
    if (nd.k === "range") {
      const p = R.splitRef(nd.a, c.sheetId);
      return p ? p.c + 1 : ERR.REF();
    ...
    if (type === 0) {
      for (let i = 0; i < arr.length; i++) {
        if (typeof target === "string" && /[*?]/.test(target)) {
          if (wildToRe(target).test(String(arr[i] ?? ""))) return i + 1;
  ...
  F.HLOOKUP = (a, c, H) => {
    ...
    for (let j = 0; j < m.cols; j++) {
      const v = m.get(0, j);
      if (!approx) {
        if (H.compare(v, target, "=")) {
          found = j;
          break;
        }
      } else {
        if (H.compare(v, target, "<=")) found = j;
        else break;
      }
    }
  ...
  F.LOOKUP = (a, c, H) => {
    ...
    for (let i = 0; i < vec.length; i++) {
      if (H.compare(vec[i], target, "<=")) found = i;
      else break;
    }
  ...
  F.TODAY = () => {
    const n = new Date();
    return Math.floor(dateToSerial(new Date(Date.UTC(n.getFullYear(), n.getMonth(), n.getDate()))));
  };
  F.NOW = () => {
    const n = new Date();
    return dateToSerial(
  ...
  F.DATE = (a, c, H) => {
    const y = H.toNum(s(a, c, H, 0));
    const m = H.toNum(s(a, c, H, 1));
    const d = H.toNum(s(a, c, H, 2));
    return dateToSerial(new Date(Date.UTC(y, m - 1, d)));
  };
  F.TIME = (a, c, H) => {
    const h = H.toNum(s(a, c, H, 0));
    const m = H.toNum(s(a, c, H, 1));
    const sec = H.toNum(s(a, c, H, 2));
  ...
  F.WEEKDAY = (a, c, H) => {
    const d = serialToDate(H.toNum(s(a, c, H, 0))).getUTCDay();
    const type = a.length > 1 ? H.toNum(s(a, c, H, 1)) : 1;
    if (type === 2) return d === 0 ? 7 : d;
    if (type === 3) return (d + 6) % 7;
    return d + 1;
  };
  F.WEEKNUM = (a, c, H) => {
    const d = serialToDate(H.toNum(s(a, c, H, 0)));
    const start = Date.UTC(d.getUTCFullYear(), 0, 1);
    return Math.floor(((d.getTime() - start) / 86400000 + new Date(start).getUTCDay()) / 7) + 1;
  };
  F.DAYS = (a, c, H) => H.toNum(s(a, c, H, 0)) - H.toNum(s(a, c, H, 1));
  F.EDATE = (a, c, H) => {
    const d = serialToDate(H.toNum(s(a, c, H, 0)));
    const mo = H.toNum(s(a, c, H, 1));
    return dateToSerial(
      new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth() + mo, d.getUTCDate())),
    );
  };
  F.EOMONTH = (a, c, H) => {
    const d = serialToDate(H.toNum(s(a, c, H, 0)));
    const mo = H.toNum(s(a, c, H, 1));
    return dateToSerial(new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth() + mo + 1, 0)));
  };
  F.DATEDIF = (a, c, H) => {
    const s1 = serialToDate(H.toNum(s(a, c, H, 0)));
    const s2 = serialToDate(H.toNum(s(a, c, H, 1)));
    const unit = H.toStr(H.scalar(a[2], c)).toUpperCase();
    const days = (s2.getTime() - s1.getTime()) / 86400000;
    if (unit === "D") return Math.round(days);
    if (unit === "M")
      return (
        (s2.getUTCFullYear() - s1.getUTCFullYear()) * 12 + (s2.getUTCMonth() - s1.getUTCMonth())
      );
    if (unit === "Y") return s2.getUTCFullYear() - s1.getUTCFullYear();
    return ERR.NUM();
  };
...
// Sheet accessors
// ===========================================================================
function curSheet(): SheetMeta {
  return model.sheets[activeSheetId];
}
function curCells(): CellMap {
  return model.cells[activeSheetId] || (model.cells[activeSheetId] = {});
}
function getCell(ref: string): Cell | null {
  return curCells()[ref] || null;
}
function cellRaw(ref: string): string {
  const c = getCell(ref);
  return c ? c.value : "";
}
function colWidth(c: number): number {
  return curSheet().colWidths[c] || DEFAULT_COL_W;
}
function rowHeight(r: number): number {
  return curSheet().rowHeights[r] || DEFAULT_ROW_H;
}
...
const currencyBtn = iconBtn(ICONS.currency, "Format as currency", () =>
  setFmtOnSelection((f) => {
    f.nf = "currency";
  }),
);
const percentBtn = iconBtn(ICONS.percent, "Format as percent", () =>
  setFmtOnSelection((f) => {
    f.nf = "percent";
  }),
);
...
function setColorOnSelection(key: "c" | "bg", color: string): void {
  setFmtOnSelection((f) => {
    f[key] = color;
  });
}
function setAlignOnSelection(align: "l" | "c" | "r"): void {
  setFmtOnSelection((f) => {
    if (align === "l") delete f.a;
    else f.a = align;
  });
}
const textColorBtn = colorBtn(ICONS.textcolor, "Text color", "#1d1d20", (color) =>
  setColorOnSelection("c", color),
);
const fillColorBtn = colorBtn(ICONS.fill, "Fill color", "#fff3a3", (color) =>
  setColorOnSelection("bg", color),
);
...
const gridScroll = el("div", { class: "grid-scroll", tabindex: "0" }, [
  gridTable,
  remoteLayer,
  cellEditor,
]);
...
const saver = new SaveScheduler({
  ...
  isDirty: () =>
    pendingCellOps.size > 0 || pendingStructure !== null || pendingReplacements.size > 0,
  onStatus: (kind, message) => {
    if (kind === "saved" && reloadedAfterFailure) {
      ...
      setTimeout(() => {
        if (!saver.busy && !reloadedAfterFailure) saveStatus.set("saved", "Saved");
      }, 1800);
      return;
    }
...
async function sendPendingOperation(): Promise<SaveOutcome> {
  ...
  for (const [key, op] of sentCellOps)
    if (pendingCellOps.get(key) === op) pendingCellOps.delete(key);
...
function beginBatch(): void {
  historyBatch = { cells: new Map(), sheetId: activeSheetId };
}
...
function commitBatch(): void {
  if (!historyBatch || !historyBatch.cells.size) {
    historyBatch = null;
    return;
  }
...
function applyHistory(entry: HistoryBatch, into: HistoryBatch[]): void {
  ...
  for (const [key, rec] of entry.cells) {
    ...
    if (rec.prev) {
      cells[rec.ref] = { ...rec.prev };
    } else delete cells[rec.ref];
    queueCellOp(
      rec.sheetId,
      rec.ref,
      rec.prev ? rec.prev.value : null,
      rec.prev ? rec.prev.fmt : null,
      now ? now.version : 0,
    );
  }
  into.push(inverse);
...
function undo(): void {
  ...
  applyHistory(undoStack.pop()!, redoStack);
}
function redo(): void {
  if (!redoStack.length) return;
  applyHistory(redoStack.pop()!, undoStack);
}
...
function setCellValue(
  ...
  if ((value == null || value === "") && (!cur || !cur.fmt)) {
    if (cur) {
      delete cells[ref];
      queueCellOp(sheetId, ref, null, null, cur.version);
    }
    return;
  }
...
function forEachSelected(fn: (ref: string, row: number, col: number) => void): void {
  ...
  for (let row = r.r1; row <= r.r2; row++)
    for (let col = r.c1; col <= r.c2; col++) fn(rcToRef(row, col), row, col);
  commitBatch();
  rebuildEngine();
  renderGrid();
}
  ...
  forEachSelected((ref) =>
    setCellFmt(ref, (f) => {
      let d =
        f.d != null
          ? f.d
          : f.nf === "currency" || f.nf === "percent" || f.nf === "number"
            ? 2
            : defaultDecimalsFor(ref);
...
function clearFormatting(): void {
  forEachSelected((ref) =>
    setCellFmt(ref, (f) => {
      for (const k of Object.keys(f) as (keyof CellFmt)[]) delete f[k];
    }),
  );
  refreshToolbarState();
}
...
function autoSum(): void {
  ...
  if (r.r1 === r.r2 && r.c1 === r.c2) {
    ...
    top++;
    if (top <= r.r1 - 1) {
      rangeStr = rcToRef(top, col) + ":" + rcToRef(r.r1 - 1, col);
      target = rcToRef(r.r1, col);
    } else {
      let left = r.c1 - 1;
      while (left >= 0 && isNumericCell(rcToRef(r.r1, left))) left--;
      left++;
      if (left <= r.c1 - 1) {
        rangeStr = rcToRef(r.r1, left) + ":" + rcToRef(r.r1, r.c1 - 1);
        target = rcToRef(r.r1, r.c1);
      }
    }
  } else {
  ...
  if (rangeStr && target) {
    beginBatch();
    setCellValue(target, "=SUM(" + rangeStr + ")");
    commitBatch();
    rebuildEngine();
    renderGrid();
    moveActive(r.r1, r.c1);
    startEdit(target, false);
  }
}
function isNumericCell(ref: string): boolean {
  const v = engine.computeRef(activeSheetId, ref);
...
function walkRefs(node: Ast | undefined, fn: (ref: string) => string | null): void {
  ...
  if (node.k === "ref") {
    const nr = fn(node.ref);
    if (nr != null) node.ref = nr;
    ...
    for (const key of ["a", "b"] as const) if (branches[key]) walkRefs(branches[key], fn);
    if (branches.args) branches.args.forEach((n) => walkRefs(n, fn));
  }
}
function adjustRef(
  ref: string,
  ...
  let { r, c } = rc;
  if (rowDelta) {
    if (r >= rowAt) r += rowDelta;
  }
  if (colDelta) {
    if (c >= colAt) c += colDelta;
  }
...
function rewriteAllFormulas(
  ...
  for (const [ref, cell] of Object.entries(cells)) {
    if (cell.value && cell.value[0] === "=") {
      const nf = shiftRefsInFormula(cell.value, (r) =>
        adjustRef(r, rowAt, rowDelta, colAt, colDelta),
      );
...
function deleteRows(): void {
  ...
  rebuildSheetCells((row, c) =>
    row >= at && row < at + count ? null : { r: row > at ? row - count : row, c },
  );
  sh.rows -= count;
...
function deleteCols(): void {
  ...
  rebuildSheetCells((row, c) =>
    c >= at && c < at + count ? null : { r: row, c: c > at ? c - count : c },
  );
  sh.cols -= count;
...
function commitStructuralChange(): void {
  ...
  undoStack.length = 0;
  redoStack.length = 0;
  updateUndoButtons(); // structural ops aren't locally undoable
}
...
function doRenderGrid(): void {
  ...
  for (let c = 0; c < sh.cols; c++) {
    ...
    if (c >= rng.c1 && c <= rng.c2)
      th.classList.add(rng.r1 === 0 && rng.r2 === sh.rows - 1 ? "full" : "hl");
  ...
  for (let r = 0; r < sh.rows; r++) {
    ...
    if (r >= rng.r1 && r <= rng.r2)
      rh.classList.add(rng.c1 === 0 && rng.c2 === sh.cols - 1 ? "full" : "hl");
...
function renderCell(ref: string, r: number, c: number, rng: Range): HTMLTableCellElement {
  ...
  const computed =
    cell && cell.value !== "" && cell.value != null
      ? engine.computeRef(activeSheetId, ref)
      : cell
        ? ""
        : null;
  ...
  if (fmt) {
    ...
    if (fmt.u || fmt.s)
      td.style.textDecoration = (fmt.u ? "underline " : "") + (fmt.s ? "line-through" : "");
...
function renderPrintWorkbook(): void {
  printWorkbook.replaceChildren(
    ...model.sheetOrder.filter((sheetId) => model.sheets[sheetId]).map(renderPrintSheet),
  );
}
...
function positionActiveOverlays(): void {
  /* active box handled via .active class */
}
...
function setSelection(a: RC, f: RC): void {
  anchor = { ...a };
  focus = { ...f };
  updateSelectionUI();
  presence.schedule();
...
function updateSelectionUI(): void {
  ...
  gridTable.querySelectorAll<HTMLTableCellElement>("th.rowhead").forEach((th) => {
    ...
    if (r >= rng.r1 && r <= rng.r2)
      th.classList.add(rng.c1 === 0 && rng.c2 === sh.cols - 1 ? "full" : "hl");
  });
  // name box + formula bar
  nameBox.value =
    rng.r1 === rng.r2 && rng.c1 === rng.c2
      ? rcToRef(focus.r, focus.c)
...
function cellEl(r: number, c: number): HTMLTableCellElement | null {
  return gridTable.querySelector<HTMLTableCellElement>(`td.cell[data-r="${r}"][data-c="${c}"]`);
}
...
function startEdit(ref: string, replace = false, seed: string | null = null): void {
  ...
  cellEditor.focus();
  if (replace || seed != null) {
    const L = cellEditor.value.length;
    cellEditor.setSelectionRange(L, L);
...
cellEditor.addEventListener("keydown", (e) => {
  if (e.key === "Enter" && !e.shiftKey && !e.altKey) {
    ...
    e.preventDefault();
    const s = cellEditor.selectionStart;
    cellEditor.value =
      cellEditor.value.slice(0, s) + "\n" + cellEditor.value.slice(cellEditor.selectionEnd);
    cellEditor.setSelectionRange(s + 1, s + 1);
    syncEditorSize();
  } else if (e.key === "Tab") {
    e.preventDefault();
...
gridTable.addEventListener("mousedown", (e) => {
  ...
  if (colResize) {
    startColResize(+colResize.dataset.col!, e);
    e.preventDefault();
    return;
  }
  const rowResize = e.target.closest<HTMLElement>(".row-resize");
  if (rowResize) {
    startRowResize(+rowResize.dataset.row!, e);
    e.preventDefault();
    return;
  }
...
gridTable.addEventListener("mousemove", (e) => {
  ...
  const sh = curSheet();
  if (mouseSelecting === "col") {
    focus = { r: sh.rows - 1, c };
    anchor = { r: 0, c: anchor.c };
  } else if (mouseSelecting === "row") {
    focus = { r, c: sh.cols - 1 };
...
function startColResize(col: number, e: MouseEvent): void {
  ...
  const move = (ev: MouseEvent) => {
    ...
    curSheet().colWidths[col] = Math.round(w);
    applyColWidth(col);
  };
  const up = () => {
...
function startRowResize(row: number, e: MouseEvent): void {
  ...
  const move = (ev: MouseEvent) => {
    ...
    curSheet().rowHeights[row] = Math.round(h);
    applyRowHeight(row);
  };
  const up = () => {
    window.removeEventListener("mousemove", move);
...
gridScroll.addEventListener("keydown", (e) => {
  ...
  if (meta) {
    switch (e.key.toLowerCase()) {
      ...
      case "a":
        ...
        {
          const sh = curSheet();
          setSelection({ r: 0, c: 0 }, { r: sh.rows - 1, c: sh.cols - 1 });
          focus = { r: 0, c: 0 };
          updateSelectionUI();
        }
        return;
      case "arrowdown":
        e.preventDefault();
        moveActive(jumpEdge(focus.r, focus.c, 1), focus.c, e.shiftKey);
        return;
      case "arrowup":
        e.preventDefault();
        moveActive(jumpEdge(focus.r, focus.c, -1), focus.c, e.shiftKey);
      ...
      case "arrowright":
        e.preventDefault();
        moveActive(focus.r, jumpEdgeCol(focus.r, focus.c, 1), e.shiftKey);
        return;
      case "arrowleft":
        e.preventDefault();
        moveActive(focus.r, jumpEdgeCol(focus.r, focus.c, -1), e.shiftKey);
  ...
  switch (e.key) {
    case "ArrowUp":
      e.preventDefault();
      moveActive(focus.r - 1, focus.c, e.shiftKey);
      break;
    case "ArrowDown":
      e.preventDefault();
      moveActive(focus.r + 1, focus.c, e.shiftKey);
    ...
    case "ArrowLeft":
      e.preventDefault();
      moveActive(focus.r, focus.c - 1, e.shiftKey);
      break;
    case "ArrowRight":
      e.preventDefault();
      moveActive(focus.r, focus.c + 1, e.shiftKey);
    ...
    case "Tab":
      e.preventDefault();
      moveWithinSelection(e.shiftKey ? -1 : 1, "h");
      break;
    case "Enter":
      e.preventDefault();
      if (r.r1 !== r.r2 || r.c1 !== r.c2) moveWithinSelection(e.shiftKey ? -1 : 1, "v");
      else {
        startEdit(rcToRef(focus.r, focus.c), false);
      }
      break;
    case "F2":
      e.preventDefault();
    ...
    case "Home":
      e.preventDefault();
      moveActive(focus.r, 0, e.shiftKey);
      break;
    case "End":
      e.preventDefault();
      moveActive(focus.r, curSheet().cols - 1, e.shiftKey);
      break;
    case "PageDown":
      e.preventDefault();
      moveActive(focus.r + 20, focus.c, e.shiftKey);
      break;
    case "PageUp":
      e.preventDefault();
      moveActive(focus.r - 20, focus.c, e.shiftKey);
    ...
    default:
      if (e.key.length === 1 && !meta && !e.altKey) {
        e.preventDefault();
        startEdit(rcToRef(focus.r, focus.c), true, e.key);
      }
  }
});
function jumpEdge(r: number, c: number, dr: number): number {
  ...
  let nr = r + dr;
  const has = (rr: number) => {
    const v = cellRaw(rcToRef(rr, c));
    return v !== "" && v != null;
  };
  if (nr < 0 || nr >= sh.rows) return r;
  if (has(r) && has(nr)) {
    while (nr + dr >= 0 && nr + dr < sh.rows && has(nr + dr)) nr += dr;
    return nr;
  }
...
function jumpEdgeCol(r: number, c: number, dc: number): number {
  ...
  let nc = c + dc;
  const has = (cc: number) => {
    const v = cellRaw(rcToRef(r, cc));
    return v !== "" && v != null;
  };
  if (nc < 0 || nc >= sh.cols) return c;
  if (has(c) && has(nc)) {
    while (nc + dc >= 0 && nc + dc < sh.cols && has(nc + dc)) nc += dc;
    return nc;
  }
...
function moveWithinSelection(dir: number, mode: "h" | "v"): void {
  ...
  if (single) {
    if (mode === "h") moveActive(focus.r, focus.c + dir);
    else moveActive(focus.r + dir, focus.c);
    return;
  }
  let { r, c } = focus;
  if (mode === "h") {
  ...
  for (let row = r.r1; row <= r.r2; row++)
    for (let col = r.c1; col <= r.c2; col++) {
      ...
      if (cell) {
        ...
        if (cell.fmt) {
          setCellValue(ref, null);
        } else {
          delete curCells()[ref];
          queueCellOp(activeSheetId, ref, null, null, cell.version);
        }
      }
    }
  commitBatch();
...
function copySelection(): void {
  ...
  for (let row = r.r1; row <= r.r2; row++) {
    ...
    for (let col = r.c1; col <= r.c2; col++) {
      ...
      let out = "";
      if (cell) {
        const v = engine.computeRef(activeSheetId, rcToRef(row, col));
        out =
...
function snapshotRange(r: Range): CopiedCell[][] {
  ...
  for (let row = r.r1; row <= r.r2; row++) {
    const line: CopiedCell[] = [];
    for (let col = r.c1; col <= r.c2; col++) {
      const c = getCell(rcToRef(row, col));
      line.push(c ? { value: c.value, fmt: c.fmt } : null);
    }
    out.push(line);
  }
  return out;
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let j = 0; j < cols.length; j++) {
      ...
      if (useSnapshot && copyFallback!.cells[i] && copyFallback!.cells[i][j] !== undefined) {
        ...
        recordCell(activeSheetId, ref);
        if (src) {
          curCells()[ref] = { value: src.value, fmt: src.fmt, version: getCell(ref)?.version || 0 };
          queueCellOp(activeSheetId, ref, src.value, src.fmt, curCells()[ref].version);
        } else {
          const was = getCell(ref);
          if (was) {
            delete curCells()[ref];
            queueCellOp(activeSheetId, ref, null, null, was.version);
          }
        }
      } else {
  ...
  if (e.key === "Enter") {
    ...
    if (range) {
      const a = parseRef(range[1]),
        b = parseRef(range[2]);
      if (a && b) {
        setSelection({ r: a.r, c: a.c }, { r: b.r, c: b.c });
        focus = { r: b.r, c: b.c };
        updateSelectionUI();
        scrollActiveIntoView();
      }
      ...
      const rc = parseRef(v);
      if (rc) moveActive(rc.r, rc.c);
    }
    gridScroll.focus();
  }
});
formulaInput.addEventListener("focus", () => {
  if (!editing) startEdit(rcToRef(focus.r, focus.c), false);
});
...
titleInput.addEventListener("input", () => {
  model.title = titleInput.value.trim() || "Untitled spreadsheet";
  queueStructure();
});
    ...
    menu.appendChild(
      el(
        "div",
        {
          class: "cmenu-item",
          style:
            "color:var(--faint);font-size:11px;pointer-events:none;text-transform:uppercase;letter-spacing:.05em;",
        },
        g,
      ),
    );
    for (const fn of fns) {
      const it = el("div", { class: "ctx-item" }, [
        el("span", {}, fn),
        el("span", { class: "k" }, "ƒ"),
      ]);
...
document.addEventListener("mousedown", (e) => {
  if (ctxEl && !ctxEl.contains(e.target instanceof Node ? e.target : null)) closeCtx();
});
...
gridTable.addEventListener("contextmenu", (e) => {
  ...
  if (td) {
    const r = +td.dataset.r!,
      c = +td.dataset.c!;
    const rng = selRange();
    if (r < rng.r1 || r > rng.r2 || c < rng.c1 || c > rng.c2) moveActive(r, c);
  }
  const menu = el("div", { class: "ctx" });
  const item = (label: string, k: string, fn: () => void, danger?: boolean) => {
    const it = el("div", { class: "ctx-item" + (danger ? " danger" : "") }, [
      el("span", {}, label),
      k ? el("span", { class: "k" }, k) : null,
    ]);
    it.addEventListener("click", () => {
      closeCtx();
      fn();
...
function renderTabs(): void {
  ...
  for (const id of model.sheetOrder) {
    ...
    const tab = el(
      "div",
      { class: "tab" + (id === activeSheetId ? " active" : ""), "data-id": id },
      [el("span", { class: "tname" }, sh.name)],
    );
...
function sheetTabMenu(id: string, e: MouseEvent): void {
  ...
  if (model.sheetOrder.length > 1) {
    menu.appendChild(el("div", { class: "ctx-sep" }));
    item("Delete", () => deleteSheet(id), true);
  }
  showCtx(menu, e.clientX, e.clientY);
}
...
function renderPresence(): void {
  ...
  for (const person of roster.entries()) {
    ...
    const r1 = Math.min(p.r1, p.r2),
      r2 = Math.max(p.r1, p.r2),
      c1 = Math.min(p.c1, p.c2),
      c2 = Math.max(p.c1, p.c2);
    const tdA = cellEl(r1, c1)!,
      tdB = cellEl(r2, c2)!;
    if (!tdA || !tdB) continue;
    const left = tdA.offsetLeft,
      top = tdA.offsetTop;
    const width = tdB.offsetLeft + tdB.offsetWidth - left,
      height = tdB.offsetTop + tdB.offsetHeight - top;
    ...
    remoteLayer.appendChild(fill);
    remoteLayer.appendChild(box);
    remoteLayer.appendChild(tag);
  }
}
...
function applyRemoteOperation(event: OperationEvent): void {
  ...
  if (event.replacedCells)
    for (const [sid, cells] of Object.entries(event.replacedCells)) model.cells[sid] = cells;
  ...
  setTimeout(() => {
    if (!saver.busy && !pendingCellOps.size) saveStatus.set("saved", "Saved");
  }, 900);
}
function applyStructure(s: Structure): void {
  if (s.title != null && document.activeElement !== titleInput) {
    model.title = s.title;
  ...
  for (const id of Object.keys(model.sheets))
    if (!model.sheetOrder.includes(id)) {
      delete model.sheets[id];
      delete model.cells[id];
    }
}
