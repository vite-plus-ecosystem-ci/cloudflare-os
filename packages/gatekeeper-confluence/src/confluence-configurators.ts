...
import { ConfluenceApi, buildCql, type AccessibleResource } from "./confluence-api";
...
@validateRpc()
export class ConfluenceConfiguratorUI
  extends RpcTarget
  implements
    ...
    ConfluencePageConfiguratorRpc
{
  ...
  async listSites(query: string): Promise<ConfiguratorOption[]> {
    ...
    return (await getSites())
      .filter(
        (site) => !q || site.name.toLowerCase().includes(q) || site.url.toLowerCase().includes(q),
      )
    ...
    const perSite = await Promise.all(
      sites.map(async (site) => {
        ...
        return results
          .filter(
            (space) =>
              !q || space.name.toLowerCase().includes(q) || space.key.toLowerCase().includes(q),
          )
          .map((space) => ({
    ...
    const perSite = await Promise.all(
      sites.map(async (site) => {
        ...
        return results.map((summary) => ({
          ...
          subtitle: [
            summary.type === "blogpost" ? "Blog post" : "Page",
            sites.length > 1 ? site.name : undefined,
          ]
            .filter(Boolean)
            .join(" · "),
        }));
