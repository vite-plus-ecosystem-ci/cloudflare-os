...
export function storageToMarkdown(xhtml: string): string {
  ...
  return renderBlocks(root.childNodes)
    .trim()
    .replace(
      new RegExp(`${CDATA_SENTINEL}(\\d+)${CDATA_SENTINEL}`, "g"),
      (_, n) => cdata[Number(n)] ?? "",
    );
}
...
function renderBlock(el: HTMLElement): string | null {
  ...
  switch (tag) {
    case "p":
      return renderInline(el.childNodes).trim() || null;
    case "ul":
      return renderList(el, false);
    case "ol":
      return renderList(el, true);
    case "blockquote":
      return prefixLines(renderBlocks(el.childNodes), "> ");
    case "hr":
      return "---";
    case "pre":
      return fence("", el.text);
    case "table":
      return renderTable(el);
    case "ac:task-list":
      return renderTaskList(el);
    case "ac:structured-macro":
      return renderMacro(el);
    case "br":
      return null;
    default:
...
function renderMacro(el: HTMLElement): string | null {
  ...
  if (name === "code") {
    return fence(
      paramValue(el, "language") ?? "",
      childByTag(el, "ac:plain-text-body")?.text ?? "",
    );
  }
...
function renderInline(nodes: Node[]): string {
  return nodes
    .map((node) => {
      ...
      if (tag === "a")
        return `[${renderInline(node.childNodes)}](${node.getAttribute("href") ?? ""})`;
      if (tag === "br") return "\n";
...
export function markdownToStorage(markdown: string): string {
  ...
  while (i < lines.length) {
    ...
    if (/^[-*]\s+/.test(line)) {
      const items = collect((l) => /^[-*]\s+/.test(l) && !TASK_ITEM.test(l)).map(
        (l) => `<li>${inlineToStorage(l.replace(/^[-*]\s+/, "").trim())}</li>`,
      );
    ...
    if (/^\d+\.\s+/.test(line)) {
      const items = collect((l) => /^\d+\.\s+/.test(l)).map(
        (l) => `<li>${inlineToStorage(l.replace(/^\d+\.\s+/, "").trim())}</li>`,
      );
...
function inlineToStorage(text: string): string {
  ...
  while (i < text.length) {
    ...
    if (link) {
      out += `<a href="${escapeHtml(link[2])}">${inlineToStorage(link[1])}</a>`;
      i += link[0].length;
      continue;
    }
