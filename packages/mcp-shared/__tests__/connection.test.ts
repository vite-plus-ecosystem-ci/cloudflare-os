...
import { callMayHaveTakenEffect, McpCallNotDispatchedError, McpClient } from "../src/client.js";
...
it("classifies an expired pre-fetch deadline as not dispatched", async () => {
  const client = new McpClient("https://mcp.example.com", async () => "token", null, {
    deadline: 0,
  });
...
it("persists a successful initialization before running the requested operation", async () => {
  ...
  await expect(
    withClient({}, account, "https://mcp.example.com", async () => {
...
it("reports credentials expired when session recovery is rejected", async () => {
  ...
  const error = await withClient({}, account, "https://mcp.example.com", (client) =>
    client.listTools(1),
  ).catch((err) => err);
