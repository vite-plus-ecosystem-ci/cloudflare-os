...
The guarantee is _at most once_, not exactly once. MCP has no idempotency key that would make a
...
its own small write _before_ the result is attached, so nothing about handling a server-controlled
