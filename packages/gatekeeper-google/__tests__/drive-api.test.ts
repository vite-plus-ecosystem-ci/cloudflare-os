...
describe("listDrives", () => {
  it("returns shared-drive picker options and forwards pagination", async () => {
    let calls = stubFetch([
      jsonResponse({
        drives: [{ id: "drive-1", name: "Product" }],
        nextPageToken: "p2",
      }),
    ]);
  ...
  it("collects every shared-drive page", async () => {
    ...
    expect(calls.map((call) => call.url.searchParams.get("q"))).toEqual([
      "name contains 'Pro'",
      "name contains 'Pro'",
    ]);
  });
...
describe("bulk access verification", () => {
  it("maps fresh files.get outcomes back to the requested ID order", async () => {
    ...
    await expect(api().checkFileAccess(["one", "two", "three"])).resolves.toEqual([
      true,
      false,
      false,
    ]);
  ...
  it("concatenates batch outcomes in request order across the 100-file chunk boundary", async () => {
    ...
    await expect(
      api().checkFileAccess(Array.from({ length: 101 }, (_, index) => `file-${index}`)),
  ...
  it("distinguishes an API-disabled inner response", async () => {
    ...
    await expect(api().checkFileAccess(["one"])).rejects.toBeInstanceOf(DriveApiDisabledError);
  });
  ...
  it.each(["dailyLimitExceeded", "rateLimitExceeded", "userRateLimitExceeded"])(
    ...
    async (reason) => {
      ...
      await expect(api().checkFileAccess(["one"])).rejects.toThrow(
        "Google Drive batch subrequest failed: 403",
      );
    },
  );
  ...
  it("fails a transient inner response instead of reporting an access denial", async () => {
    ...
    await expect(api().checkFileAccess(["one"])).rejects.toThrow(
      "Google Drive batch subrequest failed: 429",
    );
  });
  ...
  it("rejects a batch response whose Content-Type carries no boundary", async () => {
    ...
    await expect(api().checkFileAccess(["one"])).rejects.toThrow(
      "Invalid Google Drive batch response boundary",
    );
  });

  it("rejects a truncated batch with fewer parts than files", async () => {
    stubFetch([batchResponse([{ status: 200 }])]);
    await expect(api().checkFileAccess(["one", "two"])).rejects.toThrow(
      "Google Drive batch response did not contain one result per file",
    );
  });
  ...
  it("surfaces an outer non-ok batch POST", async () => {
    ...
    await expect(api().checkFileAccess(["one"])).rejects.toThrow(
      "Google Drive API request failed: 500",
    );
  ...
  it("throws when a batch subrequest still 401s after the forced-refresh replay", async () => {
    ...
    await expect(drive.checkFileAccess(["one"])).rejects.toThrow(
      "Google Drive batch subrequest failed: 401",
    );
    expect(calls).toHaveLength(2);
  });
  ...
  it("rejects a batch part whose Content-ID does not name a requested file", async () => {
    ...
    await expect(api().checkFileAccess(["one"])).rejects.toThrow(
      "Google Drive batch response part had an unrecognised Content-ID",
    );
  });
});
