...
describe("restricted Gmail capability scope", () => {
  it("never exposes a nonmatching sibling from the same thread", () => {
    ...
    expect(
      gmailMessagesAllowedByScope(scope, [
  ...
  it("uses message-level mutations for restricted threads", () => {
    expect(gmailMutationTarget(gmailRestrictedScope(["m1", "m2"]), "thread")).toEqual({
      kind: "messages",
      messageIds: ["m1", "m2"],
    });
  });
  ...
  it("uses a thread endpoint only for whole-mailbox authority", () => {
    expect(gmailMutationTarget(GMAIL_MAILBOX_SCOPE, "thread")).toEqual({
      kind: "thread",
      threadId: "thread",
    });
  });
