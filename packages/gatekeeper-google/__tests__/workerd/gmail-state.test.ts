...
import {
  ...
  GMAIL_FORWARD_SNAPSHOT_CHUNK_BYTES,
  GmailDraftState,
  ...
  MAX_GMAIL_PENDING_FORWARD_SNAPSHOT_BYTES,
  overlayGmailDraft,
  overlayGmailLabels,
...
function snapshotStore(): SnapshotTestStore {
  const stub = testEnv.TestHooks.get(
    testEnv.TestHooks.idFromName(`gmail-state-${crypto.randomUUID()}`),
  );
  function run<T>(
    callback: (store: GmailForwardSnapshotStore, storage: DurableObjectStorage) => T | Promise<T>,
  ) {
  ...
  return {
    ...
    prune: (referenced, createdBefore) =>
      run((store) => {
...
const snapshotBytes = () =>
  Uint8Array.from({ length: GMAIL_FORWARD_SNAPSHOT_CHUNK_BYTES + 17 }, (_, index) => index % 251);
...
describe("Gmail draft overlays", () => {
  ...
  it("applies create, update, and delete overlays in dependency order", () => {
    ...
    expect(
      overlayGmailDraft(created.logicalId, undefined, [
        ...createAndUpdate,
        {
          id: 3,
  ...
  it("hides a draft after a pending delete", () => {
    ...
    expect(
      overlayGmailDraft("d", base, [{ id: 2, action: { type: "draftDelete", draftId: "d" } }]),
    ).toBeNull();
  ...
  it("invalidates a rejected provisional create and retains a mapped logical ID", () => {
    ...
    expect(
      overlayGmailDraft(provisional.logicalId, undefined, [], new Map([[1, "rejected"]])),
    ).toBeNull();
  ...
  it("reports pending and rejected prerequisites distinctly", () => {
    ...
    expect(gmailDependencyError(action, new Set(), new Map([[1, "rejected"]]))).toMatch(
      /was rejected/,
    );
    expect(gmailDependencyError(action, new Set(), new Map())).toMatch(
      /no recorded successful outcome/,
    );
  });

  it("keeps a reply subject immutable", () => {
    expect(() =>
      applyGmailDraftPatch(draft({ source: { kind: "reply", messageId: "m" } }), {
        subject: "Different",
      }),
    ).toThrow(/immutable/);
  });
  ...
  it("uses the same semantic fingerprint before and after a provider MIME round trip", async () => {
    ...
    expect(await gmailDraftFingerprint(parsed, "t")).toBe(await gmailDraftStateFingerprint(state));
  });
  ...
  it("includes the Date header in raw and simulated draft fingerprints", async () => {
    ...
    expect(
      await gmailDraftFingerprint({ ...parsed, date: "Fri, 2 Jan 1970 00:00:00 +0000" }),
    ).not.toBe(await gmailDraftFingerprint(parsed));
    expect(
      await gmailDraftStateFingerprint({
        ...state,
  ...
  it("normalizes display-name formatting but fingerprints visible name changes", async () => {
    ...
    expect(await gmailDraftFingerprint(parsed)).toBe(await gmailDraftStateFingerprint(state));
    expect(
      await gmailDraftFingerprint({ ...parsed, to: ["Other Name <to@example.com>"] }),
    ).not.toBe(await gmailDraftStateFingerprint(state));
  });

  it("does not fingerprint provider draft, message, or unthreaded thread IDs", async () => {
    const first = draft({
      providerId: "draft-one",
      messageId: "message-one",
      threadId: "thread-one",
    });
    const second = draft({
      providerId: "draft-two",
  ...
  it.each(["line one\nline two", "line one\r\nline two", "line one\rline two", "trailing\n"])(
    ...
    async (text) => {
      ...
      expect(await gmailDraftFingerprint(parsed, "t")).toBe(
        await gmailDraftStateFingerprint(state),
      );
    },
  );
...
describe("Gmail forward snapshot storage", () => {
  ...
  it.each([
    ...
    [
      "reordered",
      async (store: SnapshotTestStore, keys: string[]) => {
  ...
  it("fails old pending action snapshots closed with resubmission guidance", async () => {
    const store = snapshotStore();
    await expect(store.read({ size: 1, digest: "0".repeat(64) } as never)).rejects.toThrow(
      /reject and resubmit/i,
    );
  });
});
...
describe("Gmail label state and canonicalization", () => {
  ...
  it("does not trust a caller-provided custom label name", () => {
    expect(
      canonicalizeGmailMutableLabel(
        { id: "Label_1", name: "Forged name", type: "custom" },
        provider,
        [],
      ),
  ...
  it("requires exact system ID/name consistency and the mutable allowlist", () => {
    expect(() =>
      canonicalizeGmailMutableLabel({ id: "INBOX", name: "TRASH", type: "system" }, provider, []),
  ...
  it("accepts only this binding's active provisional labels", () => {
    ...
    expect(
      canonicalizeGmailMutableLabel(
        { id: "provisional-label-1", name: "forged", type: "custom" },
        provider,
        local,
      ),
    ...
    expect(() =>
      canonicalizeGmailMutableLabel(
        { id: "provisional-label-other", name: "Pending", type: "custom" },
        provider,
        local,
      ),
    ).toThrow(/not found/);
  ...
  it("uses the provider's current name for a mapped custom label", () => {
    expect(
      canonicalizeGmailMutableLabel(
        { id: "logical-label", name: "forged", type: "custom" },
        provider,
        [
          {
