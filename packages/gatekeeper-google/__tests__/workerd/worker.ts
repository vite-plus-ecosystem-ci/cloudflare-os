...
async function withMessage<T>(
  ...
  id: string,
  callback: (message: GmailMessage) => Promise<T>,
): Promise<T> {
...
class TestApprovalQueue extends RpcTarget {
  ...
  async authorizeObservation(description: unknown): Promise<void> {
    ...
    if (
      ...
      description.title === this.#pausedTitle
    ) {
  ...
  pauseActionSubmission(): void {
    ...
    this.#pausedSubmission = new Promise((resolve) => {
      this.#markSubmissionPaused = resolve;
    });
    this.#releaseSubmission = new Promise((resolve) => {
      this.#releasePausedSubmission = resolve;
    });
  }

  waitForPausedActionSubmission(): Promise<void> {
    if (!this.#pausedSubmission)
      throw new Error("No test action submission is configured to pause.");
    return this.#pausedSubmission;
  }

  releasePausedActionSubmission(): void {
    if (!this.#releasePausedSubmission)
      throw new Error("The test action submission has not paused yet.");
...
export class TestHooks extends DurableObject<Cloudflare.Env> {
  ...
  #gatekeeper(facetName: string, id: string, props: GmailGatekeeperImplProps) {
    const exports = this.ctx.exports as unknown as {
      GmailGatekeeperImpl(options: {
    ...
    return this.ctx.facets.get<GmailGatekeeperImpl>(facetName, () => ({
      id,
      class: exports.GmailGatekeeperImpl({ props }),
    }));
  }

  async initialize(facetName: string, id: string, props: GmailGatekeeperImplProps): Promise<void> {
  ...
  async describe(facetName: string, id: string, props: GmailGatekeeperImplProps) {
  ...
  async applyStorage(
    ...
    (this.#gatekeeper(facetName, id, props) as unknown as TestGmail).applyTestStorage(operations);
  }
  ...
  async captureForwardSnapshot(
    ...
    return (this.#gatekeeper(facetName, id, props) as unknown as TestGmail).captureTestSnapshot(
      bytes,
    );
  }
}
    ...
    switch (operation) {
      ...
      case "session.send":
        return await session.send(
          ...
          value as string,
          extra as string,
          options as GmailComposeOptions,
        );
      ...
      case "message.forward":
        return await withMessage(session, id as string, (message) =>
          message.forward(value as string[], extra as string, options as GmailComposeOptions),
        );
      ...
      case "message.createReplyAllDraft": {
        return await withMessage(session, id as string, async (message) => {
          const draft = await message.createReplyAllDraft(
            value as string,
            extra as GmailReplyOptions,
          );
          try {
            return await draft.getMetadata();
          } finally {
      ...
      case "message.applyLabel":
        return await withMessage(session, id as string, (message) =>
          message.applyLabel(value as never),
        );
      case "message.removeLabel":
        return await withMessage(session, id as string, (message) =>
          message.removeLabel(value as never),
        );
