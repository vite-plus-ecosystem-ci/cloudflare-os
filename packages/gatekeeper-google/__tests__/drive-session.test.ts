...
function core(
  ...
  let getDrive = vi.fn(
    overrides.getDrive ?? (async (id: string) => ({ id, name: "Current shared drive" })),
  );
  let prepared: string[][] = [];
...
describe("Drive session scope", () => {
  ...
  it("preserves a shared-drive provider outage", async () => {
    let { session } = core({
      scope: { kind: "sharedDrive", driveId: "drive-1" },
      getFile: async () => {
        throw new DriveApiRequestError(500);
      },
    });

    await expect(session.getEntry("file-1")).rejects.toThrow(
      "Google Drive API request failed: 500",
    );
  });

  it.each(["dailyLimitExceeded", "rateLimitExceeded", "userRateLimitExceeded"])(
    "preserves a shared-drive quota failure reported as %s",
    async (reason) => {
...
describe("Drive parent folder probe", () => {
  it("rejects a parent from another shared drive before listing", async () => {
    ...
    await expect(session.list({ directParentId: "folder-x" })).rejects.toThrow(
  ...
  it("observes a readable non-folder parent before disclosing its type", async () => {
    ...
    await expect(session.list({ directParentId: "file-x" })).rejects.toThrow(
      /must identify a folder/,
    );
  ...
  it("rejects search when the parent is outside the shared drive", async () => {
    ...
    await expect(session.search({ directParentId: "folder-x" })).rejects.toThrow(
      /outside this Drive binding/,
    );
    expect(listFiles).not.toHaveBeenCalled();
    expect(prepared).toEqual([]);
...
describe("Drive native sessions", () => {
  ...
  it("locks out a collaborator admitted after a failed probe", async () => {
    ...
    await expect(track.addObserver("late", "verifier")).rejects.toThrow(
      /cannot access Drive file file-1/,
    );
  ...
  it("rejects a foreign shared-drive file without authorizing or tracking it", async () => {
    ...
    await expect(session.openNativeFile("foreign", docMime, "Google Doc")).rejects.toThrow(
      /outside this Drive binding/,
    );
    expect(prepared).toEqual([]);
    expect(authorizations).toEqual([]);
  ...
  it.each([403, 404])(
    ...
    async (status) => {
      ...
      await expect(session.openNativeFile("foreign", docMime, "Google Doc")).rejects.toThrow(
        new Error("The requested file is outside this Drive binding."),
      );
      expect(prepared).toEqual([]);
      expect(authorizations).toEqual([]);
    },
