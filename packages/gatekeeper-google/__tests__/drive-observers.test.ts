...
describe("driveObserverTracker", () => {
  ...
  it("refuses a joiner holding no Drive grant at all", async () => {
    let { track } = tracker({ kind: "file", fileId: "file-1" }, (ids) => ({
      baselineAllowed: false,
      allowed: ids.map(() => false),
    }));
  ...
  it("keeps the old Drive verifier after failed same-ID re-verification", async () => {
    let { track } = tracker({ kind: "file", fileId: "file-1" }, (ids, verifier) =>
      verifier === "old" ? allow(ids) : deny(ids),
    );
    await track.addObserver("obs", "old");

    await expect(track.addObserver("obs", "new")).rejects.toThrow(
      /cannot access Drive file file-1/,
    );
