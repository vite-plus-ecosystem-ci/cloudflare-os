...
function scopePhrase(scope: DriveBindingScope): string {
  switch (scope.kind) {
    case "account":
      return "the connected Drive account";
    case "sharedDrive":
      return `shared drive ${scope.driveId}`;
    case "file":
      return `file ${scope.fileId}`;
  }
}
...
function queryClauses(query: DriveListFilesOptions): string[] {
  ...
  if (query.mimeTypes?.length) {
    parts.push(
      `mime types ${query.mimeTypes.map((value) => clip(value, MAX_OBSERVATION_VALUE)).join(", ")}`,
    );
  }
...
export class DriveSessionCore {
  ...
  #cursor(query: DriveListFilesOptions, denyEmptySearch = false): Pager<DriveEntry> {
    ...
    return new CursorPager<DriveFile, DriveEntry>({
      ...
      buildEntries: async (files) =>
        files.filter((file) => this.#inScope(file)).map(driveFileToEntry),
  ...
  #inScope(file: DriveFile): boolean {
    switch (this.#scope.kind) {
      ...
      case "file":
        return file.id === this.#scope.fileId;
    }
  }
  ...
  async #assertParent(parentId: string): Promise<void> {
    ...
    if (parent.mimeType !== FOLDER_MIME_TYPE)
      throw new Error("directParentId must identify a folder");
  }
  ...
  async #getFileInScope(fileId: string): Promise<DriveFile> {
    ...
    try {
      ...
      if (
        err instanceof DriveApiRequestError &&
        !err.isQuotaExceeded &&
        ...
        if (this.#scope.kind === "account") {
          ...
          await this.#authorizeIds(
            [fileId],
            "Check Google Drive file access",
