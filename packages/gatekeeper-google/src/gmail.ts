...
import {
  ...
  enumerateGmailAttachments,
  emailRecipientToAddress,
  getGoogleAccountDescription,
  getGoogleAccountSubject,
  GmailApi,
  GmailApiError,
  GmailDraftFull,
  GmailDraftRaw,
...
import {
  ...
  validateGmailRecipientMaximum,
  validateGmailSubject,
  validateOutboundFields,
} from "./gmail-validate";
import {
  GMAIL_MAILBOX_SCOPE,
  GmailCapabilityScope,
  gmailMessagesAllowedByScope,
  gmailMutationTarget,
  gmailRestrictedScope,
  gmailScopeAllowsMessage,
  groupGmailMessagesByThread,
} from "./gmail-scope";
import {
  applyGmailDraftPatch,
  canonicalizeGmailMutableLabel,
  CanonicalMutableLabel,
  GmailDecision,
  GmailDraftAttachmentState,
  GmailDraftOverlayAction,
  GmailDraftResource,
  GmailDraftSource,
  GmailDraftState,
  GmailForwardSnapshotReference,
  GmailForwardSnapshotStore,
  GmailLabelOverlayAction,
  GmailLabelResource,
  gmailDependencyError,
  gmailDraftFingerprint,
...
type GmailAction =
  | GmailMessageMutationAction
  | GmailSendAction
  | GmailDraftCreateAction
  | GmailDraftUpdateAction
  | GmailDraftDeleteAction
  | GmailDraftSendAction
  | GmailLabelCreateAction
  | GmailLabelRenameAction
  | GmailLabelDeleteAction;
...
type GmailResourceActionType = Exclude<
  GmailAction["type"],
  "messageMutation" | "send" | "draftSend"
>;
...
class GmailStore {
  ...
  constructor(storage: DurableObjectStorage) {
    ...
    this.#forwardSnapshots.pruneUnreferenced(
      referencedHandles,
      Date.now() - GMAIL_FORWARD_SNAPSHOT_ORPHAN_GRACE_MS,
    );
  }
  ...
  #draftAliasKey(id: string) {
    return `gmail:draftAlias:${id}`;
  }
  #labelKey(id: string) {
    return `gmail:label:${id}`;
  }
  #decisionKey(id: number) {
    return `gmail:decision:${id}`;
  }
  #applyingKey(id: number) {
    return `gmail:applying:${id}`;
  }
  #draftWriteReceiptKey(id: number) {
    return `gmail:draftWriteReceipt:${id}`;
  }
  #sendFingerprintKey(id: number) {
    return `gmail:sendFingerprint:${id}`;
  }
  #admittedLabelKey(id: string) {
    return `gmail:admittedLabel:${id}`;
  }
  #sentAliasKey(rfcMessageId: string) {
    return `gmail:sentAlias:${gmailMessageIdQueryValue(rfcMessageId)}`;
  }
  #sentProviderKey(providerId: string) {
    return `gmail:sentProvider:${providerId}`;
  }
  ...
  markApplying(id: number): void {
    this.#kv.put(this.#applyingKey(id), Date.now());
  }
  isApplying(id: number): boolean {
    return this.#kv.get<number>(this.#applyingKey(id)) !== undefined;
  }
  clearApplying(id: number): void {
    this.#kv.delete(this.#applyingKey(id));
  }
  ...
  setSendFingerprint(id: number, rfcMessageId: string, fingerprint: string): void {
    this.#storage.transactionSync(() => {
      ...
      if (
        !action ||
        expectedMessageId !== rfcMessageId ||
        (current && (current.rfcMessageId !== rfcMessageId || current.fingerprint !== fingerprint))
      ) {
  ...
  #recordSentMessage(
    ...
    for (const existing of [byAlias, byProvider]) {
      if (
        existing &&
        (existing.rfcMessageId !== receipt.rfcMessageId ||
          existing.providerId !== receipt.providerId ||
          existing.threadId !== receipt.threadId)
      ) {
  ...
  hasApplyingDraftSend(logicalId: string): boolean {
    return this.listActions().some(
      ({ id, action }) =>
        action.type === "draftSend" &&
        this.isApplying(id) &&
        this.resolveDraftId(action.draftId) === logicalId,
    );
  }
  ...
  completeMissingDraftUpdate(actionId: number): StoredGmailAction {
    return this.#storage.transactionSync(() => {
      ...
      if (
        !resource ||
        (resource.status !== "active" && resource.status !== "deleted") ||
  ...
  decisions(): Map<number, GmailDecision> {
    return new Map(
      [...this.#kv.list<GmailDecision>({ prefix: "gmail:decision:" })].map(([key, decision]) => [
        Number(key.slice("gmail:decision:".length)),
        decision,
      ]),
    );
  }
  ...
  listDrafts(): GmailDraftResource[] {
    return [...this.#kv.list<GmailDraftResource>({ prefix: "gmail:draft:" })].map(
      ([, value]) => value,
    );
  }
  ...
  mapDraftToProvider(
    ...
    afterMap?: (resource: GmailDraftResource) => void,
  ): GmailDraftResource {
    return this.#storage.transactionSync(() => {
      ...
      if (expectedApplyingCreateId !== undefined) {
        ...
        if (
          create?.type !== "draftCreate" ||
          !this.isApplying(expectedApplyingCreateId) ||
          this.resolveDraftId(create.draft.logicalId) !== resource.logicalId
        ) {
          throw new Error(
            "The uncertain Gmail draft create changed while it was being reconciled.",
          );
        }
      }
      const duplicate = this.listDrafts().find(
        (candidate) =>
          candidate.providerId === providerId && candidate.logicalId !== resource.logicalId,
      );
      if (duplicate) {
        if (
          this.listActions().some(
            ({ action }) =>
              action.type === "draftCreate" &&
              this.resolveDraftId(action.draft.logicalId) === duplicate.logicalId,
          )
        ) {
          throw new Error(
            "Gmail returned a provider draft ID already owned by another pending create.",
          );
        }
      ...
      const mergedActions = this.listActions().filter(
        ({ action }) =>
          isDraftAction(action) && mergedIds.has(this.resolveDraftId(actionResourceId(action)!)),
      );
      ...
      for (const { id, action } of mergedActions) {
        ...
        retargeted = {
          ...retargeted,
          dependsOn: [...new Set([...(retargeted.dependsOn ?? []), ...predecessors])].filter(
  ...
  completeDraftWrite(
    actionId: number,
    baseline: GmailDraftProviderBaseline,
    approvedOutputFingerprint: string,
    expectedReceipt: GmailDraftWriteReceipt = baseline,
  ): StoredGmailAction {
    const action = this.getAction(actionId);
    ...
    if (action.type === "draftCreate") {
      this.mapDraftToProvider(action.draft.logicalId, baseline.draftId, actionId, (resource) => {
      ...
      completed = this.#storage.transactionSync(() =>
        this.#completeDraftWrite(
          actionId,
          this.resolveDraftId(action.draftId),
          baseline,
          approvedOutputFingerprint,
          expectedReceipt,
        ),
  ...
  #completeDraftWrite(
    ...
    baseline: GmailDraftProviderBaseline,
    approvedOutputFingerprint: string,
    expectedReceipt?: GmailDraftWriteReceipt,
  ): StoredGmailAction {
    const action = this.getAction(actionId);
    if (action?.type !== "draftCreate" && action?.type !== "draftUpdate") {
      throw new Error("The pending Gmail draft write changed before completion.");
    ...
    const successor = pending.find(
      ({ id, action: candidate }) =>
        id > actionId &&
        isDraftAction(candidate) &&
        this.resolveDraftId(actionResourceId(candidate)!) === logicalId &&
        !(candidate.dependsOn ?? []).some(
          (dependency) =>
            dependency !== actionId &&
            !pendingIds.has(dependency) &&
            decisions.get(dependency) !== "applied",
        ),
    );
    if (successor && isDraftAction(successor.action)) {
      ...
      if (expected !== approvedOutputFingerprint) {
        throw new Error(
          "The dependent Gmail draft action no longer matches the approved write output.",
        );
      }
  ...
  deleteForwardSnapshot(
    snapshot: GmailForwardSnapshotReference | undefined,
    exceptActionId?: number,
  ): void {
    if (
      snapshot &&
      this.listActions().some(
        ({ id, action }) =>
          id !== exceptActionId && actionSourceAttachment(action)?.handle === snapshot.handle,
      )
    )
      return;
    if (
      snapshot &&
      this.listDrafts().some((resource) => resource.forwardSnapshot?.handle === snapshot.handle)
    )
      return;
  ...
  restoreDraftVersion(
    ...
    this.#storage.transactionSync(() => {
      ...
      if (
        dependencies.some(
          (dependency) => this.#kv.get<GmailDecision>(this.#decisionKey(dependency)) === "applied",
        )
      )
        return;
  ...
  deleteLabel(id: string): void {
    this.#kv.delete(this.#labelKey(id));
  }

  listLabels(): GmailLabelResource[] {
    return [...this.#kv.list<GmailLabelResource>({ prefix: "gmail:label:" })].map(
      ([, value]) => value,
    );
  }
  ...
  mapLabelToProvider(actionId: number, provider: GmailLabelRaw): GmailLabelResource {
    return this.#storage.transactionSync(() => {
      const action = this.getAction(actionId);
      if (
        action?.type !== "labelCreate" ||
        !this.isApplying(actionId) ||
        action.label.name !== provider.name ||
        provider.type !== "user"
      ) {
        throw new Error(
          "The uncertain Gmail label creation changed while it was being reconciled.",
        );
      }
  ...
  admitLabel(id: string): void {
    this.#kv.put(this.#admittedLabelKey(id), true);
  }
...
function isLabelAction(
  action: StoredGmailAction,
): action is GmailLabelCreateAction | GmailLabelRenameAction | GmailLabelDeleteAction {
...
function actionSourceAttachment(action: StoredGmailAction): GmailSourceAttachment | undefined {
  ...
  if (action.type === "draftUpdate" || action.type === "draftDelete" || action.type === "draftSend")
    return action.sourceAttachment;
  if (action.type === "send" && "spec" in action) return action.sourceAttachment;
  return undefined;
}

function requireCurrentSourceSnapshot(
  snapshot: GmailSourceAttachment | undefined,
  sourceMessageId: string | undefined,
): GmailSourceAttachment {
  if (
    !snapshot ||
    typeof (snapshot as Partial<GmailSourceAttachment>).handle !== "string" ||
    !sourceMessageId ||
    snapshot.messageId !== sourceMessageId
  ) {
    throw new Error(
...
function draftSourceSnapshot(action: GmailDraftCreateAction): GmailSourceAttachment | undefined {
  if (action.draft.source?.kind === "forward") {
    const snapshot = requireCurrentSourceSnapshot(
      action.sourceAttachment,
      action.draft.source.messageId,
    );
    if (action.draft.source.format === "inline") return snapshot;
    const attachment = action.draft.attachments[0];
    if (
      action.draft.attachments.length !== 1 ||
      !attachment ||
      attachment.contentDigest !== snapshot.digest ||
      attachment.info.size !== snapshot.size ||
      attachment.info.filename !== "forwarded-message.eml" ||
      attachment.info.mimeType !== "message/rfc822" ||
      attachment.info.disposition !== "attachment" ||
      attachment.info.contentId !== undefined
    ) {
      throw new Error(
...
function dependenciesFor(store: GmailStore, resourceId: string, kind: "draft" | "label"): number[] {
  ...
  const actions = pending.filter(({ action }) => {
    if (kind === "draft") {
      return (
        isDraftAction(action) && store.resolveDraftId(actionResourceId(action)!) === canonicalId
      );
    }
  ...
  const terminal = actions.find(
    (item) =>
      item.action.type === "draftDelete" ||
      item.action.type === "draftSend" ||
  ...
  const invalid = actions.find(
    (item) =>
      ...
      item.action.dependsOn?.some(
        (dependency) => !pendingIds.has(dependency) && decisions.get(dependency) !== "applied",
      ),
  );
  if (invalid) {
    throw new Error(
...
function labelPending(store: GmailStore): PendingOverlayAction<GmailLabelOverlayAction>[] {
  return store
    .listActions()
    .filter((item) =>
      isLabelAction(item.action),
    ) as PendingOverlayAction<GmailLabelOverlayAction>[];
}
...
function validateApprovalDescription(value: string): string {
  if (
    value.length > MAX_GMAIL_APPROVAL_DESCRIPTION_BYTES ||
...
async function parseSafeGmailDraft(message: GmailMessageRaw): Promise<GmailParsedDraft> {
  ...
  const parsedIndexes = parsed.attachments.flatMap((attachment, index) =>
    attachment.contentType.split(";", 1)[0].trim().toLowerCase() === "message/rfc822"
      ? [index]
      : [],
  );
  ...
  for (const exact of exactAttachments) {
    const candidates = [...unmatched].filter((index) => {
      ...
      return (
        (attachment.filename || undefined) === exact.filename &&
...
): Promise<GmailDraftProviderBaseline> {
...
async function captureSourceAttachment(
  store: GmailStore,
  source: GmailMessageRaw,
  message: GmailOutboundMessage,
): Promise<GmailSourceAttachment> {
...
function materializeSourceAttachment(
  bytes: Uint8Array,
  snapshot: GmailSourceAttachment,
): GmailOutboundAttachment {
  if (bytes.byteLength !== snapshot.size || bytes.byteLength > MAX_GMAIL_FORWARD_SOURCE_BYTES) {
    throw new Error(
      "The stored forward source no longer matches its approved attachment snapshot.",
    );
  }
  return {
...
function validateDraftRecipientPatch(
  draft: GmailDraftState,
  patch: Partial<GmailNormalizedRecipients>,
): void {
  ...
  for (const field of fields) {
    ...
    if (
      effective[field].some((recipient) =>
...
async function parsedDraftToState(
  ...
  draft: GmailDraftRaw,
  parsed: GmailParsedDraft,
  fallbackFrom: string,
): Promise<GmailDraftState> {
  ...
  const inlineForward =
    resource.source?.kind === "forward" &&
    resource.source.format === "inline" &&
  ...
  return {
    ...
    ...(inlineForward && resource.forwardHtml !== undefined
      ...
      : !inlineForward && parsed.html !== undefined
        ? { html: parsed.html }
        : {}),
...
function parsedDraftSnapshotToState(
  logicalId: string,
  resource: GmailDraftResource,
  draft: GmailDraftFull,
  parsed: GmailParsedDraftSnapshot,
  fallbackFrom: string,
): GmailDraftState {
  const inlineForward =
    resource.source?.kind === "forward" &&
...
function pendingForwardSnapshot(
  store: GmailStore,
  logicalId: string,
): GmailSourceAttachment | undefined {
  ...
  const actions = store
    ...
    .filter(
      ({ action }) =>
        isDraftAction(action) && store.resolveDraftId(actionResourceId(action)!) === logicalId,
    )
    .toReversed();
...
async function inlineForwardMessage(
  ...
  return api.buildForwardFromBytes(
    bytes,
    state.to,
    state.text,
    {
...
async function draftOutputFingerprint(
  ...
  if (state.source?.kind === "forward" && state.source.format === "inline" && snapshot) {
    ...
    const parsed = await parseGmailDraft({
      id: "forward-snapshot",
      threadId: state.threadId ?? "forward-snapshot",
      internalDate: "0",
      raw: message.raw,
    });
...
function publicLabels(
  ids: string[],
  labels: GmailLabelRaw[],
  resources: GmailLabelResource[],
): GmailLabel[] {
...
async function mapMatchingApplyingDraftCreate(
  api: GmailApi,
  store: GmailStore,
  provider: GmailDraftRaw,
  parsed?: GmailParsedDraft,
): Promise<GmailDraftResource | undefined> {
  ...
  if (resource.providerId && resource.providerId !== provider.id) {
    throw new Error(
      "The uncertain Gmail draft create is already mapped to another provider draft.",
    );
  }
  ...
  if (
    (await gmailDraftFingerprint(current, provider.message.threadId)) !== approvedOutputFingerprint
  ) {
...
async function ensureProviderDraft(
  ...
  if (applyingDraftCreates(ctx.store).length > 0) {
    ...
    if (draft.message?.id) {
      ...
      rfcMessageId = metadata.payload?.headers
...
async function loadDraftBase(
  ...
  return parsedDraftSnapshotToState(
    ...
    resource,
    provider,
    await ctx.api.parseDraftSnapshot(provider.message),
    ctx.selfEmail,
  );
}
...
async function walkRestrictedMessages(
  ctx: GmailContext,
  visit: (messages: readonly GmailMessageRef[]) => boolean,
  allowPartial = false,
): Promise<void> {
  if (!ctx.restricted) throw new Error("Expected a restricted Gmail binding.");
  const labelIds = ctx.labelId ? [ctx.labelId] : undefined;
  if (!ctx.searchQuery && !labelIds)
    throw new Error("Restricted Gmail binding has no restriction.");
  const seenTokens = new Set<string>();
  let pages = 0;
  let pageToken: string | undefined;
  do {
    const result = await ctx.api.listMessages(
      GMAIL_RESTRICTED_THREAD_PROVIDER_PAGE_SIZE,
      ctx.searchQuery,
      pageToken,
      labelIds,
    );
    ...
    if (pageToken) {
      ...
      if (pages === MAX_GMAIL_RESTRICTED_THREAD_PROVIDER_PAGES) {
        ...
        throw new Error(
          "The Gmail restriction contains too many messages to verify this capability.",
        );
      }
    }
  } while (pageToken);
...
async function messagesAvailableThroughRestriction(
  ctx: GmailContext,
  soughtMessageIds: ReadonlySet<string>,
): Promise<Set<string>> {
...
async function restrictedThreadScope(
  ...
  knownRestrictedMessageIds?: readonly string[],
): Promise<GmailCapabilityScope> {
  const restrictedMessageIds =
    knownRestrictedMessageIds ?? (await restrictedThreadMessageIds(ctx, threadId));
  ...
  for (const messageId of sentMessageIds) {
    if (
      ...
      ctx.store.isSentMessage(messageId) &&
  ...
  for (let i = 0; i < admitted.length; i += 5) {
    await Promise.all(
      admitted.slice(i, i + 5).map(async (messageId) => {
...
function disposeEntryTargets<Entry>(
  ...
  target: (entry: Entry) => GmailRpcTarget,
): void {
...
function gmailMessageCursor(
  ...
  capabilityScope: GmailCapabilityScope,
): Cursor<GmailMessageEntry> {
  return new RpcCursor(
    new CursorPager<
      {
        id: string;
        threadId: string;
    ...
    >({
        ...
        try {
          ...
          for (let i = 0; i < messages.length; i += 5) {
            const enriched = await Promise.all(
              messages.slice(i, i + 5).map(async (ref) => {
      ...
      disposeEntries: (entries) => disposeEntryTargets(entries, (entry) => entry.message),
    }),
    ctx.approvalQueue,
  );
}
...
function gmailFullThreadCursor(
  ...
  return new RpcCursor(
    new CursorPager<{ id: string; snippet?: string }, OwnedGmailThreadEntry>({
      provider: "Gmail",
      async fetchPage(pageToken) {
      ...
      async buildEntries(threads) {
        ...
        try {
          ...
          for (let i = 0; i < threads.length; i += 5) {
            const enriched = await Promise.all(
              threads.slice(i, i + 5).map(async (thread) => {
                const metadata = await threadInfo(
                  ctx,
                  await ctx.api.getThreadInfo(thread.id),
                  labels,
                );
                return {
                  thread,
      ...
      disposeEntries: (entries) => disposeEntryTargets(entries, (entry) => entry.thread),
    }),
    ctx.approvalQueue,
  );
}
...
async function collectRestrictedThreadGroups(
  ...
  query: string | undefined,
  labelIds: string[] | undefined,
): Promise<Array<{ threadId: string; messages: GmailMessageRef[] }>> {
  ...
  do {
    ...
    if (
      total > MAX_GMAIL_RESTRICTED_THREAD_MESSAGES ||
        ...
        try {
          ...
          for (const group of groups) {
            const metadata: Array<{
              ref: (typeof group.messages)[number];
              info: GmailMessageInfoRaw;
            }> = [];
            for (let i = 0; i < group.messages.length; i += 5) {
              metadata.push(
                ...(await Promise.all(
                  group.messages.slice(i, i + 5).map(async (ref) => ({
                    ref,
...
class GmailSessionImpl extends GmailRpcTarget implements GmailSession {
  ...
  async listThreads(): Promise<Cursor<GmailThreadEntry>> {
    await this.#authorizeCursor(
      "List Gmail threads",
      "Create a cursor constrained to this Gmail binding.",
    );
  ...
  async listMessages(): Promise<Cursor<GmailMessageEntry>> {
    ...
    return gmailMessageCursor(
      this.#ctx,
      effectiveListQuery(this.#ctx),
      listLabelIds(this.#ctx, true),
  ...
  async listDrafts(): Promise<Cursor<GmailDraftEntry>> {
    await this.#authorizeCursor(
      "List Gmail drafts",
      "Create a cursor for drafts available through this Gmail binding.",
    );
  ...
  async listLabels(): Promise<GmailLabel[]> {
    ...
    return labels.labels
      .filter((label) => label.type === "user" || SYSTEM_LABELS.has(label.id))
      .map((label) =>
        label.type === "system"
  ...
  async createLabel(name: string): Promise<GmailCustomLabel> {
    ...
    await submitAction(
      this.#ctx,
      { type: "labelCreate", label: resource },
      {
  ...
  async renameLabel(label: GmailCustomLabel, name: string): Promise<GmailCustomLabel> {
    ...
    await submitAction(
      ...
      },
      {
        ...
        description:
          `${formatApprovalField("Current name", canonical.name)}\n\n` +
  ...
  async deleteLabel(label: GmailCustomLabel): Promise<void> {
    ...
    const resource = ensureLabelResource(this.#ctx, canonical);
    const pendingUses = this.#ctx.store
      .listActions()
      .filter(
        (item) =>
...
class GmailScopedSessionImpl extends GmailRpcTarget implements GmailScopedSession {
  ...
  getMailboxAddress(): Promise<EmailAddress> {
    return this.#mailbox.getMailboxAddress();
  }
  ...
  search(query: string): Promise<Cursor<GmailThreadEntry>> {
    return this.#mailbox.search(query);
  }
  ...
  listDrafts(): Promise<Cursor<GmailDraftEntry>> {
    return this.#mailbox.listDrafts();
  }
  getDraft(id: string): Promise<GmailDraft> {
    return this.#mailbox.getDraft(id);
  }
...
async function resolveMutableLabel(
  ...
  return current ? { id: canonical.id, name: current.name, type: "custom" } : canonical;
}
...
function labelActionId(
  ctx: GmailContext,
  label: CanonicalMutableLabel,
): {
  id: string;
  dependencies: number[];
} {
...
function mutationAliasMethod(
  operation: GmailMutationOperation,
  label: CanonicalMutableLabel | undefined,
): string | undefined {
...
class GmailThreadStub extends GmailRpcTarget implements GmailThread {
  ...
  async #loadInfo(): Promise<GmailThreadInfo> {
    ...
    return threadInfo(
      this.#ctx,
      summarizeGmailThread(this.#threadId, metadata[0]?.snippet, parsed),
    );
  }
  ...
  async archive(): Promise<void> {
    await this.#mutate("archive");
  }
  async trash(): Promise<void> {
    await this.#mutate("trash");
  }
  async markRead(): Promise<void> {
    await this.#mutate("markRead");
  }
  async markUnread(): Promise<void> {
    await this.#mutate("markUnread");
  }
  async star(): Promise<void> {
    await this.#mutate("star");
  }
  async unstar(): Promise<void> {
    await this.#mutate("unstar");
  }
  @skipRpcValidation()
  async applyLabel(label: GmailMutableLabel): Promise<void> {
    await this.#mutate("applyLabel", label);
  }
  @skipRpcValidation()
  async removeLabel(label: GmailMutableLabel): Promise<void> {
    await this.#mutate("removeLabel", label);
  }
}
...
class GmailMessageStub extends GmailRpcTarget implements GmailMessage {
  ...
  async thread(): Promise<GmailThread> {
    const scope =
      this.#scope.kind === "mailbox"
        ? this.#scope
        : await restrictedThreadScope(
            this.#ctx,
            this.#threadId,
            this.#scope.admittedMessageIds.filter((messageId) =>
              this.#ctx.store.isSentMessage(messageId),
            ),
          );
    await this.#ctx.approvalQueue.authorizeObservation({
  ...
  async attachments(): Promise<GmailAttachmentEntry[]> {
    ...
    return snapshots.map((snapshot) => ({
      info: snapshot.info,
      attachment: new GmailAttachmentStub(this.#ctx, snapshot.info, () =>
        this.#ctx.api.getAttachmentContent(snapshot),
      ),
    }));
  }
      ...
      },
      {
        ...
        description:
          describeOutboundMessage(
            replyAll ? "Send a reply to all calculated recipients." : "Send a reply.",
            message,
          ) +
  ...
  async forward(to: string[], body?: string, options: GmailComposeOptions = {}): Promise<string> {
    ...
    await this.#ctx.approvalQueue.authorizeObservation({
      ...
      description:
        "Read the complete source message and attachment metadata for an inline forward.",
    });
    ...
    try {
      await submitAction(
        ...
        },
        {
          ...
          description:
            describeOutboundMessage(
              "Forward this message inline with its original body and attachments.",
              message,
            ) +
            `\n\n${formatApprovalField("Source message", this.#messageId)}` +
  ...
  async #createReplyDraft(
    ...
    return createDraftFromMessage(
      this.#ctx,
      message,
      { kind: "reply", messageId: this.#messageId },
      this.#threadId,
    );
  }
  ...
  async createForwardDraft(
    ...
    return createDraftFromMessage(
      this.#ctx,
      message,
      { kind: "forward", messageId: this.#messageId, format: "inline" },
      undefined,
      snapshot,
      { body, options },
    );
...
function describeDraftAction(
  ...
  const attachments = message
    ? message.attachments.map((attachment) =>
        formatApprovalField(
          "Attachment",
          `${attachment.filename || "(unnamed)"} (${attachment.contentType})\n` +
            attachment.description,
        ),
      )
  ...
  const fields = [
    ...
    ...(state.source
      ? [
          formatApprovalField(
            "Source",
            `${state.source.kind} from message ${state.source.messageId}`,
          ),
        ]
      : []),
...
function describeDraftDeletion(state: GmailDraftState): string {
  return (
    "Delete this draft without sending it.\n\n" +
...
async function createDraftFromMessage(
  ...
  threadId?: string,
  sourceSnapshot?: GmailSourceAttachment,
  forwardInput?: { body?: string; options: GmailComposeOptions },
): Promise<GmailDraft> {
  const logicalId = newGmailLogicalId("draft");
  let submitted = false;
  try {
    ...
    const state: GmailDraftState = {
      ...
      ...(source.format === "inline"
        ? forwardInput?.options.html !== undefined
          ? { html: forwardInput.options.html }
          : {}
...
function gmailDraftCursor(ctx: GmailContext): Cursor<GmailDraftEntry> {
  ...
  return new RpcCursor(
    new CursorPager<DraftCursorRef, OwnedGmailDraftEntry>({
      provider: "Gmail",
      async fetchPage(pageToken) {
          ...
          for (const item of items) {
            try {
              ...
              entries.push({
                ...
                draft: new GmailDraftStub(ctx, item.logicalId),
              });
            } catch (error) {
              if (
                !(error instanceof Error) ||
                !/pending deletion|already been sent|has been deleted|was rejected|uncertain send outcome/.test(
                  error.message,
                )
              ) {
                throw error;
      ...
      disposeEntries: (entries) => disposeEntryTargets(entries, (entry) => entry.draft),
    }),
    ctx.approvalQueue,
  );
}
...
function restrictedDraftCursor(ctx: GmailContext): Cursor<GmailDraftEntry> {
  const resources = ctx.store
    .listDrafts()
    .filter((resource) => resource.source && resource.status === "active");
  return new RpcCursor(
    new CursorPager<GmailDraftResource, OwnedGmailDraftEntry>({
      provider: "Gmail",
      async fetchPage(pageToken) {
      ...
      async buildEntries(items) {
        ...
        try {
          const sourceIds = new Set(
            items.flatMap((resource) => (resource.source ? [resource.source.messageId] : [])),
          );
      ...
      authorize: (entries) =>
        ctx.approvalQueue.authorizeObservation({
          title: `Read ${entries.length} restricted Gmail drafts`,
          description:
            "Read only same-binding drafts whose immutable source message remains admitted.",
        }),
      disposeEntries: (entries) => disposeEntryTargets(entries, (entry) => entry.draft),
    }),
    ctx.approvalQueue,
...
class GmailDraftStub extends GmailRpcTarget implements GmailDraft {
  ...
  async attachments(): Promise<GmailAttachmentEntry[]> {
    ...
    const terminal = relevant.findLast(
      ({ action }) => action.type === "draftDelete" || action.type === "draftSend",
    );
          ...
          ? state.attachments.map((attachment, index) => ({
              ...
              attachment: new GmailAttachmentStub(
                ...
                async () => {
                  ...
                  if (data === undefined)
                    throw new Error("Forward attachment is no longer available.");
  ...
  async update(patch: GmailDraftPatch): Promise<void> {
    ...
    await submitAction(
      ...
      },
      {
        ...
        description: describeDraftAction(
          "Replace the selected draft fields.",
          after,
          descriptionMessage,
        ),
      },
  ...
  async delete(): Promise<void> {
    ...
    await submitAction(
      ...
      () => this.#ctx.store.restoreDraftVersion(logicalId, submittedVersion, version, dependencies),
    );
  }
  ...
  async send(): Promise<string> {
    ...
    const importedWithNonTextContent =
      resource.providerId === resource.logicalId &&
      (state.html !== undefined || state.attachments.length > 0);
    if (
      state.text.length === 0 &&
      !importedWithNonTextContent &&
      state.source?.kind !== "reply" &&
      !(state.source?.kind === "forward" && state.source.format === "inline")
    ) {
    ...
    await submitAction(
      ...
      },
      {
        ...
        description: describeDraftAction(
          "Send this exact draft snapshot.",
          approved,
          approvedMessage,
        ),
        awaitDecision: true,
...
function providerDraftId(store: GmailStore, logicalId: string): string {
  ...
  if (resource.status === "rejected")
    throw new Error("The draft creation prerequisite was rejected.");
  if (resource.status === "deleted") throw new Error("This Gmail draft has been deleted.");
  if (resource.status === "sent") throw new Error("This Gmail draft has already been sent.");
  if (!resource.providerId) {
...
function providerLabelId(store: GmailStore, logicalId: string): string {
  ...
  if (resource.status === "rejected")
    throw new Error("The label creation prerequisite was rejected.");
  if (!resource.providerId) {
...
async function exactSpecWithSource(
  ...
  if (inline) {
    ...
    const message = await api.buildForwardFromBytes(
      bytes,
      spec.to,
      spec.text,
      {
...
async function verifyReconciledSend(
  ...
  if (
    provider.id !== sent.id ||
    provider.threadId !== sent.threadId ||
  ...
  if ((await gmailDraftFingerprint(deliveredParsed, provider.threadId)) !== approvedFingerprint) {
    throw new Error(
...
async function sentMessageFingerprint(raw: string, threadId: string | undefined): Promise<string> {
...
async function applyMessageMutation(
  ...
  store: GmailStore,
  actionId: number,
  action: GmailMessageMutationAction,
): Promise<void> {
  const labelId = action.labelId ? providerLabelId(store, action.labelId) : undefined;
  const trashLabelOperation =
    labelId === "TRASH" &&
    (action.operation === "applyLabel" || action.operation === "removeLabel");
  const labels = (() => {
    switch (action.operation) {
      case "archive":
        return { add: [] as string[], remove: ["INBOX"] };
      case "markRead":
        return { add: [] as string[], remove: ["UNREAD"] };
      case "markUnread":
        return { add: ["UNREAD"], remove: [] as string[] };
      case "star":
        return { add: ["STARRED"], remove: [] as string[] };
      case "unstar":
        return { add: [] as string[], remove: ["STARRED"] };
      case "applyLabel":
        return { add: [labelId!], remove: [] as string[] };
      case "removeLabel":
        return { add: [] as string[], remove: [labelId!] };
      case "trash":
        return undefined;
    }
  })();
  const applyTarget = async (target: { messageId?: string; threadId?: string }) => {
    ...
    if (
      ...
      (trashLabelOperation && action.operation === "applyLabel")
    ) {
...
@validateRpc()
export class GmailGatekeeperImpl
  extends DurableObject<Env, GmailGatekeeperImplProps>
  implements Gatekeeper<GmailScopedSession | GmailSession>
{
  ...
  async #getSelfEmail(): Promise<string> {
    ...
    const [subject, description] = await Promise.all([
      this.#getTokenSubject(token),
      getGoogleAccountDescription(token),
    ]);
  ...
  async applyAction(actionId: number, _cache: RpcStub<GitCache>): Promise<void> {
    ...
    if (this.#applying.has(actionId))
      throw new Error(`Gmail action ${actionId} is already applying.`);
    ...
    try {
      ...
      if (
        (this.ctx.props.searchQuery !== undefined || this.ctx.props.labelName !== undefined) &&
        isLegacyGmailAction(action)
      ) {
        throw new Error(
      ...
      switch (action.type) {
        ...
        case "send": {
          ...
          if (store.isApplying(actionId)) {
            const existing = await api.findMessageByRfcMessageId(
              action.spec.messageId,
              "delivered",
            );
            if (!existing) {
          ...
          const sent = await applyUncertainWrite(store, actionId, () =>
            api.sendRawMessage(message.raw, action.threadId),
          );
        ...
        case "draftCreate": {
          ...
          if (resource.providerId) {
            ...
            const parsed = await parseSafeGmailDraft(current.message);
            if (
              (await gmailDraftFingerprint(parsed, current.message.threadId)) !==
              approvedOutputFingerprint
            ) {
              throw new Error(
          ...
          if (store.isApplying(actionId)) {
            if (!action.draft.rfcMessageId) {
              throw new Error(
                "The staged Gmail draft has no stable Message-ID for reconciliation.",
              );
            }
          ...
          const spec = await exactSpecWithSource(
            api,
            store,
            draftSpec(action.draft),
            sourceSnapshot,
            action.draft.source?.format === "inline",
          );
          const message = api.buildOutbound(spec);
          const created = await applyUncertainWrite(store, actionId, () =>
            api.createDraft(message.raw, action.draft.threadId),
          );
          const receipt = {
            draftId: created.id,
        ...
        case "draftUpdate": {
          ...
          if (
            !reconciling &&
            action.expectedProviderMessageId &&
          ...
          if (currentFingerprint === approvedOutputFingerprint) {
            const completed = store.completeMatchingDraftUpdate(
              actionId,
              {
                draftId: id,
                messageId: current.message.id,
          ...
          const spec = sourceSnapshot
            ? await exactSpecWithSource(
                api,
                store,
                draftSpec(action.after),
                sourceSnapshot,
                action.after.source?.format === "inline",
              )
            : draftSpec(action.after, parsed.attachments);
          const message = api.buildOutbound(spec);
          const updated = await applyUncertainWrite(store, actionId, () =>
            api.updateDraft(id, message.raw, action.after.threadId),
          );
          if (updated.id !== id) {
        ...
        case "draftDelete": {
          ...
          try {
            ...
            const resource = store.getDraft(action.draftId);
            if (!resource) {
              throw new Error("The Gmail draft resource disappeared while being deleted.", {
                cause: error,
              });
            }
            resource.status = "deleted";
          ...
          if (current.id !== id)
            throw new Error("Gmail returned a different draft during deletion.");
        ...
        case "draftSend": {
          ...
          if (store.isApplying(actionId)) {
            ...
            if (
              (await gmailDraftFingerprint(sentParsed, sentMessage.threadId)) !==
              approvedOutputFingerprint
            ) {
              throw new Error(
                "A delivered Gmail message matched the approved Message-ID but had different content.",
          ...
          const sent = await applyUncertainWrite(store, actionId, () =>
            api.sendDraft(id, approved.raw, action.approved.threadId),
          );
          if (action.messageId) {
            const completed = store.completeSentAction(actionId, action.messageId, sent);
            store.deleteForwardSnapshot(actionSourceAttachment(completed));
        ...
        case "labelCreate": {
          ...
          const created = await applyUncertainWrite(store, actionId, () =>
            api.createLabel(action.label.name),
          );
        ...
        case "labelRename": {
          ...
          if (current.name !== action.name) {
            ...
            const renamed = await applyUncertainWrite(store, actionId, () =>
              api.renameLabel(id, action.name),
            );
        ...
        case "archive":
          await api.modifyThread(action.threadId, [], ["INBOX"]);
          break;
        case "trash":
          await api.trashThread(action.threadId);
          break;
        case "markRead":
          await api.modifyThread(action.threadId, [], ["UNREAD"]);
          break;
        case "markUnread":
          await api.modifyThread(action.threadId, ["UNREAD"], []);
          break;
  ...
  async rejectAction(actionId: number): Promise<void | { restart?: boolean }> {
    ...
    const writeReceipt =
      action.type === "draftCreate" || action.type === "draftUpdate" ? receipt : undefined;
    if (
      this.#applying.has(actionId) ||
      (store.isApplying(actionId) && !(action.type === "draftCreate" && writeReceipt !== undefined))
    ) {
      throw new Error(
    ...
    const hasDependents = store
      .listActions()
      .some((item) => "dependsOn" in item.action && item.action.dependsOn?.includes(actionId));
    store.pruneDecisions();
