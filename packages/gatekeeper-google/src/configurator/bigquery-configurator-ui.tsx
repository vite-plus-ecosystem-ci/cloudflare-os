...
import type {
  BigQueryConfiguratorRpc,
  BigQueryConfiguratorValues,
...
export default {
  ...
  initialValuesFromResourceUrl({ resourceUrl }) {
    const [projectId, datasetId, tableId] = new URL(resourceUrl).pathname
      .split("/")
      .filter(Boolean);
  ...
  render({ values, setValues, clearFields, ui }) {
    return (
      <Section>
        <Field
          label="Project"
          description="Start with the Google Cloud project this connection can query."
        >
          <Autocomplete
            name="projectId"
        ...
        <Field
          label="Dataset"
          description="Leave blank to allow all datasets in the project."
          optional
        >
          <Autocomplete
            name="datasetId"
            ...
            loadOptions={(query) =>
              values.projectId ? ui.listDatasets(values.projectId, query) : Promise.resolve([])
            }
            onChange={(datasetId) => {
              clearFields("tableId");
        ...
        <Field label="Table" description="Leave blank to allow all tables in dataset." optional>
          <Autocomplete
            ...
            loadOptions={(query) =>
              values.projectId && values.datasetId
