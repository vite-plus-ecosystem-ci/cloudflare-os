...
@validateRpc()
export class LinearWorkspaceConfiguratorUI
  extends RpcTarget
  implements LinearWorkspaceConfiguratorRpc
{
...
@validateRpc()
export class LinearTeamConfiguratorUI
  extends LinearWorkspaceConfiguratorUI
  implements LinearTeamConfiguratorRpc
{
...
@validateRpc()
export class LinearIssueConfiguratorUI
  extends LinearWorkspaceConfiguratorUI
  implements LinearIssueConfiguratorRpc
{
  async listIssues(query: string): Promise<ConfiguratorOption[]> {
    ...
    const options = conn.nodes.map((issue) => ({
      ...
      subtitle: issue.assignee
        ? `Assigned to ${issue.assignee.displayName ?? issue.assignee.name}`
        : undefined,
