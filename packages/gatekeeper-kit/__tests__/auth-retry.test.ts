...
describe("withAuthRetry", () => {
  it("uses the first token without adding a stale-token hint", async () => {
    ...
    expect(await withAuthRetry({ getToken, isAuthError: () => false }, run)).toBe("current-result");
  ...
  it("passes through a first non-auth failure without refreshing", async () => {
    ...
    await expect(withAuthRetry({ getToken, isAuthError: () => false }, run)).rejects.toBe(failure);
    expect(getToken).toHaveBeenCalledOnce();
  });
  ...
  it("refreshes with the rejected token and returns the replayed result", async () => {
    ...
    expect(
      await withAuthRetry({ getToken, isAuthError: (error) => error === authError }, run),
    ).toBe("accepted");
