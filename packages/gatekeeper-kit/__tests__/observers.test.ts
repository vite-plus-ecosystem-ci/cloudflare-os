...
describe("ObserverTracker", () => {
  ...
  it("keeps a fresh in-flight admission counted against the cap", async () => {
    ...
    await expect(instance.addObserver("second", verifier("a"))).rejects.toThrow(
      /already answers for 1/,
    );
    gate.resolve();
    await admitting;
  ...
  it("treats a legacy `true` marker as observed", async () => {
    ...
    const asked: string[][] = [];
    const instance = tracker(kv, {
      hasCollectionAccess: async (_value, collectionIds) => (
        asked.push([...collectionIds]),
        collectionIds.map(() => true)
      ),
    });
    ...
    const chunking = new ObserverTracker<V>({
      ...
      hasCollectionAccess: async (value, collectionIds) => {
        ...
        while (batch.length > 0) {
          for (const collectionId of batch.splice(0, 2))
            verdicts.push(value.allowed.includes(collectionId));
        }
        return verdicts;
      },
    });
  ...
  it("excludes an observer whose verifier throws, rather than failing the read", async () => {
    ...
    try {
      ...
      expect(
        logged.mock.calls.map(([entry]) => {
          const { event, observerId, vendorId } = entry as {
            event: string;
            observerId: string;
            vendorId: string;
          };
  ...
  it("denies when the oracle answers MORE sets than it was asked about", async () => {
    ...
    const overlong = new ObserverTracker<V>({
      kv,
      hasCollectionAccess: async () => [true, true, false],
    });
  ...
  it("excludes an observer whose exclusion verdicts are overlong", async () => {
    ...
    const overlong = new ObserverTracker<V>({
      kv,
      hasCollectionAccess: async () => [true, true],
    });
  ...
  it("canonicalizes exactly once, so even a non-idempotent transform agrees with itself", async () => {
    ...
    const instance = tracker(kv, {
      collectionPrefix: "observedFile:",
      canonicalCollectionId: encodeURIComponent,
    });
...
describe("observer strategies", () => {
  it("A: private bindings refuse every observer", async () => {
    ...
    await expect(strategy.addObserver("x", someUser)).rejects.toThrow(
      "This mailbox cannot be shared.",
    );
  ...
  it("B: ACL bindings admit on a positive answer, and share one default denial", async () => {
    await expect(
      aclObservers({ hasAccess: async () => true }).addObserver("x", someUser),
    ).resolves.toBeUndefined();
    await expect(
      aclObservers({ hasAccess: async () => false }).addObserver("x", someUser),
  ...
  it("B: ACL bindings deny a malformed answer rather than admitting it", async () => {
    ...
    await expect(aclObservers({ hasAccess }).addObserver("x", someUser)).rejects.toThrow(
      OBSERVER_DENIED,
    );
  });
...
describe("ObservationGate", () => {
  ...
  it("authorizes with the strategy's exclusions, then commits", async () => {
    ...
    const authorizing = new ObservationGate(
      fakeAuthorizer(authorizeObservation),
      strategy,
  ...
  it("releases the queue dup it was handed", () => {
    ...
    } as unknown as RpcStub<ObservationAuthorizer>;
  ...
  it("discards rather than commits when the overseer refuses, keeping its error", async () => {
    ...
    const authorizeObservation = vi.fn(async () => {
      throw refusal("cannot hide observation");
    });

    await expect(
      new ObservationGate(fakeAuthorizer(authorizeObservation), strategy).authorize(read, {
        kind: "collections",
        ids: ["p1"],
      }),
  ...
  it("passes the description through untouched for a strategy that tracks nothing", async () => {
    ...
    await new ObservationGate(fakeAuthorizer(authorizeObservation), openObservers()).authorize(
      description,
      { kind: "baseline" },
    );
  ...
  it("refuses collection ids under a strategy that cannot check them", async () => {
    ...
    await expect(
      new ObservationGate(fakeAuthorizer(authorizeObservation), acl).authorize(read, {
        kind: "collections",
        ids: ["p1"],
      }),
  ...
  it("refuses collection ids under an open strategy, which draws no distinction to enforce", async () => {
    ...
    await expect(
      new ObservationGate(fakeAuthorizer(authorizeObservation), openObservers()).authorize(read, {
        kind: "collections",
        ids: ["p1"],
      }),
    ).rejects.toThrow(/cannot enforce collection ACLs/);
  ...
  it("allows a collection scope under a private strategy, where nobody is admitted to exclude", async () => {
    ...
    await new ObservationGate(
      fakeAuthorizer(authorizeObservation),
      privateObservers("no sharing"),
    ).authorize(read, { kind: "collections", ids: ["p1"] });
  ...
  it("refuses a collections scope naming no collection, which meant two opposite things in the corpus", async () => {
    ...
    ).rejects.toThrow(/at least one collection id/);
    expect(authorizeObservation).not.toHaveBeenCalled();
  });

  it("asks the strategy nothing for a read the admission baseline covers", async () => {
    const authorizeObservation = vi.fn(async () => {});
    const hasCollectionAccess = vi.fn(async (_v: V, collectionIds: readonly string[]) =>
      collectionIds.map(() => false),
    );
    const strategy = trackedCollectionObservers<V>({ kv: makeKv(), hasCollectionAccess });
    await strategy.addObserver("x", someUser);

    await new ObservationGate(fakeAuthorizer(authorizeObservation), strategy).authorize(read, {
      kind: "baseline",
    });
  ...
  it("closes admission after a withheld read, which no later verification can clear", async () => {
    ...
    await new ObservationGate(fakeAuthorizer(), strategy).authorize(read, {
      kind: "withholdFromObservers",
    });
    ...
    expect(
      new ObserverTracker<V>({ kv, hasCollectionAccess: async () => [] }).observerIds(),
    ).toEqual([]);
  });
  ...
  it("reopens admission when the overseer refuses a withheld read", async () => {
    ...
    await expect(
      new ObservationGate(refusing, strategy).authorize(read, { kind: "withholdFromObservers" }),
  ...
  it("latches the fence at once when a withheld read's outcome is unknown", async () => {
    ...
    for (const _attempt of [1, 2, 3]) {
      await expect(
        new ObservationGate(failing, strategy).authorize(read, { kind: "withholdFromObservers" }),
      ).rejects.toThrow("connection lost");
    }
  ...
  it("fences admission while a withheld read is still awaiting the overseer", async () => {
    ...
    const authorizing = new ObservationGate(
      fakeAuthorizer(vi.fn(() => overseer.promise)),
      strategy,
  ...
  it("holds the fence for a second withheld read when the first is refused", async () => {
    ...
    const reclaiming = new ObservationGate(
      fakeAuthorizer(vi.fn(() => refused.promise)),
      strategy,
    ).authorize(read, { kind: "withholdFromObservers" });
    const surviving = new ObservationGate(
      fakeAuthorizer(vi.fn(() => overseer.promise)),
      strategy,
    ).authorize(read, { kind: "withholdFromObservers" });
  ...
  it("keeps the fence when the latch write fails", async () => {
    ...
    await expect(
      new ObservationGate(fakeAuthorizer(vi.fn(async () => {})), strategy).authorize(read, {
        kind: "withholdFromObservers",
      }),
  ...
  it("fences admission durably before the overseer is asked", async () => {
    ...
    const revived = trackedCollectionObservers<V>({
      kv: { ...kv },
      hasCollectionAccess: async () => [],
    });
  ...
  it("refuses an owner-only read where the strategy shares every read", async () => {
    ...
    ).rejects.toThrow(/shares every read/);
  ...
  it("leaves the caller's containsRestrictedData alone, being a gadget-wide escalation", async () => {
    ...
    await new ObservationGate(fakeAuthorizer(authorizeObservation), strategy).authorize(
      { ...read, containsRestrictedData: true },
      { kind: "collections", ids: ["p1"] },
    );
  ...
  it("reaches the git cache through the gate", async () => {
    ...
    const gate = new ObservationGate(
      { getGitCache } as unknown as RpcStub<ObservationAuthorizer>,
      openObservers(),
    );
