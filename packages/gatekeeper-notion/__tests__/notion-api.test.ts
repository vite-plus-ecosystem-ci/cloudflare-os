...
describe("parseNotionId", () => {
  ...
  it("parses slug URLs whose last word ends in a hex character", () => {
    expect(
      parseNotionId("https://app.notion.com/p/Team-Home-8513f007d6074da88eebabcbcd0b78f4"),
    ).toBe("8513f007-d607-4da8-8eeb-abcbcd0b78f4");
    expect(parseNotionId("https://app.notion.com/p/Ad-Hoc-cf259cd6c939441da86764eef3892c66")).toBe(
      "cf259cd6-c939-441d-a867-64eef3892c66",
    );
    expect(
      parseNotionId("https://www.notion.so/My-Page-Title-1c5e94bd682e422fb42d13fea9f655a0"),
  ...
  it("parses slug URLs whose last word ends in a non-hex character", () => {
    expect(
      parseNotionId(
        "https://app.notion.com/p/V-lkommen-till-Notion-38809dfc5d6380b9a93ad369abe07009",
      ),
    ).toBe("38809dfc-5d63-80b9-a93a-d369abe07009");
  });

  it("parses bare IDs and dashed UUIDs", () => {
    expect(parseNotionId("https://www.notion.so/38809dfc5d6380b9a93ad369abe07009")).toBe(
      "38809dfc-5d63-80b9-a93a-d369abe07009",
    );
    expect(parseNotionId("38809dfc5d6380b9a93ad369abe07009")).toBe(
      "38809dfc-5d63-80b9-a93a-d369abe07009",
    );
    expect(parseNotionId("2c5e94bd-682e-422f-b42d-13fea9f655a0")).toBe(
      "2c5e94bd-682e-422f-b42d-13fea9f655a0",
    );
  });
...
describe("blocksToMarkdown emphasis", () => {
  it("merges adjacent same-style runs instead of emitting ****", () => {
    const md = blocksToMarkdown([
      paragraph([run("foo", { bold: true }), run("bar", { bold: true })]),
    ]);
...
describe("simulatedTitle", () => {
  it("derives the title from setTitle, the create title, and title-typed properties", () => {
    expect(
      simulatedTitle(
        [rec(1, { type: "setTitle", pageId: "p", title: "Renamed", previousTitle: "" })],
        "Old",
      ),
    ).toBe("Renamed");
      ...
      simulatedTitle(
        [
          rec(1, {
            type: "createPage",
            provisionalId: "~1",
            parent: { kind: "database", databaseId: "d" },
      ...
      simulatedTitle(
        [
          rec(1, {
            type: "setProperties",
            pageId: "p",
            properties: { Name: { type: "title", text: "Edited" } },
            previousProperties: {},
          }),
        ],
        "Old",
...
describe("simulatePageContent", () => {
  it("reflects the markdown->block conversion (not a raw echo)", () => {
    const md = simulatePageContent(null, [
      rec(1, { type: "appendContent", pageId: "p", markdown: "##### deep\n\n* a" }),
    ]);
        ...
        children: [
          {
            block: {
              id: "x",
              type: "paragraph",
              paragraph: { rich_text: [run("secret body")] },
            } as any,
          },
        ],
      },
...
describe("propertyValueToInput", () => {
  it("round-trips writable values and rejects computed ones", () => {
    expect(propertyValueToInput({ type: "select", option: "Doing" })).toEqual({
      type: "select",
      option: "Doing",
    });
    expect(propertyValueToInput({ type: "people", people: [{ id: "u1" }, { id: "u2" }] })).toEqual({
      type: "people",
      userIds: ["u1", "u2"],
    });
