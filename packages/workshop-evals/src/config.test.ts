...
import {
  EVAL_AGENT_BUDGET_MS,
  EVAL_TEST_TIMEOUT_MS,
  EVAL_VERIFICATION_BUDGET_MS,
} from "./budgets.js";
...
it("leaves an outer margin for setup and cleanup", () => {
  expect(EVAL_TEST_TIMEOUT_MS).toBeGreaterThan(EVAL_AGENT_BUDGET_MS + EVAL_VERIFICATION_BUDGET_MS);
});
...
it("rejects a model that is not in the catalog", () => {
  expect(() => evalMatrix({ WORKSHOP_EVAL_MODELS: "@cf/zai-org/glm-5.2, model-b" })).toThrow(
    'Unknown eval model "model-b"',
  );
});
...
it("resolves catalog models to their provider", () => {
  expect(resolveEvalModel("gemini-3.6-flash")).toEqual({
    provider: "google",
    model: "gemini-3.6-flash",
  });
  expect(resolveEvalModel("claude-sonnet-5")).toEqual({
    provider: "anthropic",
    model: "claude-sonnet-5",
  });
  expect(resolveEvalModel("@cf/deepseek-ai/deepseek-v4-pro-0813")).toEqual({
    provider: "cloudflare",
...
it("resolves eval-only models that the picker does not offer", () => {
  expect(resolveEvalModel("@cf/meta/llama-3.3-70b-instruct-fp8-fast")).toEqual({
    provider: "cloudflare",
...
it("does not guess a provider for an unlisted model", () => {
  expect(() => resolveEvalModel("@cf/vendor/not-in-catalog")).toThrow(
    "must be listed in SUGGESTED_MODELS or EVAL_ONLY_MODELS",
  );
});
...
it("versions only the task prompts", () => {
  ...
  expect(
    taskVersion({ ...task, turns: [{ ...task.turns[0], prompt: "Build it better" }] }),
  ).not.toBe(version);
});
