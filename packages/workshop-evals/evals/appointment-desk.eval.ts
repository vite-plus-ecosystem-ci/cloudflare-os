  ...
  personIds lists the people currently holding a place, and its length always equals bookedCount.`,
      verify: async (verifier) => {
        await verifier.check("sequential-booking-respects-capacity", async () => {
          ...
          const defined = OkSchema.parse(
            await api.defineSlot({
              slotId: "seq",
          ...
          return {
            pass:
              defined.ok &&
              !duplicate.ok &&
              duplicate.error === "DUPLICATE_SLOT" &&
              results[0]?.ok === true &&
              results[1]?.ok === true &&
              results[2]?.ok === false &&
              results[2].error === "SLOT_FULL" &&
              status.capacity === 2 &&
              status.bookedCount === 2 &&
        ...
        await verifier.check("cancelling-frees-a-place", async () => {
          ...
          return {
            pass:
              !blocked.ok &&
              blocked.error === "SLOT_FULL" &&
              cancelled.ok &&
              !unknown.ok &&
              unknown.error === "UNKNOWN_BOOKING" &&
              retried.ok &&
              status.capacity === 1 &&
              status.bookedCount === 1 &&
              status.personIds.join() === "ben",
        ...
        await verifier.check("rejects-unknown-slot", async () => {
          ...
          return {
            pass:
              !result.ok &&
              result.error === "UNKNOWN_SLOT" &&
              defined.ok &&
              retried.ok &&
              status.bookedCount === 1 &&
              status.personIds.join() === "ana",
        ...
        await verifier.check("concurrent-duplicate-booking-ids-collapse-to-one", async () => {
          ...
          const [racedLow, racedHigh] = (
            await Promise.all([
              ...
              api.defineSlot({
                slotId: "dupe-other",
                startIso: "2026-07-04T11:00:00Z",
                capacity: 7,
              }),
            ])
          ).map((result) => OkSchema.parse(result));
          const winnerCapacity = racedLow.ok ? 2 : racedHigh.ok ? 7 : null;
          const defineRejections = [racedLow, racedHigh].map((result) =>
          ...
          const extras = (
            await Promise.all(
              Array.from({ length: 5 }, (_unused, index) =>
                api.book({
                  bookingId: `dupe-extra-${index}`,
                  slotId: "dupe",
                  personId: `extra-${index}`,
                }),
              ),
            )
          ).map((result) => OkSchema.parse(result));
          ...
          return {
            pass:
              accepted === 1 &&
              errors.DUPLICATE_BOOKING === 5 &&
              racedLow.ok !== racedHigh.ok &&
              defineRejections.every((code) => code === null || code === "DUPLICATE_SLOT") &&
              otherStatus.capacity === winnerCapacity &&
              !crossSlot.ok &&
              crossSlot.error === "DUPLICATE_BOOKING" &&
              dupeStatus.capacity === 8 &&
              dupeStatus.bookedCount === 1 &&
              dupeStatus.personIds.join() === "ana" &&
              otherStatus.bookedCount === 0 &&
              otherStatus.personIds.length === 0 &&
              extras.every((result) => result.ok) &&
              finalStatus.capacity === 8 &&
              finalStatus.bookedCount === 6 &&
        ...
        await verifier.check("bookings-survive-code-reload", async () => {
          ...
          return {
            pass:
              status.capacity === 2 &&
              status.bookedCount === 2 &&
              status.personIds.length === 2 &&
              status.personIds.includes("ana") &&
              status.personIds.includes("ben") &&
              cancelStatus.capacity === 1 &&
              cancelStatus.bookedCount === 1 &&
              cancelStatus.personIds.join() === "ben" &&
              cancelledAgain.ok &&
              rebooked.ok &&
              rebookedStatus.capacity === 1 &&
              rebookedStatus.bookedCount === 1 &&
