...
function canonicalMonthly(monthly: z.infer<typeof MonthlySchema>) {
  return {
    ...
    byPayer: monthly.byPayer.toSorted((left, right) => left.personId.localeCompare(right.personId)),
  };
}

function canonicalBudgetStatus(status: z.infer<typeof BudgetStatusSchema>) {
  return status.byCategory.toSorted((left, right) => left.category.localeCompare(right.category));
}
  ...
  Counting only expenses whose dateIso falls inside that month.`,
      verify: async (verifier) => {
        await verifier.check("splits-leftover-cents-without-losing-any", async () => {
          ...
          const nonLexical = OkSchema.parse(
          ...
          return {
            pass:
              emptySettlement.transfers.length === 0 &&
              OkSchema.parse(added).ok &&
              nonLexical.ok &&
              ...
              net(balances, "order-z") === 2 &&
              net(balances, "order-a") === -2,
        ...
        await verifier.check("balances-always-sum-to-zero", async () => {
          ...
          const missingPeople = expectedPeople.filter(
            (personId) => net(balances, personId) === null,
          );
          ...
          const wrongBalances = Object.entries(expectedBalances).filter(
            ([personId, expected]) => net(balances, personId) !== expected,
          );
          ...
          return {
            pass:
              ...
              wrongBalances.length === 0 &&
              total === 0 &&
              net(balances, "outside-payer") === 101 &&
              net(balances, "outside-ana") === -51 &&
              net(balances, "outside-ben") === -50 &&
        ...
        await verifier.check("settlement-clears-every-balance-and-stays-short", async () => {
          ...
          const unchanged =
            ...
            JSON.stringify(canonicalMonthly(mayBefore)) ===
              JSON.stringify(canonicalMonthly(mayAfter));
          return {
            pass:
              analysis.owing >= 2 &&
              settlement.transfers.length >= 1 &&
              analysis.knownEndpoints &&
        ...
        await verifier.check("rejects-bad-input-with-named-codes", async () => {
          ...
          const rejections = [
            {
              ...
              result: OkSchema.parse(await api.addExpense({ ...valid, amountCents: 999 })),
            },
            {
              expected: "INVALID_AMOUNT",
              result: OkSchema.parse(
          ...
          const unchanged =
            JSON.stringify(canonicalBalances(before)) ===
        ...
        await verifier.check("monthly-totals-count-only-that-month", async () => {
          ...
          const duplicateRace = (
            await Promise.all(duplicateInputs.map((input) => api.addExpense(input)))
          ).map((result) => OkSchema.parse(result));
          ...
          return {
            pass:
              ...
              duplicateRace.filter((result) => !result.ok && result.error === "DUPLICATE_ID")
                .length === 1 &&
              march.totalCents === 3_700 &&
              marchPaid("month-ana") === 2_500 &&
              marchPaid("month-ben") === 1_200 &&
              winner !== undefined &&
              april.totalCents === 90_000 + winner.amountCents &&
              aprilPaid("month-ana") === 90_000 &&
              aprilPaid(winner.payerId) === winner.amountCents &&
              empty.totalCents === 0 &&
              empty.byPayer.length === 0,
        ...
        await verifier.check("turn-one-state-survives-code-reload", async () => {
          ...
          return {
            pass:
              ...
              net(balances, "month-ana") === 45_650 &&
              net(balances, "month-ben") === -45_650 &&
...
Everything that already worked must keep working, including the expenses I have already recorded.`,
      verify: async (verifier) => {
        ...
        await verifier.check("pre-existing-spending-counts-as-uncategorized", async () => {
          using api = await verifier.connect<BudgetedLedgerApi>("Ledger");
          const status = BudgetStatusSchema.parse(
            await api.budgetStatus({ month: TURN_ONE_MONTH }),
          );
          const uncategorized = status.byCategory.find((row) => row.category === "uncategorized");
          return {
            pass:
              uncategorized?.spentCents === 3_700 &&
              uncategorized.limitCents === null &&
        ...
        await verifier.check("budget-remaining-tracks-spending-and-can-go-negative", async () => {
          ...
          const uniqueCategories =
            june.byCategory.length === 4 &&
            july.byCategory.length === 1 &&
            new Set(june.byCategory.map((entry) => entry.category)).size ===
              june.byCategory.length &&
            new Set(july.byCategory.map((entry) => entry.category)).size === july.byCategory.length;
          const unchanged =
            JSON.stringify(canonicalMonthly(monthlyBefore)) ===
          ...
          return {
            pass:
              budgets.every((result) => result.ok) &&
              added.every((result) => result.ok) &&
              uniqueCategories &&
              juneRow("food")?.limitCents === 5_000 &&
              juneRow("food")?.spentCents === 6_500 &&
              juneRow("food")?.remainingCents === -1_500 &&
              juneRow("travel")?.limitCents === 20_000 &&
              juneRow("travel")?.spentCents === 4_000 &&
              juneRow("travel")?.remainingCents === 16_000 &&
              juneRow("savings")?.limitCents === 30_000 &&
              juneRow("savings")?.spentCents === 0 &&
              juneRow("savings")?.remainingCents === 30_000 &&
              juneRow("uncategorized")?.limitCents === null &&
              juneRow("uncategorized")?.spentCents === 100 &&
              julyFood?.limitCents === 8_000 &&
              julyFood.spentCents === 0 &&
              julyFood.remainingCents === 8_000 &&
              monthlyBefore.totalCents === 10_600 &&
              junePaid("budget-ana") === 6_600 &&
              junePaid("budget-ben") === 4_000 &&
      ...
      verifyAfterAccept: async (verifier) => {
        await verifier.check("turn-two-state-survives-code-reload", async () => {
          ...
          const marchTotals = MonthlySchema.parse(
            await api.monthlyTotals({ month: TURN_ONE_MONTH }),
          );
          const balances = BalancesSchema.parse(await api.balances());
          const settlement = SettlementSchema.parse(await api.settlement());
          const analysis = analyzeSettlement(balances, settlement);
          const juneAfter = BudgetStatusSchema.parse(await api.budgetStatus({ month: "2026-06" }));
          const julyAfter = BudgetStatusSchema.parse(await api.budgetStatus({ month: "2026-07" }));
          const juneTotalsAfter = MonthlySchema.parse(
            await api.monthlyTotals({ month: "2026-06" }),
          );
          const balancesAfter = BalancesSchema.parse(await api.balances());
          const unchanged =
            JSON.stringify(canonicalBudgetStatus(june)) ===
          ...
          return {
            pass:
              ...
              julyFood?.limitCents === 8_000 &&
              julyFood.spentCents === 0 &&
              juneTotals.totalCents === 10_600 &&
              marchTotals.totalCents === 3_700 &&
              net(balances, "budget-ana") === 1_300 &&
              net(balances, "budget-ben") === -1_300 &&
              analysis.knownEndpoints &&
              analysis.unsettled.length === 0 &&
              unchanged,
            evidence: {
