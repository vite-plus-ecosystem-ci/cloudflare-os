...
async function loadServerOptions(
  ...
  return needle
    ? servers.filter(
        (server) =>
          server.value.toLowerCase().includes(needle) ||
...
export default {
  ...
  async initialValuesFromResourceUrl({ resourceUrl, ui }) {
    ...
    const server = requestedServer
      ? servers.some((option) => option.value === requestedServer)
        ? requestedServer
        : null
      : servers.length === 1
        ? servers[0].value
      ...
      void serverOptions(ui).then(
        (servers) => {
          const server = values.server
            ? servers.some((option) => option.value === values.server)
              ? values.server
              : null
            : servers.length === 1
              ? servers[0].value
        ...
        {toolsReady && (
          ...
          >
            <CheckboxList
              ...
              value={values.tools}
              loadOptions={async () =>
                (await ui.listToolOptions(serverKey)).map((option) => ({
                  ...option,
