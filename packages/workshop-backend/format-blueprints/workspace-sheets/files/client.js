...
function makeEngine(model) {
  ...
  const H = {
    ...
    scalar(node, ctx) {
      ...
      if (v && v.matrix) v = v.get(0, 0);
      return v;
    },
    matrixOf(node, ctx) {
      const v = evalNode(node, ctx);
      return v && v.matrix ? v : { matrix: true, rows: 1, cols: 1, get: () => v };
    },
  };
...
const FUNCTIONS = (() => {
  ...
  for (const fn of ["SIN", "COS", "TAN", "ASIN", "ACOS", "ATAN", "SINH", "COSH", "TANH"])
    F[fn] = (a, c, H) => Math[fn.toLowerCase()](H.toNum(s(a, c, H, 0)));
  ...
  F.SUMIFS = (a, c, H) => {
    ...
    let t = 0;
    for (let i = 0; i < mask.length; i++) if (mask[i] && typeof sum[i] === "number") t += sum[i];
    return t;
  };
  ...
  F.WEEKNUM = (a, c, H) => {
    ...
    return Math.floor(((d - start) / 86400000 + new Date(start).getUTCDay()) / 7) + 1;
  };
...
function iconBtn(name, title, onClick, label) {
  ...
  else b.innerHTML = icon(ICONS[name]);
...
function customSelect({ className, title, options, value, onChange, width }) {
  ...
  const items = options
    .map((o) => {
      if (o.sep) {
        const sp = el("div", { class: "cmenu-sep" });
        menu.appendChild(sp);
        return null;
      }
      const item = el("div", { class: "cmenu-item", "data-value": String(o.value) }, [
        el("span", {}, o.label),
        o.ex ? el("span", { class: "ex" }, o.ex) : null,
      ]);
...
const currencyBtn = iconBtn("currency", "Format as currency", () =>
  setFmtOnSelection((f) => {
    f.nf = "currency";
  }),
);
const percentBtn = iconBtn("percent", "Format as percent", () =>
  setFmtOnSelection((f) => {
    f.nf = "percent";
  }),
);
...
async function doSave() {
  ...
  if (!pendingCellOps.size && !pendingStructure && !pendingReplacements.size) {
  ...
  for (const op of pendingCellOps.values()) {
    ...
    cellOps.push({
      ...
      value: op.value,
      fmt: op.fmt,
      baseVersion: op.baseVersion || (cur ? cur.version : 0),
    });
  }
  const structure = pendingStructure;
  const sheetReplacements = Array.from(pendingReplacements.entries()).map(([sheetId, cells]) => ({
    sheetId,
    cells,
  }));
...
function applyHistory(entry, into) {
  ...
  for (const [key, rec] of entry.cells) {
    ...
    const p = pendingCellOps.get(pk);
    if (p) p.baseVersion = now ? now.version : 0;
  }
  into.push(inverse);
  rebuildEngine();
  renderGrid();
...
function setCellValue(ref, value, { batch = true } = {}) {
  ...
  if ((value == null || value === "") && (!cur || !cur.fmt)) {
    if (cur) {
      delete cells[ref];
      queueCellOp(sheetId, ref, null, null);
    }
    return;
  }
...
function walkRefs(node, fn) {
  ...
  if (node.k === "ref") {
    ...
    for (const key of ["a", "b"]) if (node[key]) walkRefs(node[key], fn);
    if (node.args) node.args.forEach((n) => walkRefs(n, fn));
  }
}
...
function cellEl(r, c) {
  return gridTable.querySelector(`td.cell[data-r="${r}"][data-c="${c}"]`);
}
...
gridScroll.addEventListener("keydown", (e) => {
  ...
  if (meta) {
    switch (e.key.toLowerCase()) {
      ...
      case "arrowdown":
        e.preventDefault();
        moveActive(jumpEdge(focus.r, focus.c, 1, 0), focus.c, e.shiftKey);
        return;
      case "arrowup":
        e.preventDefault();
        moveActive(jumpEdge(focus.r, focus.c, -1, 0), focus.c, e.shiftKey);
    ...
    for (let col = r.c1; col <= r.c2; col++) {
      ...
      if (cell) {
        ...
        if (cell.fmt) {
          ...
          delete curCells()[ref];
          queueCellOp(activeSheetId, ref, null, null);
        }
      }
    }
  commitBatch();
    ...
    for (let j = 0; j < cols.length; j++) {
      ...
      if (useSnapshot && copyFallback.cells[i] && copyFallback.cells[i][j] !== undefined) {
        ...
        if (src) {
          ...
          queueCellOp(activeSheetId, ref, src.value, src.fmt);
        } else {
          if (getCell(ref)) {
            delete curCells()[ref];
            queueCellOp(activeSheetId, ref, null, null);
...
setInterval(() => {
  ...
  let changed = false;
  for (const [id, p] of collaborators)
    if ((p.seenAt || 0) < cutoff) {
...
function applyRemoteOperation(event) {
  ...
  setTimeout(() => {
    if (!saveInFlight && !pendingCellOps.size) setStatus("saved", "Saved");
  }, 900);
}
...
try {
  const doc = await gadget.subscribe(new SheetCallbacks(), {
    clientId,
    name: collaboratorName,
    color: collaboratorColor,
  });
