...
describe("worktrees in the chat change stream", () => {
  ...
  it("rejects client-submitted sets and removes over symlink and gitlink base entries", () =>
    withImpl(async (impl) => {
      ...
      let submit = (n: number, entry: [string, FileChange]) =>
      ...
      await expect(submit(2, ["vendored", { remove: true }])).rejects.toThrow(
...
describe("client delivery filtering", () => {
  ...
  it("keeps gadget entries while stripping worktree entries from a mixed batch", () =>
    withImpl(async (impl) => {
      ...
      let stored = chatMessages(impl, 1)
  ...
  it("strips worktree pins from delivered chat metadata", () =>
    withImpl(async (impl) => {
      addChat(impl, 1);
      let c1 = await commitFiles(impl, { "a.txt": "one\n" });
      let { id } = await createThroughBarrier(impl, 1, c1);
  ...
  it("a mixed chat proposes only its gadget workpieces", () =>
    withImpl(async (impl) => {
      addChat(impl, 1);
      let c1 = await commitFiles(impl, { "a.txt": "one\n" });
      let { id } = await createThroughBarrier(impl, 1, c1);
...
describe("epoch reset re-pins worktrees", () => {
  ...
  it("re-pins a clean worktree at its unchanged base, and one matching its head at the head", () =>
    withImpl(async (impl) => {
      ...
      await impl.mergeChanges(1, USER_META, "client-user");
      expect(impl.storage.gadgets.get(id)!.pinBase).toBe(c1);
      expect(chatMessages(impl, 1).find((msg) => msg.type === "merge")!.worktreePins).toEqual([
        { worktreeId: id, baseCommit: c1 },
      ]);
  ...
  it("carries worktree re-pins into compaction checkpoints", () =>
    withImpl(async (impl) => {
      addChat(impl, 1);
      let c1 = await commitFiles(impl, { "a.txt": "one\n" });
      let { id } = await createThroughBarrier(impl, 1, c1);
