...
describe("assertChatAttachmentSupportedByProvider", () => {
  ...
  it("rejects pdfs without a selected model", () => {
    expect(() => assertChatAttachmentSupportedByProvider(undefined, "application/pdf", 1)).toThrow(
  ...
  it("applies provider raw-file policy", () => {
    ...
    expect(() =>
      assertChatAttachmentSupportedByProvider("anthropic", "text/plain", 1),
    ).not.toThrow();
    expect(() =>
      assertChatAttachmentSupportedByProvider("anthropic", "application/pdf", 1),
    ).not.toThrow();
    expect(() => assertChatAttachmentSupportedByProvider("openai", "image/jpeg", 1)).not.toThrow();
    expect(() =>
      assertChatAttachmentSupportedByProvider("openai", "application/pdf", 1),
    ).not.toThrow();
    expect(() =>
      assertChatAttachmentSupportedByProvider("google", "application/pdf", 1),
    ).not.toThrow();
    expect(() => assertChatAttachmentSupportedByProvider("google", "application/zip", 1)).toThrow(
      "Unsupported file type",
    );
    expect(() =>
      assertChatAttachmentSupportedByProvider("cloudflare", "application/pdf", 1),
    ).toThrow("Unsupported file type");
    expect(() => assertChatAttachmentSupportedByProvider("ollama", "application/zip", 1)).toThrow(
      "Unsupported file type",
    );
  });
  ...
  it("enforces the per-file byte limit", () => {
    expect(() =>
      assertChatAttachmentSupportedByProvider(undefined, "text/plain", 1024 * 1024 + 1),
...
describe("validateChatAttachmentUpload", () => {
  ...
  it.each([
    ...
    expect(() => validateChatAttachmentUpload(attachment, "google")).toThrow(
      "Unsupported file type",
    );
