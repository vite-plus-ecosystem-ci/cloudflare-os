...
describe("restarting sessions when verification scope widens", () => {
  ...
  it("a connection under construction is unreachable until the restart is scheduled", () =>
    withImpl(async (impl, restarts) => {
      ...
      let describing = new Promise<void>((resolve) => {
  ...
  it("adding a vendorless connection widens nothing", () =>
    withImpl(async (impl, restarts) => {
      joinSession(impl);
      stubFacets(impl);
  ...
  it("promoting a pending bind at merge restarts a connected use collaborator", () =>
    withImpl(async (impl, restarts) => {
      ...
      expect(await impl.mergeChanges(1, USER_META, "owner-user-do")).toEqual({ outcome: "merged" });
  ...
  it("a session of the unaffected role is left alone", () =>
    withImpl(async (impl, restarts) => {
  ...
  it("an external message counts as a live build session once its caller is authorized", () =>
    withImpl(async (impl, restarts, instance) => {
      ...
      let pending = instance.receiveExternalMessage({
        callerEmail: "carol@example.com",
        externalChatKey: "k",
        idempotencyKey: "i",
        prompt: "hello",
        chatGatewayRpcTarget: {} as any,
        title: "T",
      } as any);
      await reached;
      ...
      let reached = new Promise<void>((resolve) => {
        impl.authorizeCollaborator = () => {
          ...
          return new Promise((resolveAuth) => {
            deny = () => resolveAuth(null);
          });
        };
      });

      let pending = instance.receiveExternalMessage({
        callerEmail: "mallory@example.com",
        externalChatKey: "k",
        idempotencyKey: "i",
        prompt: "hello",
        chatGatewayRpcTarget: {} as any,
  ...
  it("a failed re-verification leaves the collaborator's other sessions alone", () =>
    withImpl(async (impl, restarts) => {
      ...
      impl.getGatekeeperFacet = () => ({
        addObserver: async () => {
          throw new Error("access revoked upstream");
        },
...
describe("hooks widen use scope", () => {
  function seedHook(
    impl: any,
    opts: { enabled?: boolean; controller?: object; gadgetId?: number } = {},
  ): void {
      ...
      let client = await openFakeOverseer(
        {},
        {
          impl: {
            ...
            joinSession: (kind: string) => impl.joinSession(kind),
            subscribeToConsoleLogs: async () =>
              new NativeRpcStub<{}>({ [Symbol.dispose]() {} } as any),
          },
        },
      );
...
describe("connections blocked pending restart", () => {
  ...
  it("a blocked connection's slash-command invoke is refused", () =>
    withImpl(async (impl) => {
      ...
      await expect(
        impl.sendChatMessage({ id: { toString: () => "user-do" } } as any, USER_META as any, 1, {
          id: { gatekeeperId: id, commandId: "deploy" },
          args: "",
        } as any),
  ...
  it("usable ambient catalogs retry and refresh while blocked connections stay skipped", () =>
    withImpl(async (impl) => {
      ...
      let added = await impl.addGatekeeper(usable.class, { type: "ambient", vendorId: "v2" });
