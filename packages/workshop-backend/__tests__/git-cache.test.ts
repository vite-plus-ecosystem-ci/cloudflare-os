...
describe("client-facing reads (readCommitTree / readFilesAtCommit)", () => {
  ...
  it("answers every requested path in request order, pulling all blobs in one batch", async () => {
    let t = await fixtureWithRemoteBlobs();
    let paths = [
      "src/util.js",
      "nope.txt",
    ...
    expect(result).toStrictEqual([
      ...
      [
        ...
        {
          kind: "unreadable",
          message: `vendored is a submodule (gitlink) pointing at commit ${GITLINK_TARGET}`,
        },
      ],
  ...
  it("reports binary and oversized content per file, tolerating oversized blobs in the batch", async () => {
    let t = makeCache();
    ...
    let measuredOid = await t.cache
      .putFromGatekeeper(G1, "blob", measured)
      .catch((err: GitObjectTooLargeError) => err.oid);
    ...
    let tree = await t.cache.putFromGatekeeper(
      G1,
      "tree",
      treePayload([
    ...
    let result = await t.cache.readFilesAtCommit(commit, [
      "huge-measured.txt",
      "text.txt",
      "huge-omitted.txt",
      "binary.png",
    ...
    expect(result).toStrictEqual([
      [
        ...
        {
          kind: "unreadable",
          message: `huge-measured.txt is too large to read (over ${MAX_GIT_OBJECT_SIZE} bytes)`,
        },
      ],
      ...
      [
        ...
        {
          kind: "unreadable",
          message: `huge-omitted.txt is too large to read (over ${MAX_GIT_OBJECT_SIZE} bytes)`,
        },
      ],
    ...
    expect(t.pulls).toHaveLength(1);
    expect([...t.pulls[0].oids].toSorted()).toStrictEqual(
      [omittedOid, textOid, binaryOid].toSorted(),
    );
  });

  it("stops after the response budget, omitting the rest for the client to re-request", async () => {
    let t = makeCache();
    let blob = await storeLocal(t.storage, {
      type: "blob",
      payload: new Uint8Array(MAX_GIT_OBJECT_SIZE).fill(0x61),
    });
    ...
    let commit = await storeLocal(t.storage, {
      type: "commit",
      payload: commitPayload(tree, [], "big"),
    });
    ...
    expect(
      result.every(([, file]) => file.kind === "text" && file.text.length === MAX_GIT_OBJECT_SIZE),
    ).toBe(true);
    expect(t.pulls).toHaveLength(0);
  });
  ...
  it("fails the whole call on a pull failure rather than answering per file", async () => {
    ...
    await expect(t.cache.readFilesAtCommit(COMMIT_1, ["README.md", "nope.txt"])).rejects.toThrow(
      /Could not pull git object/,
    );
  });
});
