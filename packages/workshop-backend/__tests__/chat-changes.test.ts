...
async function submit(
  ...
  author: AiChatAuthorInfo = USER,
  userId: string = USER_DO_ID,
...
describe("submitCodeChange", () => {
  ...
  it("accepts a pin at the head's parent but rejects older bases", () =>
    withImpl(async (impl) => {
      let c1 = await commitFiles(impl, { "a.txt": "one\n" });
  ...
  it("rejects stale generations, conflicting pins, headless pins, and missing declarations", () =>
    withImpl(async (impl) => {
      ...
      await expect(
        submit(impl, 1, { generation: 1, revision: 0, clientId: "c1", seq: 1, change }),
      ...
      await expect(
        submit(impl, 1, { generation: 0, revision: 0, clientId: "c3", seq: 1, change }),
  ...
  it("validates the transformed change against current content", () =>
    withImpl(async (impl) => {
      let c1 = await commitFiles(impl, { "a.txt": "one\n" });
      addGadget(impl, 1, "APP", c1);
      addChat(impl, 1);
  ...
  it("rejects a base beyond the retention horizon", () =>
    withImpl(async (impl) => {
      let c1 = await commitFiles(impl, { "a.txt": "middle\n" });
      addGadget(impl, 1, "APP", c1);
      addChat(impl, 1);
  ...
  it("rejects submissions while an agent turn is active", () =>
    withImpl(async (impl) => {
      let c1 = await commitFiles(impl, { "a.txt": "one\n" });
      addGadget(impl, 1, "APP", c1);
      addChat(impl, 1);
...
describe("submitCodeChange dedupe", () => {
  it("acknowledges a retry with its recorded landing spot without re-applying", () =>
    withImpl(async (impl) => {
      ...
      await expect(submit(impl, 1, { ...submission, seq: 3 })).rejects.toThrow(/Out-of-sequence/);
      await expect(submit(impl, 1, { ...submission, clientId: "fresh", seq: 2 })).rejects.toThrow(
        /Unknown client session/,
      );
    }));
  ...
  it("scopes dedupe records to the authenticated user", () =>
    withImpl(async (impl) => {
      let c1 = await commitFiles(impl, { "a.txt": "one\n" });
      addGadget(impl, 1, "APP", c1);
      addChat(impl, 1);
...
describe("mergeChanges", () => {
  it("commits, fast-forwards, and closes the epoch content-preservingly", () =>
    withImpl(async (impl) => {
      ...
      expect(await impl.gitStore.readCommitFiles(head)).toEqual(
        new Map([["a.txt", "one\nedited\n"]]),
      );
  ...
  it("gives up when a row lands during the accept's awaits, preserving it", () =>
    withImpl(async (impl) => {
      ...
      await expect(impl.mergeChanges(1, USER_META, "user-do-id")).rejects.toThrow(
        /actively edited/,
      );
...
describe("straggler bridge", () => {
  ...
  it("carries a submission across a merge, pinning at the boundary commit", () =>
    withImpl(async (impl) => {
      ...
      expect((await impl.gitStore.readCommitLog(newHead, { depth: 1 }))[0].parents).toEqual([head]);
    }));
  ...
  it("does not bridge across a destructive bump", () =>
    withImpl(async (impl) => {
      let c1 = await commitFiles(impl, { "a.txt": "one\n" });
      addGadget(impl, 1, "APP", c1);
      addChat(impl, 1);
...
describe("revert and draft discard", () => {
  ...
  it("discard-all reverts through the conversion boundary and its legacy batches", () =>
    withImpl(async (impl) => {
      ...
      await expect(impl.revertChanges(1, boundary, USER)).rejects.toThrow(/discarded together/);
...
describe("updateChatFromMainline", () => {
  it("merges only pinned-and-behind gadgets as a change row, and its batch cannot be reverted", () =>
    withImpl(async (impl) => {
      ...
      let mainlineBatch = chatMessages(impl, 1).find(
        (msg) => msg.type === "changes" && msg.mainlineMerge !== undefined,
      )!;
      ...
      await expect(impl.revertChanges(1, mainlineBatch.sequence, USER)).rejects.toThrow(
...
describe("agent step barrier", () => {
  ...
  it("persists the step message, appends rows in order, and materializes -- one transaction", () =>
    withImpl(async (impl) => {
      ...
      expect(
        await impl.commitAgentStep(1, AGENT, stepMsgs("wrote files"), {
          ...NO_EXTRAS,
          changes: [
            {
      ...
      expect(rows.map((row: any) => [row.revision, row.author.type, row.retired])).toEqual([
        [1, "agent", true],
        [2, "agent", true],
      ]);
      expect(impl.storage.chatMeta.get(1)!.codeBase!.pins).toEqual([
      ...
      expect(
        await impl.commitAgentStep(1, AGENT, stepMsgs("just talk"), { ...NO_EXTRAS, changes: [] }),
      ).toBe(false);
      expect(chatMessages(impl, 1).map((msg) => msg.type)).toEqual([
        "message",
        "changes",
        "message",
      ]);
  ...
  it("writes the step's buffer as one message even past the message byte budget", () =>
    withImpl(async (impl) => {
      ...
      expect(
        await impl.commitAgentStep(1, AGENT, stepMsgs("big step"), {
          ...NO_EXTRAS,
          changes: [
  ...
  it("a mid-barrier failure leaves no partial durable state and the chat stays usable", () =>
    withImpl(async (impl) => {
      ...
      // make impossible.
      await expect(
        impl.commitAgentStep(1, AGENT, stepMsgs("doomed"), {
          ...NO_EXTRAS,
          changes: [
            {
              change: { 1: [["a.txt", { set: "one\nagent\n" }]] },
      ...
      expect(
        await impl.commitAgentStep(1, AGENT, stepMsgs("retried"), {
          ...NO_EXTRAS,
          changes: [
            {
              change: { 1: [["a.txt", { set: "one\nagent\n" }]] },
      ...
      expect(chatMessages(impl, 1).find((msg) => msg.type === "changes")!.watermark).toEqual({
        changesGeneration: 0,
        throughRevision: 1,
      });
      expect(await gadgetContent(impl, 1, 1)).toEqual({ "a.txt": "one\nagent\n" });
  ...
  it("a pin whose base is no longer the head fails the barrier with no durable trace", () =>
    withImpl(async (impl) => {
      ...
      setHead(impl, 1, c1);
      await expect(
        impl.commitAgentStep(1, AGENT, stepMsgs("stale pin"), {
          ...NO_EXTRAS,
          changes: [
            { change: { 1: [["a.txt", { set: "x" }]] }, pin: { gadgetId: 1, baseCommit: c2 } },
          ],
        }),
  ...
  it("stamps a pending gadget creation with the step's changes message", () =>
    withImpl(async (impl) => {
      ...
      expect(
        await impl.commitAgentStep(1, AGENT, stepMsgs("created a gadget"), {
      ...
      expect(impl.storage.gadgets.get(created.id)!.pending).toEqual({
        chatId: 1,
  ...
  it("removes a stamped record once the log marks its creation reverted", () =>
    withImpl(async (impl) => {
      ...
      await impl.commitAgentStep(1, AGENT, [{ type: "message", message: "created a gadget" }], {
        changes: [{ change: { [created.id]: [["main.js", { set: "code\n" }]] } }],
        createdGadgets: [{ gadgetId: created.id, title: created.title, bindingName: "MY_GADGET" }],
        createdWorktrees: [],
        addedBindings: [],
        worktreeCommits: [],
      });
...
describe("proposed-changes derivation", () => {
  ...
  it("a revert re-broadcasts derived metadata after reaping the doomed creation", () =>
    withImpl(async (impl) => {
      ...
      impl.materializeChatChanges(1, undefined, {
        author: USER,
        createdGadgets: [{ gadgetId: created.id, title: "Mine", bindingName: "MINE" }],
      });
      expect(derived(impl, 1)).toEqual([created.id]);
