...
describe("AiGatewayConfig transport selection", () => {
  ...
  it("requires the account id", () => {
    expect(() => new AiGatewayConfig(env({ CF_AI_GATEWAY_ACCOUNT_ID: undefined }))).toThrow(
...
describe("getAiGatewayLogCost", () => {
      ...
      vi.fn(
        async () =>
          ({
            ...
            status: 200,
            json: async () => {
              throw new Error("response body reset");
            },
          }) as Response,
      ),
    );
  ...
  it("rejects failed or malformed cross-account responses", async () => {
    ...
    await expect(getAiGatewayLogCost(env(), route, "log-id")).rejects.toThrow(
      "AI Gateway log request failed with status 403.",
    );
    await expect(getAiGatewayLogCost(env(), route, "log-id")).rejects.toThrow(
      "AI Gateway log response contained an invalid cost.",
    );
    await expect(getAiGatewayLogCost(env(), route, "log-id")).rejects.toThrow(
      "AI Gateway log response contained an invalid cost.",
    );
    await expect(getAiGatewayLogCost(env(), route, "log-id")).rejects.toBeInstanceOf(
      AiGatewayLogRetryableError,
    );
    await expect(getAiGatewayLogCost(env(), route, "log-id")).rejects.toBeInstanceOf(
      AiGatewayLogRetryableError,
