...
describe("GitStore", () => {
  ...
  it("survives Cap'n Web in Overseer.getCodeAtCommit's entry-list shape", async () => {
    ...
    let store = new GitStore(makeObjects());
    let files = new Map(
      ["__proto__", "constructor", "toString", "toJSON", "lib/hasOwnProperty"].map(
        (name, i) => [name, `v${i}\n`] as const,
      ),
    );
  ...
  it("diffs commits by path with changedPaths", async () => {
    let store = new GitStore(makeObjects());
    await writeFixtureHistory(store);
    let third = await store.writeFilesAsCommit(
      new Map([
    ...
    expect(await store.changedPaths(INITIAL_COMMIT_OID, SECOND_COMMIT_OID)).toEqual(
      new Set(["client.js"]),
    );
    expect(await store.changedPaths(SECOND_COMMIT_OID, third)).toEqual(
      new Set(["client.js", "lib/util.js", "extra.txt"]),
    );
    // An undefined side is an empty tree, so a one-sided diff lists the whole tree.
    expect(await store.changedPaths(undefined, INITIAL_COMMIT_OID)).toEqual(
      new Set(INITIAL_FILES.keys()),
    );
    expect(await store.changedPaths(INITIAL_COMMIT_OID, undefined)).toEqual(
      new Set(INITIAL_FILES.keys()),
    );
  });
  ...
  it("rejects malformed file paths", async () => {
    ...
    for (let path of ["a//b", "/a", "a/", ".", "a/../b"]) {
      await expect(store.writeFilesAsCommit(new Map([[path, "x"]]), options)).rejects.toThrow(
        "invalid file path",
      );
    }
    ...
    await expect(store.writeFilesAsCommit(collision, options)).rejects.toThrow(
...
describe("writeChangedFilesAsCommit", () => {
  ...
  it("applies edits while reusing unchanged subtree oids verbatim", async () => {
    ...
    expect(await store.changedPaths(COMMIT_1, commit)).toStrictEqual(
      new Set(["README.md", "src/util.js"]),
    );
    ...
    expect(next.entries.find((e) => e.name === "docs")!.oid).toBe(
      base.entries.find((e) => e.name === "docs")!.oid,
    );
    // And the parent is as declared.
    expect(
      parseGitCommitRefs(decodeLooseObject(objects.get(commit)!.data).payload).parents,
    ).toStrictEqual([COMMIT_1]);
  });

  it("separates treeBase from parents (squash semantics)", async () => {
    let { objects, store } = makeFixtureStore();
    let commit = await store.writeChangedFilesAsCommit(new Map([["README.md", "squashed\n"]]), {
      ...OPTIONS,
      parents: [COMMIT_3],
    }); // tree from COMMIT_1, parent COMMIT_3
    expect(
      parseGitCommitRefs(decodeLooseObject(objects.get(commit)!.data).payload).parents,
    ).toStrictEqual([COMMIT_3]);
    expect(await store.changedPaths(COMMIT_1, commit)).toStrictEqual(new Set(["README.md"]));
  });
  ...
  it("preserves an edited executable's mode and defaults new files to 100644", async () => {
    ...
    expect(next.entries.find((e) => e.name === "link.md")).toStrictEqual(
      base.entries.find((e) => e.name === "link.md"),
    );
    expect(next.entries.find((e) => e.name === "vendored")).toStrictEqual(
      base.entries.find((e) => e.name === "vendored"),
    );
  });
  ...
  it("rejects changes landing on non-regular-file entries", async () => {
    ...
    await expect(
      store.writeChangedFilesAsCommit(new Map([["link.md", "x"]]), OPTIONS),
    ).rejects.toThrow("cannot write link.md: not a regular file");
    await expect(
      store.writeChangedFilesAsCommit(new Map([["vendored", "x"]]), OPTIONS),
    ).rejects.toThrow("cannot write vendored: not a regular file");
    await expect(store.writeChangedFilesAsCommit(new Map([["src", "x"]]), OPTIONS)).rejects.toThrow(
      "cannot write src: not a regular file",
    );
    await expect(
      store.writeChangedFilesAsCommit(new Map([["src", null]]), OPTIONS),
    ).rejects.toThrow("cannot delete src: it is a directory");
    await expect(
      store.writeChangedFilesAsCommit(new Map([["README.md/x", "y"]]), OPTIONS),
  ...
  it("prunes directories emptied by deletions and creates new nested ones", async () => {
    ...
    expect(entriesOf(objects, commit, "a/deep").entries.map((e) => e.name)).toStrictEqual([
      "new.txt",
    ]);
    // The prune cascades: deleting a nested directory's last file drops every directory the
    // deletion emptied, all the way up.
    let cascade = await store.writeChangedFilesAsCommit(new Map([["a/deep/new.txt", null]]), {
      ...OPTIONS,
      treeBase: commit,
      parents: [commit],
    });
    ...
    let again = await store.writeChangedFilesAsCommit(
      new Map([["never-existed.txt", null]]),
      OPTIONS,
    );
...
describe("makeGitObjectsFs", () => {
  ...
  it("rejects paths outside the loose-object layout", async () => {
    let fs = makeGitObjectsFs(makeObjects());
    await expect(fs.promises.readFile(`${GITDIR}/config`)).rejects.toMatchObject({
      code: "ENOENT",
    });
    await expect(fs.promises.stat(GITDIR)).rejects.toMatchObject({ code: "ENOENT" });
    await expect(
      fs.promises.writeFile(`${GITDIR}/refs/heads/main`, new Uint8Array()),
    ).rejects.toMatchObject({ code: "EPERM" });
    await expect(fs.promises.readdir(`${GITDIR}/refs`)).rejects.toMatchObject({ code: "ENOENT" });
...
describe("commitIdentityForAuthor", () => {
  it("uses the display name and an email profile ID directly", () => {
    expect(
      commitIdentityForAuthor({ type: "user", id: "alice@example.com", name: "Alice Example" }),
    ).toEqual({ name: "Alice Example", email: "alice@example.com" });
  });

  it("gives bare-username profile IDs a placeholder host", () => {
    expect(commitIdentityForAuthor({ type: "user", id: "bob", name: "Bob Builder" })).toEqual({
      name: "Bob Builder",
      email: "bob@localhost",
    });
  });
