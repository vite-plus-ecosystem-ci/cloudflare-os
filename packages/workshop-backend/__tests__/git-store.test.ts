...
describe("GitStore", () => {
  ...
  it("blobOid is the content address a written file gets, computed without writing", async () => {
    ...
    let tree = parseGitCommitRefs(
      decodeLooseObject(objects.get(INITIAL_COMMIT_OID)!.data).payload,
    ).tree;
...
describe("writeChangedFilesAsCommit", () => {
  ...
  it("applies edits while reusing unchanged subtree oids verbatim", async () => {
    ...
    expect(changedPaths(objects, COMMIT_1, commit)).toStrictEqual(
  ...
  it("separates treeBase from parents (squash semantics)", async () => {
    ...
    expect(changedPaths(objects, COMMIT_1, commit)).toStrictEqual(new Set(["README.md"]));
  });
  ...
  it("prefers the author's commit email over the profile ID", () => {
    expect(
      commitIdentityForAuthor({
        ...
        id: "bob",
        name: "Bob Builder",
        commitEmail: "bob@builder.example",
      }),
