...
function makeTurn(impl: any, chatId: number) {
  ...
  let baseCommit = (id: number): string | undefined => pins.get(id) ?? impl.getWorktreePinBase(id);
  let pin = (id: number) => {
  ...
  let access: WorktreeTurnAccess = {
    ...
    appendChange: (id, path, change) => {
      ...
      let updated: CodeContent = applyCodeChange(
        new Map([[id, content.get(id) ?? new Map()]]),
        one,
      );
  ...
  return {
    ...
    barrier: async () =>
      impl.commitAgentStep(chatId, AGENT, [{ type: "message", message: "step" }], {
        changes: changes.splice(0),
        createdGadgets: [],
        createdWorktrees: [],
        addedBindings: [],
        worktreeCommits: commits.splice(0),
      }),
  };
}
...
describe("listFiles", () => {
  ...
  it("overlays writes and removals: new files (and their directories) appear, deletions vanish", () =>
    withImpl(async (impl) => {
      ...
      expect(await session.listFiles("notes")).toEqual([{ path: "notes/todo.txt", kind: "file" }]);
    }));
  ...
  it("prunes directories hollowed out by deletions (git has no empty directories)", () =>
    withImpl(async (impl) => {
      ...
      expect(await session.listFiles(undefined, { recursive: true })).toEqual([
        { path: "top.txt", kind: "file" },
      ]);
  ...
  it("a directory kept alive only by a symlink survives its files' deletion", () =>
    withImpl(async (impl) => {
      ...
      let subTree = await impl.gitCache.putFromGatekeeper(
        999,
        "tree",
        treePayload([
      ...
      let rootTree = await impl.gitCache.putFromGatekeeper(
        999,
        "tree",
        treePayload([
      ...
      let commitOid = await impl.gitCache.putFromGatekeeper(
        999,
        "commit",
        commitPayload(rootTree, [], "symlink fixture"),
      );
...
describe("file operations", () => {
  ...
  it("writeFile falls back to a whole-file set over an unreadable (binary) base", () =>
    withImpl(async (impl) => {
      ...
      let treeOid = await impl.gitCache.putFromGatekeeper(
        999,
        "tree",
        treePayload([{ mode: "100644", name: "img.bin", oid: blobOid }]),
      );
      let commitOid = await impl.gitCache.putFromGatekeeper(
        999,
        "commit",
        commitPayload(treeOid, [], "binary fixture"),
      );
  ...
  it("rejects writes and deletes over symlink, submodule, and directory entries", () =>
    withImpl(async (impl) => {
      ...
      await expect(session.deleteFile("missing.txt")).rejects.toThrow("missing.txt: no such file");
    }));
});
...
describe("grep", () => {
  it("formats single-file and directory results like grep -n, skipping special entries", () =>
    withImpl(async (impl) => {
      ...
      expect(await session.grep(/answer/, "src/util.js")).toBe("1:export const answer = 42;");
      expect(await session.grep(/answer/, "src")).toBe("src/util.js:1:export const answer = 42;");
  ...
  it("returns structured matches and errors", () =>
    withImpl(async (impl) => {
      addChat(impl, 1);
      await loadFixtureRepo(impl);
      let { session } = await createWorktreeSession(impl, 1, COMMIT_1);
  ...
  it("fills a directory's missing blobs in one batched pull", async () => {
    ...
    for (let object of FIXTURE_OBJECTS) {
      if (
        PACKED_OIDS.includes(object.oid) &&
  ...
  it("notes every path sharing one unobtainable oversized blob", async () => {
    ...
    let smallBlob = await cache.putFromGatekeeper(
      7,
      "blob",
      new TextEncoder().encode("x marks the spot\n"),
    );
    let sharedOid = "cd".repeat(20);
    let treeOid = await cache.putFromGatekeeper(
      7,
      "tree",
      treePayload([
    ...
    let commitOid = await cache.putFromGatekeeper(
      7,
      "commit",
      commitPayload(treeOid, [], "shared blob fixture"),
    );
...
describe("commit and diff", () => {
  ...
  it("explicit commits squash out accepts' auto-commits", () =>
    withImpl(async (impl) => {
      addChat(impl, 1);
      let c1 = await commitFiles(impl, { "a.txt": "one\n" });
      let { id, session, turn } = await createWorktreeSession(impl, 1, c1);
  ...
  it("an edited executable keeps its mode; untouched special entries ride through", () =>
    withImpl(async (impl) => {
      ...
      expect((await impl.gitCache.pathEntryAtCommit(commit, "run.sh"))!.kind).toBe("executable");
      expect((await impl.gitCache.pathEntryAtCommit(commit, "link.md"))!.kind).toBe("symlink");
      expect((await impl.gitCache.pathEntryAtCommit(commit, "vendored"))!.kind).toBe("submodule");
  ...
  it("diff() renders unified diffs of the overlay against a commit", () =>
    withImpl(async (impl) => {
      addChat(impl, 1);
      let c1 = await commitFiles(impl, { "a.txt": "one\ntwo\n", "b.txt": "bee\n" });
      let { session } = await createWorktreeSession(impl, 1, c1);
  ...
  it("diff() reports EOF-newline changes with git's no-newline markers", () =>
    withImpl(async (impl) => {
      addChat(impl, 1);
  ...
  it("diff() notes special entries instead of failing", () =>
    withImpl(async (impl) => {
      addChat(impl, 1);
      await loadFixtureRepo(impl);
      let c1 = await commitFiles(impl, { "a.txt": "one\n" });
  ...
  it("diff() notes only unrenderable content; operational read failures propagate", async () => {
    ...
    let renderableTree = await cache.putFromGatekeeper(
      7,
      "tree",
      treePayload([{ mode: "100644", name: "img.bin", oid: binaryBlob }]),
    );
    let renderableTarget = await cache.putFromGatekeeper(
      7,
      "commit",
      commitPayload(renderableTree, [], "binary target"),
    );
    ...
    let ghostTree = await cache.putFromGatekeeper(
      7,
      "tree",
      treePayload([{ mode: "100644", name: "ghost.txt", oid: ghostOid }]),
    );
    let ghostTarget = await cache.putFromGatekeeper(
      7,
      "commit",
      commitPayload(ghostTree, [], "ghost target"),
    );
  ...
  it("structuredDiff() returns numbered hunks per changed file", () =>
    withImpl(async (impl) => {
      addChat(impl, 1);
      let c1 = await commitFiles(impl, {
        "a.txt": "one\ntwo\n",
        ...
        files: [
          ...
          },
          {
            ...
            hunks: [
              {
                header: "@@ -1,1 +0,0 @@",
                lines: [{ kind: "removed", text: "bee", oldLineNumber: 1 }],
              },
            ],
          },
          {
            ...
            hunks: [
              {
                header: "@@ -0,0 +1,1 @@",
                lines: [{ kind: "added", text: "sea", newLineNumber: 1 }],
              },
            ],
          },
      ...
      await expect(session.structuredDiff(c1.slice(0, 8))).rejects.toThrow(
  ...
  it("structuredDiff() reports special entries as errors", () =>
    withImpl(async (impl) => {
      addChat(impl, 1);
      await loadFixtureRepo(impl);
      let c1 = await commitFiles(impl, { "a.txt": "one\n" });
      ...
      expect(result.errors).toEqual(
        expect.arrayContaining([
          { file: "link.md", error: "link.md is a symlink to README.md" },
          {
            file: "vendored",
            error:
              "vendored is a submodule (gitlink) pointing at commit " +
              "1111111111111111111111111111111111111111",
          },
        ]),
      );
    ...
    withImpl(async (impl) => {
      ...
      let commitWithModes = async (entries: { mode: string; name: string; oid: string }[]) => {
        ...
        return await impl.gitCache.putFromGatekeeper(
          999,
          "commit",
          commitPayload(tree, [], JSON.stringify(entries)),
        );
      };
      ...
      let scriptHunks = [
        {
          header: "@@ -0,0 +1,1 @@",
          lines: [{ kind: "added", text: "#!/bin/sh", newLineNumber: 1 }],
        },
      ];
