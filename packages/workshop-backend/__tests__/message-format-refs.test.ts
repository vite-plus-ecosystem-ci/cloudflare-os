...
describe("sanitizeMessageFormatRefs", () => {
  it("keeps well-formed refs, sorted by position", () => {
    let result = sanitizeMessageFormatRefs(
      [ref({ position: SLIDES_AT, length: 6, noun: "Slides", icon: "presentation" }), ref()],
      MESSAGE,
    );
  ...
  it("drops refs naming an icon outside the closed set", () => {
    expect(
      sanitizeMessageFormatRefs([ref({ icon: "skull" as MessageFormatRef["icon"] })], MESSAGE),
    ).toBeUndefined();
  });
  ...
  it("drops the second of two overlapping refs", () => {
    ...
    let result = sanitizeMessageFormatRefs(
      [ref(), ref({ position: DOC_AT + 1, length: 2, noun: "oc" })],
      MESSAGE,
    );
  ...
  it("keeps a repeated format that appears twice in the text", () => {
    ...
    let result = sanitizeMessageFormatRefs(
      [ref({ position: 2 }), ref({ position: message.lastIndexOf("Doc") })],
      message,
    );
  ...
  it("drops an unreasonably long noun", () => {
    ...
    expect(
      sanitizeMessageFormatRefs([ref({ position: 0, length: long.length, noun: long })], long),
    ).toBeUndefined();
  });
});
