...
import { collectSlashCommands, invokeSlashCommand } from "../src/slash-commands";
    ...
    getSlashCommandProvider() {
      let provider = {
        ...
        async invoke(id: string, message: string, authorizer: unknown) {
          return options.invoke ? options.invoke(id, message, authorizer) : { message: id };
        },
        [Symbol.dispose]() {
          options.onDispose?.();
        },
...
describe("slash command helpers", () => {
  ...
  it("forwards arguments and keeps the provider alive until invocation completes", async () => {
    ...
    let invoked = vi.fn(
      (_id: string, _message: string, _authorizer: unknown) =>
