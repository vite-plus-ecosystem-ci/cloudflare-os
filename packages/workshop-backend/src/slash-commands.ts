...
import type { SlashCommandChoice, SlashCommandRequest } from "@gadgets/workshop-shared/api";

type SlashCommandGatekeeper = Fetcher<
  Gatekeeper<any> & Required<Pick<Gatekeeper<any>, "getSlashCommandProvider">>
>;
...
): Promise<SlashCommandChoice[]> {
  let catalogs = await Promise.all(
    sources.map(async (source) => {
      try {
        using provider = await (
          source.gatekeeper as SlashCommandGatekeeper
        ...
        console.error(
          `Failed to load slash commands for gatekeeper ${source.gatekeeperId}:`,
          error,
        );
        return [];
      }
