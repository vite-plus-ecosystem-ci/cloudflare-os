...
`,
  ...
  it("rejects malformed frontmatter with an actionable diagnostic", () => {
    expect(() => parseSkillManifest("skills/example/SKILL.md", "# Missing frontmatter")).toThrow(
      "Skill manifest must start with YAML frontmatter.",
    );
    expect(() => parseSkillManifest("skills/example/SKILL.md", "---\nname: example\n")).toThrow(
      "Skill manifest frontmatter is not closed.",
    );
    expect(() => parseSkillManifest("skills/example/SKILL.md", "---\nname: [\n---\nBody")).toThrow(
    ...
    expect(buildAgentSkillCatalogEntries(loaded)).toEqual([
      {
        ...
        title: "deploy",
        description:
          "Agent Skill. Read with env[N].read(id) and console.log(document.content). Deploy for engineering",
      },
      {
        id: "sales/deploy/SKILL.md",
        title: "deploy",
        description:
          "Agent Skill. Read with env[N].read(id) and console.log(document.content). Deploy for sales",
      },
    ]);
  });
...
describe("buildAgentSkillMessage", () => {
  ...
  let root =
    'skill root: lib/skills/de"ck/ — read the documents it references before following ' +
  ...
  it("replaces $ARGUMENT with the full argument text", () => {
    expect(
      buildAgentSkillMessage(docId, "Create a presentation about $ARGUMENT.", "CloudflareOS"),
  ...
  it("appends the argument when the skill has no placeholder", () => {
    expect(
      buildAgentSkillMessage(docId, "Create a clear presentation.", "Make it nice and clean"),
    ).toBe(
