...
const IGNORES_SIGNALS = "process.on('SIGTERM', () => {}); process.on('SIGINT', () => {}); " + IDLE;
...
async function spawnWrapper({
  ...
  const grandchildProgram = `${grandchildBody};require("node:fs").writeFileSync(${JSON.stringify(readyFile)}, "")`;
...
describe(
  "relayTermination",
  {
    concurrency: true,
    skip:
      process.platform === "win32" ? "the relay and these signal fixtures are POSIX-only" : false,
  },
  () => {
    ...
    it("reaches a descendant that ignores the forwarded signal, via the escalation", async () => {
      const { wrapperPid, grandchildPid, exit, cleanUp } = await spawnWrapper({
        grandchildBody: IGNORES_SIGNALS,
        graceMs: 250,
      });
      try {
    ...
    it("gives a descendant the full grace after the child has already exited", async () => {
      ...
      const { wrapperPid, grandchildPid, exit, cleanUp } = await spawnWrapper({
        grandchildBody: cleansUpSlowly(cleaned),
        graceMs: 3_000,
      });
      try {
        process.kill(wrapperPid, "SIGTERM");
        assert.ok(
          ...
          "the grandchild outlived the forwarded signal",
        );
        ...
        assert.ok(
          existsSync(cleaned),
          "the grandchild was SIGKILLed before it finished cleaning up",
        );
        assert.deepEqual(await exit, { code: null, signal: "SIGTERM" });
      } finally {
        cleanUp();
    ...
    it("still reaps a stubborn grandchild when the signal is group-delivered", async () => {
      ...
      try {
        ...
        assert.ok(
          await waitUntilGone(grandchildPid),
          "the grandchild survived the group broadcast",
        );
    ...
    it("delivers one group-broadcast interrupt to a descendant twice", async () => {
      ...
      try {
        process.kill(-wrapperPid, "SIGINT");
        assert.ok(
          await waitForOutput(/^sigint 2$/m),
